class DataHelper {
    ...
    async update() {
        ...
        if (isNaN(this.lastUpdate) || new Date - this.lastUpdate > 5 * 60 * 1e3) {
            ...
            this.loading = true;
            let players = {};
          await this._updateHighscore(players).then((players => this._updatePlayers(players))).then((players => this._updatePlanets(players))).then((players => this._updateAlliances(players))).then((players => {
    ...
    _fetchXML(url) {
        return fetch(url).then((rep => rep.text())).then((str => (new window.DOMParser).parseFromString(str, "text/xml"))).then((xml => xml))
    }
    ...
    _updatePlayers(players) {
        return fetch(`https://${this.universe}.ogame.gameforge.com/api/players.xml`).then((rep => rep.text())).then((str => (new window.DOMParser).parseFromString(str, "text/xml"))).then((xml => {
    ...
    _updatePlanets(players) {
        return fetch(`https://${this.universe}.ogame.gameforge.com/api/universe.xml`).then((rep => rep.text())).then((str => (new window.DOMParser).parseFromString(str, "text/xml"))).then((xml => {
            let update = new Date(Number(xml.children[0].getAttribute("timestamp")) * 1e3);
    ...
    _updateAlliances(players) {
        return fetch(`https://${this.universe}.ogame.gameforge.com/api/alliances.xml`).then((rep => rep.text())).then((str => (new window.DOMParser).parseFromString(str, "text/xml"))).then((xml => {
