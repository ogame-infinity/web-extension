...
class OGLight {
  ...
  init() {
    ...
    this.language =
        ['fr', 'de', 'es', 'pl', 'it'].includes(this.gameLang) ? 0 : 1;
  ...
  betterFleetDispatcher() {
    ...
    if (this.page == 'fleetdispatch' && document.querySelector('#civilships') &&
      ...
      $('#shortcuts .combatunits').on('change', () => {
        ...
        delayTimeDiv2 = document.querySelector('#fleet3 #arrivalTime')
  ...
  welcome() {
    ...
    if ('fr'.indexOf(this.gameLang) == -1) {
      languageDiv.html(`<div class='mmoflag mmo_${
          this.gameLang
              .toUpperCase()}'> </div><strong class="neutral"> Expeditions statistics are not available in your language at the moment. Please also see the settings page to help me make it available in your language...</strong>`);
    }

    container.appendChild(this.createDOM(
        'p', {class: 'ogk-tips friendly'},
        'Oh, and here are some quick tips: '));

    let ctrl = container.appendChild(this.createDOM(
        'p', {
          style:
  ...
  sideOptions() {
    ...
    let empireBtn = harvestOptions.appendChild(this.createDOM('div', {
        ...
        document.querySelectorAll(`div[id=${id}] li.msg`).forEach((msg) => {
          ...
          if (result.result == 'Fleet') {
            for (let [key, value] of Object.entries(result.fleet)) {
              sums.fleet[key] ? (sums.fleet[key] += value) :
          ...
          sums.type[result.result] ? (sums.type[result.result] += 1) :
          ...
          this.json.expeditions[id].favorited =
              msg.querySelector('.icon_favorited') ? true : false;
          this.saveData();
        });
      }
  ...
  expeditionTextToJson(content) {
    ...
    if (textContent.includes('nouveaux vaisseaux')) {
      ...
      textContent.match(/.*: [1-9].*/gm).forEach((result) => {
        ...
        textContent.includes('accélérer') || textContent.includes('avance')) {
      json.result = 'Early';
    } else if (
        textContent.includes('fausse route') ||
        textContent.includes('retard') || textContent.includes('dommages') ||
  ...
  ressourceFiller() {
    if (this.page == 'fleetdispatch' && document.querySelector('#civilships') &&
      ...
      let update = (submit) => {
        ...
        if (fleetDispatcher.targetPlanet.position ===
    ...
    this.planetList.forEach((planet) => {
      ...
      let pTimer = planet.querySelector('.planetlink')
                       .appendChild(this.createDOM('div', {
                         ...
                         'data-timer': Math.min(
                             Math.round((now - timers[0]) / 60000), 60),
                       }));
  ...
  updateStalk(planets) {
    ...
    sorted.forEach((planet) => {
      ...
      let plaspy =
          panel.appendChild(this.createDOM('button', {class: 'icon_eye'}));
      let plaFleet =
          panel.appendChild(this.createDOM('div', {class: 'ogl-atk'}));
...
`);
  ...
  spyTable() {
    ...
    document.querySelectorAll('.msg').forEach((msg) => {
      ...
      report.apiKey = msg.querySelector('.icon_apikey')
                          .getAttribute('title')
                          .split('\'')[1];
...
, rgba(115, 229, 255, 0.78) ${
  ...
  settings() {
    ...
    if ('fr'.indexOf(this.gameLang) == -1) {
      let lang = dataDiv.appendChild(this.createDOM(
