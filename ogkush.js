...
class OGInfinity {
    ...
    start() {
        ...
        sendShips = function (order, galaxy, system, planet, planettype, shipCount) {
            if (shipsendingDone == 1) {
                shipsendingDone = 0;
                let params = { mission: order, galaxy: galaxy, system: system, position: planet, type: planettype, shipCount: shipCount, token: token };
                $.ajax(miniFleetLink, {
                    data: params, dataType: "json", type: "POST", success: function (data) {
                        if (!data.response) {
                            shipsendingDone = 1;
                            ...
                            if (typeof data.newAjaxToken != "undefined") {                               
                                ...
                                let page = rawURL.searchParams.get("component") || this.rawURL.searchParams.get("page");
                                if (page === "galaxy") {                                 
                                   let phalanxElements = document.querySelectorAll("#galaxyContent .phalanxlink");
                                    for (let i = 0; i < phalanxElements.length; i++) {                                                                 
                                        $(phalanxElements[i]).data('overlay-token', data.newAjaxToken);
                                    }
                                }
                            }
    ...
    technoDetail() {
        if (this.page == "research" || this.page == "supplies" || this.page == "facilities" || this.page == "shipyard" || this.page == "defenses") {
            ...
            if (this.page == "research") {
                ...
                this.json.tech124 = Number(document.querySelector('.technology[data-technology="124"] .level').getAttribute("data-value"));
                this.json.tech108 = Number(document.querySelector('.technology[data-technology="108"] .level').getAttribute("data-value"))
            }
            ...
            let currentRes = [document.querySelector("#resources_metal").innerText.split(".").join(""), document.querySelector("#resources_crystal").innerText.split(".").join(""), document.querySelector("#resources_deuterium").innerText.split(".").join("")];
            ...
            technologyDetails.show = function (technologyId) {
                ...
                $.ajax({ url: this.technologyDetailsEndpoint, data: { technology: technologyId } }).done((function (data) {
                    ...
                    if ([202, 203, 208, 209, 210, 204, 205, 206, 219, 207, 215, 211, 212, 217, 213, 218, 214, 401, 402, 403, 404, 405, 406, 407, 408, 502, 503].includes(technologyId)) {
                        ...
                        updateShipDetails(1);
                        document.querySelector(".maximum") && document.querySelector(".maximum").addEventListener("click", (() => {
    ...
    initUnionCombat(union) {
        if (this.unionInterval) {
            ...
            this.delayTimeDiv = document.querySelector("#fleetBriefingPart1 li:first-of-type .value").appendChild(this.createDOM("div", { class: "undermark" }));
            this.delayTimeDiv2 = document.querySelector("#fleet2 #arrivalTime").parentElement.appendChild(this.createDOM("div", { class: "undermark" }));
            this.delayDiv2 = document.querySelector("#missionNameWrapper").appendChild(this.createDOM("div", { class: "ogk-delay" }));
            this.delayTimeDiv3 = document.querySelector("#fleet1 .ogl-info").appendChild(this.createDOM("div", {
                class: "undermark",
        ...
        let update = () => {
            ...
            this.delayDiv2.innerText = end > 0 ? "Time to join " + getFormatedTime(abs) : "Too late to join !" + getFormatedTime(abs);
            this.delayDiv3.innerText = end > 0 ? "Time to join " + getFormatedTime(abs) : "Too late to join " + getFormatedTime(abs);
            if (end > 0) {
    ...
    updateServerSettings() {
        ...
        return fetch(settingsUrl).then((rep => rep.text())).then((str => (new window.DOMParser).parseFromString(str, "text/xml"))).then((xml => {
    ...
    addGalaxyTooltips() {
        document.querySelectorAll(".tooltipRel").forEach((sender => {
            ...
            if (rel.indexOf("player") == 0 && rel != "player99999") {
                ...
                sender.appendChild(this.createDOM("a", {
                    href: this.generateHiscoreLink(id) || "",
                    class: "ogl-ranking"
    ...
    addGalaxyMarkers() {
        ...
        document.querySelectorAll("#galaxyContent .galaxyRow.ctContentRow .galaxyCell.cellAlliance").forEach(((element, index) => {
            ...
            let id = playerDiv && playerDiv.getAttribute("rel") && playerDiv.getAttribute("rel").replace("player", "") || 99999;
            let coords = galaxy + ":" + system + ":" + Number(index + 1);
            let colors = this.createDOM("div", { class: "ogl-colors", "data-coords": coords, "data-context": "galaxy" });
    ...
    scan() {
        ...
        let secureCoords = document.getElementById("galaxy_input").value + ':' + document.getElementById("system_input").value;
        if (secureCoords !== baseCords) {
            //FIX FALSE NEW PLANETS;
            return;
        }
        document.querySelectorAll("#galaxycomponent .galaxyRow.ctContentRow").forEach(((row, index) => {
            ...
            if (playerDiv) {
                ...
                //PROBLEM IS HERE !!!!
                changes.push({ id: playerId, name: name, planetId: planetId, moon: row.querySelector(".cellMoon .tooltipRel") ? true : false, moonId: moonId, coords: coords });
                //console.log(changes);
                ...
                // PTRE activities
                if (this.json.options.ptreTK && this.json.options.ptreTK && planetId > -1 && playerId > -1 && (this.json.sideStalk.indexOf(playerId) > -1 || this.markedPlayers.indexOf(playerId) > -1)) {

                    let planetActivity = row.querySelector('[data-planet-id] .activity.minute15') ? '*' : row.querySelector('[data-planet-id] .activity')?.textContent.trim() || 60;
                    let moonActivity = row.querySelector('[data-moon-id] .activity.minute15') ? '*' : row.querySelector('[data-moon-id] .activity')?.textContent.trim() || 60;
    ...
    async ptreActivityUpdate(ptreJSON, systemCoords) {
        ...
        fetch('https://ptre.chez.gg/scripts/oglight_import_player_activity.php', { priority: 'low', method: 'POST', body: JSON.stringify(ptreJSON) })
    ...
    async flyingFleet() {
        ...
        let inter = setInterval((() => {
            ...
            if (eventList) {
                ...
                document.querySelector(".event_list").appendChild(this.createDOM("span", {
    ...
    welcome() {
        ...
        container.appendChild(this.createDOM("p", {}, "<strong class='friendly'>Note</strong>: Ogame Infinity is now officially tolarated by Ogame! (<a href='https://board.en.ogame.gameforge.com/index.php?thread/819842-ogame-infinity-extension/' target='_blank'>Origin board</a>)."));
        if (!this.commander) {
        ...
        container.appendChild(this.createDOM("p", {}, "If you see a bug or have a feature request please report to discord 🙏 <a href='https://discord.gg/Z7MDHmk' target='_blank'>Link</a> also in the setting page. <span class='overmark'> Be advised that using multiple addons/script might generate conflicts. </span>"));
        let shortcutsDiv = container.appendChild(this.createDOM("p", {
        ...
        let heart = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path style="fill:#C80909" d="M12 4.435c-1.989-5.399-12-4.597-12 3.568 0 4.068 3.06 9.481 12 14.997 8.94-5.516 12-10.929 12-14.997 0-8.118-10-8.999-12-3.568z"/></svg>';
                        ...
                        link.addEventListener("click", (() => {
                            ...
                            let json = {
                                0: [{
                                    ...
                                    planet: { galaxy: coords[0], system: coords[1], position: coords[2] }
                                }]
                            };
                            let base64 = btoa(JSON.stringify(json));
                            window.open(`https://trashsim.universeview.be/en?SR_KEY=${key}#prefill=${base64}`, "_blank")
                        }))
                    } else if (key.startsWith("cr")) {
                        let link = elem.appendChild(this.createDOM("a", { class: "ogk-ogotcha tooltip", title: "Ogotcha" }));
                        link.addEventListener("click", (() => window.open(`https://ogotcha.universeview.be/${this.gameLang}?CR_KEY=${key}`, "_blank", `location=yes,scrollbars=yes,status=yes,width=${screen.availWidth},height=${screen.availHeight}`)))
                    }
                }
            }));
            setTimeout((() => {
    ...
    profitGraph(profits, max, callback) {
        ...
        profits.slice().reverse().forEach(((elem, index) => {
            ...
            span.addEventListener("click", (() => {
                ...
                let contentHtml = `<strong>${getFormatedDate(elem.date.getTime(), "[d].[m].[y]")}</strong> <span class="${elem.profit >= 0 ? "undermark" : "overmark"}">${elem.profit >= 0 ? " +" : " -"}${getNumberFormatShort(Math.abs(elem.profit), 2)}</strong></span>`;
                if (elem.start) {
    ...
    repartitionGraph(eco, tech, fleet, def) {
        ...
        let config = {
            type: "doughnut",
            data: {
                datasets: [{ data: [eco, tech, fleet, def], backgroundColor: ["#656565", "#83ba33", "#b73536", "#3d4800"], borderColor: "#1b232c" }],
    ...
    winGraph(win, draw, count) {
        ...
        let config = {
            type: "doughnut",
            data: {
                datasets: [{ data: [win, count - win - draw, draw], backgroundColor: ["#83ba33", "#b73536", "#d29d00"], borderColor: "#1b232c" }],
    ...
    generalStats(player) {
        ...
        playerDiv.appendChild(this.createDOM("div", { class: "characterclass small sprite " + playerClassName, style: "margin-top: -2px;margin-left: 10px;" }));
        let stats = playerDiv.appendChild(this.createDOM("a", { class: "ogl-mmorpgstats tooltip", title: "test" }));
        ...
        detailRank.html(`\n          <div><div class="ogl-ecoIcon"></div>${dotted(player.economy.score)} <small>pts</small><span class="ogl-ranking">#${player.economy.position} </span></div>\n          <div><div class="ogl-techIcon"></div>${dotted(player.research.score)} <small>pts</small><span class="ogl-ranking">#${player.research.position} </span></div>\n          <div><div class="ogl-fleetIcon"></div>${dotted(player.military.score)} <small>pts</small><span class="ogl-ranking">#${player.military.position} </span></div>\n          <div><div class="ogl-fleetIcon grey"></div>${dotted(player.def)} <small>pts</small></div>\n          `);
        ...
        div.appendChild(this.createDOM("a", { class: "ogl-option ogl-fleet-ship ogl-tech-" + 108 }));
        ...
        prod.appendChild(this.createDOM("span"));
        let innerAstro = prod.appendChild(this.createDOM("span", { style: "display: flex; align-items: center; margin-left: auto; margin-top: 10px;" }));
        innerAstro.appendChild(this.createDOM("span", {}, "Astro"));
        innerAstro.appendChild(this.createDOM("a", { class: "ogl-option ogl-fleet-ship ogl-tech-124", style: "margin-left: 5px; margin-right: 5px;" }));
        innerAstro.appendChild(this.createDOM("span", {}, `<strong>${this.json.tech124 || 0}</strong>`));
        let innerEnergy = prod.appendChild(this.createDOM("span", { style: "display: flex; align-items: center; margin-left: auto; margin-top: 10px;" }));
        innerEnergy.appendChild(this.createDOM("span", {}, "Energy"));
        innerEnergy.appendChild(this.createDOM("a", { class: "ogl-option ogl-fleet-ship ogl-tech-113", style: "margin-left: 5px; margin-right: 5px;" }));
        innerEnergy.appendChild(this.createDOM("span", {}, `<strong>${this.json.tech113 || 0}</strong>`));
        let innerPlasma = prod.appendChild(this.createDOM("span", { style: "display: flex; align-items: center; margin-left: auto; margin-top: 10px;" }));
        innerPlasma.appendChild(this.createDOM("span", {}, "Plasma"));
        innerPlasma.appendChild(this.createDOM("a", { class: "ogl-option ogl-fleet-ship ogl-tech-122", style: "margin-left: 5px; margin-right: 5px;" }));
    ...
    async ptreAction(frame, player) {
        ...
        this.getJSON(`https://ptre.chez.gg/scripts/oglight_get_player_infos.php?team_key=${this.json.options.ptreTK}&pseudo=${cleanPlayerName}&player_id=${player.id}&input_frame=${frame}`, result => {
            if (result.code == 1) {
                ...
                container.innerHTML = `
                            ...
                            <div class="ptreBestReport">
                                <div>
                                    <div><b class="ogl_fleet"><i class="material-icons">military_tech</i>${this.formatToUnits(result.top_sr_fleet_points)} pts</b></div>
                                    <div><b>${new Date(result.top_sr_timestamp * 1000).toLocaleDateString('fr-FR')}</b></div>
                                </div>
                                <div>
                                    <a class="ogl_button" target="_blank" href="${result.top_sr_link}">${(this.gameLang == 'fr' ? 'Détails du rapport' : 'Report Details')}</a>
                                    <a class="ogl_button" target="_blank" href="https://ptre.chez.gg/?country=${this.gameLang}&univers=${this.universe}&player_id=${player.id}">${(this.gameLang == 'fr' ? 'Profil de la cible' : 'Target Profile')}</a>
                                </div>
                            </div>
                ...
                ['last24h', '2days', '3days', 'week', '2weeks', 'month'].forEach(f => {
                    let btn = container.querySelector('.ptreFrames').appendChild(this.createDOM('div', { 'class': 'ogl_button' }, f));
                ...
                if (result.activity_array.succes == 1) {
                    arrData.forEach((line, index) => {
                        if (!isNaN(line[1])) {
                            ...
                            let span = div.appendChild(this.createDOM('span', { 'class': 'ptreDotStats' }));
                            let dot = span.appendChild(this.createDOM('div', { 'data-acti': line[1], 'data-check': checkData[index][1] }));
                ...
                document.querySelectorAll(`div[id=${id}] li.msg`).forEach((msg => {
                    ...
                    if (id in this.json.expeditions && this.json.expeditions[id].result) {
                        ...
                        if (this.json.expeditions[id].result == "Unknown") {
                            msg.querySelector(".ogl-unknown-warning") || msg.querySelector(".msg_actions").appendChild(this.createDOM("div", { class: "ogl-unknown-warning" }, "Unknown expedition message... <a href='https://discord.gg/8Y4SWup'> Help me find them all</a>"))
                        } else if (this.json.expeditions[id].busy) {
                            msg.querySelector(".ogl-warning") || msg.querySelector(".msg_actions").appendChild(this.createDOM("a", {
    ...
    fetchAndConvertRC(messageId) {
        return fetch(`https://s${this.universe}-${this.gameLang}.ogame.gameforge.com/game/index.php?page=messages&messageId=${messageId}&tabid=21&ajax=1`).then((rep => rep.text())).then((str => {
    ...
    expeditionStats() {
        ...
        let renderDetails = (sums, onchange) => {
            ...
            let box = this.resourceBox([{
                ...
                crystal: 0,
                deuterium: sums.fuel
            }, { title: "B. Hole", metal: sums.adjust[0], crystal: sums.adjust[1], deuterium: sums.adjust[2], edit: onchange ? true : false }], true, (() => {
                globalDiv.empty();
        ...
        content.appendChild(this.tabs({
            D: () => {
                ...
                let contentHtml = `<strong>${getFormatedDate(this.dateStrToDate(minDate).getTime(), "[d].[m].[y]")}</strong> <span class="${total > 0 ? "undermark" : "overmark"}">${total > 0 ? " +" : " -"}${getNumberFormatShort(Math.abs(total), 2)}</strong></span>`;
    ...
    combatStats() {
        ...
        let renderDetails = (sums, onchange) => {
            ...
            let details = content.appendChild(this.createDOM("div", { class: "ogk-details" }));
            let losses = this.getFleetCost(sums.losses);
            let box = this.resourceBox([{ title: "Loot", metal: sums.loot[0], crystal: sums.loot[1], deuterium: sums.loot[2] }, {
                title: "Recycled",
                metal: sums.harvest[0],
                crystal: sums.harvest[1],
                deuterium: 0
                ...
                crystal: 0,
                deuterium: sums.fuel
            }, { title: "Adjust", metal: sums.adjust[0], crystal: sums.adjust[1], deuterium: sums.adjust[2], edit: onchange ? true : false }], false, (() => {
                globalDiv.empty();
    ...
    expeditionGraph(sums) {
        ...
        let config = {
            ...
            data: {
                datasets: [{
                    data: [sums["Metal"] || 0, sums["Crystal"] || 0, sums["Deuterium"] || 0, sums["AM"] || 0, sums["Object"] || 0, sums["Fleet"] || 0, sums["Aliens"] || 0, sums["Pirates"] || 0, sums["Late"] || 0, sums["Early"] || 0, sums["Bhole"] || 0, sums["Void"] || 0, sums["Merchant"] || 0],
                    label: "expeditions",
            ...
            options: {
                ...
                animation: { animateScale: true, animateRotate: true },
                plugins: {
                    labels: [{ fontSize: 15, fontStyle: "bold", textMargin: 10, render: "label", fontColor: "#ccc", position: "outside" }, {
                        fontSize: 15,
                        fontStyle: "bold",
                        fontColor: "#0d1117",
                        render: "percentage"
    ...
    playerSearch(show, name) {
        ...
        let renderPlayerInfo = player => {
            ...
            let name = `<span class="ogl-${player.status}">${player.name}</span> ${status}\n                  <a target="_self"\n                    href="https://s${this.universe}-${this.gameLang}.ogame.gameforge.com/game/index.php?page=highscore&searchRelId=${player.id}"\n                    class="ogl-ranking">#${player.points.position || "b"}\n                  </a>`;
        ...
        let updatePlayerList = (players, forced) => {
            players.forEach((async (player, index) => {
                ...
                playerNode.appendChild(this.createDOM("a", {
    ...
    minesStats() {
        ...
        this.planetList.forEach((planet => {
            ...
            let link = `?page=ingame&component=supplies&cp=${planet.id}`;
            header.appendChild(this.createDOM("th", {}, `<p>${planet.name}</p> <a href="${link}" class="ogl-fleet-coords">${planet.coordinates}</a> <span class="ogl-planet-fields">${planet.fieldUsed} / ${planet.fieldMax}</span><br><span class="ogl-planet-temperature">${planet.temperature}ºC</span>`));
            let td = metalRow.appendChild(this.createDOM("td"));
            td.appendChild(this.createDOM("div", { class: "ogl-metal" }, this.json.myMines[coords].metal));
            td.appendChild(this.createDOM("div", { class: "ogl-metal" }, this.formatToUnits(this.json.myMines[coords].metalProd || this.production(1, this.json.myMines[coords].metal, true))));
            td.appendChild(this.createDOM("div", { class: "ogl-metal" }, this.formatToUnits((this.json.myMines[coords].metalProd || this.production(1, this.json.myMines[coords].metal, true)) * 24)));
            td.appendChild(this.createDOM("div", { class: "ogl-metal" }, this.formatToUnits((this.json.myMines[coords].metalProd || this.production(1, this.json.myMines[coords].metal, true)) * 24 * 7)));
            if (current) td.classList.add("ogl-current");
            td = crystalRow.appendChild(this.createDOM("td"));
            td.appendChild(this.createDOM("div", { class: "ogl-crystal" }, this.json.myMines[coords].crystal));
            td.appendChild(this.createDOM("div", { class: "ogl-crystal" }, this.formatToUnits(this.json.myMines[coords].crystalProd || this.production(2, this.json.myMines[coords].crystal, true))));
            td.appendChild(this.createDOM("div", { class: "ogl-crystal" }, this.formatToUnits((this.json.myMines[coords].crystalProd || this.production(2, this.json.myMines[coords].crystal, true)) * 24)));
            td.appendChild(this.createDOM("div", { class: "ogl-crystal" }, this.formatToUnits((this.json.myMines[coords].crystalProd || this.production(2, this.json.myMines[coords].crystal, true)) * 24 * 7)));
            if (current) td.classList.add("ogl-current");
            td = deutRow.appendChild(this.createDOM("td"));
            td.appendChild(this.createDOM("div", { class: "ogl-deut" }, this.json.myMines[coords].deuterium));
            td.appendChild(this.createDOM("div", { class: "ogl-deut" }, this.formatToUnits(this.json.myMines[coords].deuteriumProd || this.production(3, this.json.myMines[coords].deuterium, true))));
            td.appendChild(this.createDOM("div", { class: "ogl-deut" }, this.formatToUnits((this.json.myMines[coords].deuteriumProd || this.production(3, this.json.myMines[coords].deuterium, true)) * 24)));
            td.appendChild(this.createDOM("div", { class: "ogl-deut" }, this.formatToUnits((this.json.myMines[coords].deuteriumProd || this.production(3, this.json.myMines[coords].deuterium, true)) * 24 * 7)));
            if (current) td.classList.add("ogl-current");
            td = nrjRow.appendChild(this.createDOM("td"));
            let diff = this.json.myMines[coords].energy;
            td.appendChild(this.createDOM("div", {
    ...
    buildDispatcherUI() {
        ...
        let planetList = coords.appendChild(this.createDOM("div", { class: "ogl-homes" }, `<svg height="12px" viewBox="0 0 512 512" width="12px"><path fill="white" d="m498.195312 222.695312c-.011718-.011718-.023437-.023437-.035156-.035156l-208.855468-208.847656c-8.902344-8.90625-20.738282-13.8125-33.328126-13.8125-12.589843 0-24.425781 4.902344-33.332031 13.808594l-208.746093 208.742187c-.070313.070313-.140626.144531-.210938.214844-18.28125 18.386719-18.25 48.21875.089844 66.558594 8.378906 8.382812 19.445312 13.238281 31.277344 13.746093.480468.046876.964843.070313 1.453124.070313h8.324219v153.699219c0 30.414062 24.746094 55.160156 55.167969 55.160156h81.710938c8.28125 0 15-6.714844 15-15v-120.5c0-13.878906 11.289062-25.167969 25.167968-25.167969h48.195313c13.878906 0 25.167969 11.289063 25.167969 25.167969v120.5c0 8.285156 6.714843 15 15 15h81.710937c30.421875 0 55.167969-24.746094 55.167969-55.160156v-153.699219h7.71875c12.585937 0 24.421875-4.902344 33.332031-13.808594 18.359375-18.371093 18.367187-48.253906.023437-66.636719zm0 0"/></svg>`));
        if (unions.length != 0) {
        ...
        bar.html(`<div class="fleft bar_container" data-current-amount="0" data-capacity="0">\n        <div class="filllevel_bar"></div>\n        </div>\n        <div>\n        <span class="undermark">0</span>\n        / <span>0</span>\n        </div>`);
        ...
        if (this.page == "fleetdispatch" && document.querySelector("#civilships") && fleetDispatcher.shipsOnPlanet.length != 0) {
            ...
            let needCargo = fret => {
                ...
                let amount = this.calcNeededShips({
                    fret: fret,
                    resources: Math.min(metal, metalAvailable) + Math.min(crystal, crystalAvailable) + Math.min(deut, deutAvailable)
                });
                return amount
            };
            let dispatch = document.querySelector("#shipsChosen").appendChild(this.createDOM("div", { class: "ogl-dispatch" }));
            ...
            let update = submit => {
                ...
                interval = setInterval((() => {
                    ...
                    returnDiv.html(getFormatedDate(new Date(serverTime).getTime() + 2 * fleetDispatcher.getDuration() * 1e3 + (fleetDispatcher.mission == 15 ? 3600 : 0) * 1e3, "<strong> [G]:[i]:[s] </strong> - [d].[m]"))
                }), 100);
                onResChange(2);
                onResChange(1);
                onResChange(0);
            ...
            document.querySelector("input[id=metal]").addEventListener("keyup", (e => {
                ...
                fleetDispatcher.cargoMetal = Math.min(Math.min(val, capacity + fleetDispatcher.cargoMetal), Math.max(0, metalAvailable));
                metalFiller.value = fleetDispatcher.cargoMetal;
                fleetDispatcher.refresh();
                refreshRes()
            }));
            document.querySelector("input[id=crystal]").addEventListener("keyup", (e => {
                ...
                let capacity = fleetDispatcher.getFreeCargoSpace();
                fleetDispatcher.cargoCrystal = Math.min(Math.min(val, capacity + fleetDispatcher.cargoCrystal), Math.max(0, crystalAvailable));
                crystalFiller.value = fleetDispatcher.cargoCrystal;
                fleetDispatcher.refresh();
                refreshRes()
            }));
            document.querySelector("input[id=deuterium]").addEventListener("keyup", (e => {
                ...
                let capacity = fleetDispatcher.getFreeCargoSpace();
                fleetDispatcher.cargoDeuterium = Math.min(Math.min(val, capacity + fleetDispatcher.cargoDeuterium), Math.max(0, deutAvailable));
                deutFiller.value = fleetDispatcher.cargoDeuterium;
                fleetDispatcher.refresh();
                refreshRes()
            }));
            ...
            $("#selectMostMetal").on("click", (() => {
                ...
                fleetDispatcher.cargoMetal = Math.min(fleetDispatcher.cargoMetal + capacity, Math.max(0, metalAvailable - (kept[0] || 0)));
                metalFiller.value = fleetDispatcher.cargoMetal;
                fleetDispatcher.refresh();
                refreshRes()
            }));
            $("#selectMostCrystal").on("click", (() => {
                let capacity = fleetDispatcher.getFreeCargoSpace();
                fleetDispatcher.cargoCrystal = Math.min(fleetDispatcher.cargoCrystal + capacity, Math.max(0, crystalAvailable - (kept[1] || 0)));
                crystalFiller.value = fleetDispatcher.cargoCrystal;
                fleetDispatcher.refresh();
                refreshRes()
            }));
            $("#selectMostDeuterium").on("click", (() => {
                let capacity = fleetDispatcher.getFreeCargoSpace();
                fleetDispatcher.cargoDeuterium = Math.min(fleetDispatcher.cargoDeuterium + capacity, Math.max(0, deutAvailable - fleetDispatcher.getConsumption() - (kept[2] || 0)));
                deutFiller.value = fleetDispatcher.cargoDeuterium;
                fleetDispatcher.refresh();
                refreshRes()
            }));
    ...
    openPlanetList(callcback) {
        ...
        let buildButton = (planet, id, galaxy, system, position, type) => {
            ...
            if (type == 1) div.classList.add("ogl-quickPlanet"); else div.classList.add("ogl-quickMoon");
            ...
            dataHelper.getPlayer(playerId).then((player => {
                ...
                [ptExpe, gtExpe].forEach((btn => {
                    btn.addEventListener("click", (() => {
                        ...
                        inputs[2].value = 16;
                        shipsOnPlanet.forEach((ship => {
                            if (ship.id == type) this.selectShips(ship.id, count); else if (ship.id == bigship) this.selectShips(ship.id, 1); else if (ship.id == 210) this.selectShips(ship.id, 1); else if (ship.id == 219) this.selectShips(ship.id, 1)
                        }));
    ...
    activitytimers() {
        ...
        if (this.current.isMoon) moonActivity = now; else planetActivity = now;
        ...
        this.planetList.forEach((planet => {
            ...
            if (planet.querySelector(".moonlink")) {
                let mTimer = planet.querySelector(".moonlink").appendChild(this.createDOM("div", {
    ...
    async getEmpireInfo() {
        ...
        return fetch(`https://s${this.universe}-${this.gameLang}.ogame.gameforge.com/game/index.php?page=standalone&component=empire`, { signal: abortController.signal }).then((rep => rep.text())).then((str => {
            ...
            if (hasMoon) {
                return fetch(`https://s${this.universe}-${this.gameLang}.ogame.gameforge.com/game/index.php?page=standalone&component=empire&planetType=1`, { signal: abortController.signal }).then((rep => rep.text())).then((str => {
        ...
        let renderTagetList = () => {
            ...
            ["red", "orange", "yellow", "green", "blue", "violet", "gray", "brown"].forEach((color => {
                ...
                toggle.addEventListener("click", (() => {
                    ...
                    this.saveData();
                    if (this.json.options.hiddenTargets[color]) toggle.classList.remove("ogl-active"); else toggle.classList.add("ogl-active");
                    content.querySelectorAll(`[data-marked="${color}"]`).forEach((planet => {
                        if (this.json.options.hiddenTargets[color]) planet.classList.add("ogl-colorHidden"); else planet.classList.remove("ogl-colorHidden")
                    }));
            ...
            let checkEmpty = (galaxy, system) => {
                for (let g = 1; g <= 10; g++) {
                    if (galaxy) {
                        ...
                        if (children) header.querySelector(`.ogl-tab[data-galaxy="${g}"]`).classList.remove("ogl-isEmpty"); else header.querySelector(`.ogl-tab[data-galaxy="${g}"]`).classList.add("ogl-isEmpty")
                    } else {
                ...
                for (let s = 0; s < step * 10; s += step) {
                    if (system) {
                        ...
                        if (children) header.querySelector(`.ogl-tab[data-system="${s}"]`).classList.remove("ogl-isEmpty"); else header.querySelector(`.ogl-tab[data-system="${s}"]`).classList.add("ogl-isEmpty")
                    } else {
    ...
    renderPlanet(coords, main, scanned, moon, deleted) {
        ...
        a.addEventListener("click", (() => {
            ...
            link = "https://" + window.location.host + window.location.pathname + link;
            if (event.ctrlKey) window.open(link, "_blank"); else {
                if (this.page == "galaxy") {
    ...
    sideStalk(playerid) {
        ...
        if (last) {
            ...
            if (!this.json.options.sideStalkVisible) {
                ...
                ptreBtn = sideStalk.appendChild(this.createDOM('a', { 'class': 'ogl-text-btn ogl-ptre-acti tooltip', 'title': 'Display PTRE data' }, 'PTRE'));
            ...
            dataHelper.getPlayer(playerid).then((player => {
                ...
                watchlistBtn && watchlistBtn.addEventListener("click", (() => {
                    sideStalk.empty();
                    sideStalk.appendChild(this.createDOM("div", { class: "title" }, "Historic " + this.json.sideStalk.length + "/10"));
                    sideStalk.appendChild(this.createDOM("hr"));
    ...
    sortTable(arr) {
        ...
        arr.forEach((async (report, index) => {
            ...
            if (report.activity <= 15) date.classList.add("ogl-danger"); else if (report.activity < 60) date.classList.add("ogl-care"); else date.classList.add("ogl-good");
            ...
            let colors = colorsContainer.appendChild(this.createDOM("div", { class: "ogl-colors", "data-coords": report.coords, "data-context": "spytable" }));
            let moon = false;
            ...
            simulateBtn.addEventListener("click", (() => {
                ...
                let base64 = btoa(JSON.stringify(json));
                window.open(`https://trashsim.universeview.be/en?SR_KEY=${report.apiKey}#prefill=${base64}`, "_blank")
            }));

            ptreBtn.addEventListener("click", (() => {
                this.getJSON(`https://ptre.chez.gg/scripts/oglight_import.php?team_key=${this.json.options.ptreTK}&sr_id=${report.apiKey}`, result => {
    ...
    deleteMSg(msgId) {
        ...
        return fetch(`https://s${this.universe}-${this.gameLang}.ogame.gameforge.com/game/index.php?page=messages`, { method: 'POST', body: requestData })
            .then((response) => response.json())
            .then((responseData) => {
    ...
    formatToUnits(value) {
        ...
        if (value == 0) precision = 0; else if (value < 1e3) precision = 0; else if (value < 1e6) precision = 1; else if (value < 1e10) precision = 2; else precision = 3;
    ...
    minesProduction(id, lvl, position, temp) {
        ...
        if (id == 12) {
            prod = 30 * lvl * Math.pow(1.05 + tech113 * .01, lvl)
        }
    ...
    production(id, lvl, total, coords) {
        ...
        let crawlerBonus = Math.min(mines.crawlers, maxCrawlerCount) * (this.playerClass == PLAYER_CLASS_MINER ? .003 : .002);
        let geologistFactor = this.geologist ? .1 : 0;
        if (this.allOfficers) {
            geologistFactor = .12
        }
        let baseProd = { 1: 30, 2: 20, 3: 10, 4: 20 };
        let plasmaFactor = { 1: .01, 2: .0066, 3: .0033 };
        let initProd = 0;
        if (id == 1) {
            initProd += 30
        } else if (id == 2) {
        ...
        let miner = (this.playerClass == PLAYER_CLASS_MINER ? 1 : 0) * .25 * prod;
    ...
    research(id, lvl, labs, technocrat, explorer) {
        ...
        if (technocrat) time -= time * .25;
        if (explorer) time -= time * .25;
    ...
    getRobotsNanites(id, lvl, time) {
        for (let i = 0; i < 15; i++) {
            for (let j = 0; j < 25; j++) {
                ...
                if (time == formatTimeWrapper(newTime * 60 * 60, 2, true, " ", false, "") || time == formatTimeWrapper(newTime * 60 * 60 - 1, 2, true, " ", false, "") || time == formatTimeWrapper(newTime * 60 * 60 + 1, 2, true, " ", false, "")) {
    ...
    building(id, lvl, robotic, nanite) {
        ...
        if (id == 15 || id == 36 || id == 43 || id == 42 || id == 41) {
            time = (cost[0] + cost[1]) / 2500 * (1 / (1 + robotic)) * Math.pow(.5, nanite)
        }
    ...
    trashsimTooltip(container, fleetinfo) {
        ...
        btn.addEventListener("click", (() => {
            ...
            let base64 = btoa(JSON.stringify(json));
            window.open(`https://trashsim.universeview.be/en?#prefill=${base64}`, "_blank")
        }))
    }
    ...
    utilities() {
        ...
        if (this.page == "movement") {
            ...
            if (lastFleetBtn) {
                ...
                let backlast = document.querySelector(".fleetStatus").appendChild(this.createDOM("span", { class: "reload ogl-backLast" }, `\n          <a class="dark_highlight_tablet"">\n            <span class="icon icon_link"></span>\n            <span>Back latest</span>\n          </a>\n            `));
    ...
    settings() {
        ...
        di.html(`<a class="bmc-button" target="_blank" href="https://www.buymeacoffee.com/DarkInfinity"><img src="https://cdn.buymeacoffee.com/buttons/bmc-new-btn-logo.svg" alt="Buy me a coffee"><span>Buy me a coffee</span></a>`);
        let di2 = dataDiv.appendChild(this.createDOM("div", { style: "margin: 0px 0px 20px 38px;" }));
        di2.html('<a style="color: #485f93;" target="_blank" href="https://www.buymeacoffee.com/ogameinfinity">Buy Cofee to creator</a>');
        dataDiv.appendChild(this.createDOM("hr"));
        let srvDatas = dataDiv.appendChild(this.createDOM("span", { style: "display: flex;justify-content: space-between; align-items: center;" }, "Ogame Infinity Server Settings : <br/>Top Score : " + this.formatToUnits(this.json.topScore) + "<br/>Eco Speed : " + this.json.speed + "<br/>Fleet Speed War: " + this.json.speedFleetWar + "<br/>Fleet Speed Peaceful: " + this.json.speedFleetPeaceful + "<br/>Fleet Speed Holding: " + this.json.speedFleetHolding));
        ...
        let purgeBox = dataDiv.appendChild(this.createDOM("div", {
            class: "ogi-checkbox",
        ...
        settingDiv.appendChild(this.createDOM("hr"));
        span = settingDiv.appendChild(this.createDOM("span", {}, '<a href="https://ptre.chez.gg/" target="_blank">PTRE</a> Teamkey'));
        let ptreInput = span.appendChild(this.createDOM("input", {
            type: "password",
