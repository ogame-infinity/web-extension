...
class OGInfinity {
  ...
  async statistics() {
    let showStats = async () => {
      ...
      if (this.hasLifeforms) {
        tabNames[this.getTranslatedText(139, 'text', false)] =
          this.discoveryStats.bind(this);
      }
  ...
  discoveryStats() {
    ...
    tabNames[LocalizationStrings.timeunits.short.day] = () => {
      ...
      for (let i = 0; i < 12; i++) {
        let dateStr = getFormatedDate(date.getTime(), '[d].[m].[y]');
        let sums =
          computeRangeSums(this.json.discoveriesSums, date, date) || sum;
        let profit = sums ? getTotal(sums) : 0;
        if (Math.abs(profit) > max) max = profit;
        profits.push({
          date: new Date(date.getTime()),
  ...
  checkDebris() {
    if (this.page == 'galaxy') {
        ...
        if (debris && !debris.classList.contains('ogl-debrisReady')) {
          ...
          debris.querySelectorAll('.debris-content').forEach(resources => {
            let value = fromFormatedNumber(
              resources.textContent.replace(/(\D*)/, ''),
            );
      ...
      if (expeBox && !expeBox.classList.contains('ogl-done')) {
        ...
        frag.appendChild(
          createDOM('img', {
            src: 'https://gf1.geo.gfsrv.net/cdnc5/fa3e396b8af2ae31e28ef3b44eca91.gif',
          }),
        );
        const res = [];
        expeBox
          .querySelectorAll('.ListLinks li.debris-content')
          .forEach(element => {
            res.push(element.textContent.replace(/(\D*)/, ''));
          });
        const debris = createDOM('div');
        debris.appendChild(
          createDOM('div', { class: 'ogl-metal' }, `${res[0]}`),
        );
        debris.appendChild(
          createDOM('div', { class: 'ogl-crystal' }, `${res[1]}`),
        );
        if (res[2]) {
          debris.appendChild(
            createDOM('div', { class: 'ogl-deuterium' }, `${res[2]}`),
          );
        }
        ...
        if (action) {
          link.appendChild(
            createDOM(
              'a',
              { href: '#', onclick: action.getAttribute('onclick') },
              action.textContent,
            ),
          );
        }
  ...
  utilities() {
    ...
    if (this.page == 'movement') {
      ...
      document.querySelectorAll('.fleetDetails').forEach(fleet => {
        ...
        fleet.appendChild(
          this.createDOM('a', {
            class: `ogl-mission-icon ogl-mission-${type}`,
          }),
        );
  ...
  getTranslatedText(id, type = 'text', html = true) {
    let language = ['de', 'en', 'es', 'fr', 'tr'].includes(this.gameLang)
      ? this.gameLang
      : 'en';
    let translation = {
      tech: {
  ...
  getBestRoi() {
    ...
    this.json.empire.forEach((planet, index) => {
      ...
      let AllyClassProd = MineProd.map(x =>
        Math.round(
          x * (that.json.allianceClass == ALLY_CLASS_MINER ? 0.05 : 0),
        ),
      );
  ...
  async initializeLFTypeName() {
    fetch('/game/index.php?page=ingame&component=lfoverview')
      .then(rep => rep.text())
      .then(str => {
        let htmlDocument = new window.DOMParser().parseFromString(
          str,
          'text/html',
        );
