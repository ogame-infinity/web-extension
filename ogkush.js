...
(function goodbyeTipped() {
  if (typeof Tipped !== 'undefined') {
    ...
    requestAnimationFrame(() => goodbyeTipped());
})();
...
class OGInfinity {
  constructor() {
    ...
    this.mode = this.rawURL.searchParams.get('oglMode') ||
        0;  // 0=default; 1=transpo; 2=lock; 3=autoHarvest; 4=raid;
    ...
    this.current = {};
    this.current.planet = (document.querySelector('.smallplanet .active') ||
                           document.querySelector('.smallplanet .planetlink'))
                              .parentNode;

    document.querySelectorAll('.planet-koords')
    ...
    this.current.hasMoon =
        this.current.planet.querySelector('.moonlink') ? true : false;
    this.current.isMoon = this.current.hasMoon &&
  ...
  init() {
    ...
    this.gameLang = document.querySelector('meta[name="ogame-language"]')
  ...
  start() {
    ...
    // dataHelper
    //   .getExpeditionType(
    //     `Der Hilferuf, dem die Expedition folgte, stellte sich als böse Falle
    //     einiger arglistiger Sternen-Freibeuter heraus. Ein Gefecht war
    //     unvermeidlich.`
    //   )
    //   .then((type) => {
    //     alert(type);
    ...
    if (this.json.welcome) {
      if (this.page == 'fleetdispatch') {
        ...
        window.location.href = `https://s${this.universe}-${
            this.gameLang}.ogame.gameforge.com/game/index.php?page=ingame&component=fleetdispatch`;
      }
    }
  ...
  timeZone() {
    ...
    hourDiff != 0 &&
        $('.ogk-ping')
            .prepend(this.createDOM(
                'span', {style: 'color: white'},
                `(${hourDiff > 0 ? '+' : ''}${hourDiff}h) `));
  }

  updateEmpireData() {
    if (isNaN(new Date(this.json.lastEmpireUpdate)) ||
  ...
  overviewDates() {
    ...
    document
        ...
        .forEach((timer) => {
          ...
          timer.parentNode.appendChild(this.createDOM(
              'div', {class: 'ogl-date'},
              getFormatedDate(
                  newDate.getTime(),
  ...
  minesLevel() {
    ...
    if (this.page == 'supplies' && !this.current.isMoon) {
      ...
      let crawlers = document.querySelector('.technology.resbuggy .amount')
                         .getAttribute('data-value');
  ...
  technoDetail() {
    if (this.page == 'research' || this.page == 'supplies' ||
        this.page == 'facilities' || this.page == 'shipyard' ||
        this.page == 'defenses') {
      let nanites = 0;
      let robotics = 0;
      let lock;
      ...
      if (this.page == 'research') {
        this.json.tech113 = Number(
            document.querySelector('.technology[data-technology="113"] .level')
                .getAttribute('data-value'));
        this.json.tech122 = Number(
            document.querySelector('.technology[data-technology="122"] .level')
                .getAttribute('data-value'));
      }
      ...
      let currentRes = [
        document.querySelector('#resources_metal')
            .innerText.split('.')
            .join(''),
        document.querySelector('#resources_crystal')
            .innerText.split('.')
            .join(''),
        document.querySelector('#resources_deuterium')
            .innerText.split('.')
            .join(''),
      ];
      ...
      let updateResearchDetails = (technoId, baselvl, tolvl) => {
        ...
        if (that.page == 'research') {
          techno = that.research(
              technoId, tolvl, labs, technocrat, that.playerClass == 3);
        } else if (that.page == 'supplies' || that.page == 'facilities') {
        ...
        if (that.page == 'supplies') {
          let consDiv =
              document.querySelector('.additional_energy_consumption span');
          let prodDiv = (document.querySelector('.narrow') &&
                         document.querySelector('.ogk-production')) ||
              document.querySelector('.narrow').appendChild(
                  that.createDOM('li', {class: 'ogk-production'}));
          ...
          if (consDiv) {
            ...
            let currentProd =
                that.minesProduction(technoId, baselvl - 1, pos, temp);
            ...
            consDiv.html(`<span>${
                (currentCons - baseCons).toLocaleString('de-DE')}<span class="${
                diff < 0 ? 'overmark' : 'undermark'}"> (${
                diff.toLocaleString('de-DE')})</span></span>`);

            prodDiv.html(`<strong>Production :</strong> <span class="value">${
                parseInt(baseProd).toLocaleString(
                    'de-DE')} <span class="bonus"> (+${
          ...
          if (energyDiv) {
            ...
            energyDiv.html(`<span class="value">${
                parseInt(baseProd).toLocaleString(
                    'de-DE')} <span class="bonus"> (+${
                parseInt(baseProd - currentProd)
                    .toLocaleString('de-DE')})</span></span>`);
        ...
        timeDiv.innerText =
            formatTimeWrapper(techno.time * 60 * 60, 2, true, ' ', false, '');
        let currentDate = new Date();
        let finishDate =
            new Date(currentDate.getTime() + techno.time * 60 * 60 * 1000);

        timeDiv.appendChild(this.createDOM(
            'div', {class: 'ogl-date'},
            getFormatedDate(
                finishDate.getTime(),
        ...
        if (baselvl != tolvl) {
          timeSumDiv.innerText =
              formatTimeWrapper(timeSum * 60 * 60, 2, true, ' ', false, '');
          finishDate =
              new Date(currentDate.getTime() + timeSum * 60 * 60 * 1000);
          timeSumDiv.appendChild(this.createDOM(
              'div', {class: 'ogl-date'},
              getFormatedDate(
                  finishDate.getTime(),
                  '<strong>[d].[m]</strong> - [G]:[i]:[s]')));
        ...
        if (techno.cost[0] != 0) {
          ...
          metal.setAttribute(
              'data-title', parseInt(techno.cost[0]).toLocaleString('de-DE'));
          if (baselvl != tolvl) {
            metal.appendChild(that.createDOM(
                'li', {class: 'ogk-sum'}, that.formatToUnits(resSum[0])));
          }
          missing[0] = Math.min(0, currentRes[0] - resSum[0]);
          metal.appendChild(that.createDOM(
              'li', {class: missing[0] != 0 ? 'overmark' : ''},
        ...
        if (techno.cost[1] != 0) {
          ...
          crystal.setAttribute(
              'data-title', parseInt(techno.cost[1]).toLocaleString('de-DE'));
          if (baselvl != tolvl) {
            crystal.appendChild(that.createDOM(
                'li', {class: 'ogk-sum'}, that.formatToUnits(resSum[1])));
          }
          missing[1] = Math.min(0, currentRes[1] - resSum[1]);
          crystal.appendChild(that.createDOM(
              'li', {class: missing[1] != 0 ? 'overmark' : ''},
        ...
        if (techno.cost[2] != 0) {
          ...
          deuterium.setAttribute(
              'data-title', parseInt(techno.cost[2]).toLocaleString('de-DE'));
          if (baselvl != tolvl) {
            deuterium.appendChild(that.createDOM(
                'li', {class: 'ogk-sum'}, that.formatToUnits(resSum[2])));
          }
          missing[2] = Math.min(0, currentRes[2] - resSum[2]);
          deuterium.appendChild(that.createDOM(
              'li', {class: missing[2] != 0 ? 'overmark' : ''},
        ...
        if (techno.cost[3] != 0) {
          ...
          if (energy) {
            ...
            energy.setAttribute(
                'data-title', parseInt(techno.cost[3]).toLocaleString('de-DE'));
            if (baselvl != tolvl) {
              energy.appendChild(that.createDOM(
                  'li', {class: 'ogk-sum'}, that.formatToUnits(resSum[3])));
            }
            // In that case, we need the all energy prod
            let tooltip =
                document.querySelector('#energy_box').getAttribute('title') ||
                document.querySelector('#energy_box')
      ...
      technologyDetails.show = function(technologyId) {
        ...
        $.ajax({
          ...
          let titleDiv =
              costDiv.appendChild(that.createDOM('div', {class: 'ogk-titles'}));
          ...
          if ([
                202, 203, 208, 209, 210, 204, 205, 206, 219,
                207, 215, 211, 212, 217, 213, 218, 214, 401,
                402, 403, 404, 405, 406, 407, 408, 502, 503,
            ...
            let infoDiv = document.querySelector('#technologydetails .sprite')
                              .appendChild(that.createDOM(
                                  'div', {class: 'ogk-tech-controls'}));
            ...
            let helpNode =
                document.querySelector('.txt_box .details').cloneNode(true);
            ...
            let updateShipDetails = (value) => {
              ...
              resDivs.forEach((div, index) => {
                ...
                div.appendChild(that.createDOM(
                    'div', {class: 'ogk-sum'}, that.formatToUnits(tot)));
                div.appendChild(that.createDOM(
                    'div', {class: min != 0 ? 'overmark' : ''},
              ...
              let currentDate = new Date();
              let finishDate =
                  new Date(currentDate.getTime() + baseTime * value * 1000);
              timeDiv.appendChild(that.createDOM(
                  'div', {class: 'ogl-date'},
                  getFormatedDate(
                      finishDate.getTime(),
                      '<strong>[d].[m]</strong> - [G]:[i]:[s]')));
              ...
              if (technologyId == 212) {
                ...
                energyDiv.html(
                    ...
                    `<span class="${diff < 0 ? 'overmark' : 'undermark'}"> (${
            ...
            let initTime =
                getTimeFromString(document.querySelector('.build_duration time')
                                      .getAttribute('datetime'));
            initTime = formatTimeWrapper(initTime, 2, true, ' ', false, '');

            if (that.page == 'research') {
              labs = that.getLabs(
                  technologyId, baseLvl, initTime, technocrat,
                  that.playerClass == 3);
            } else if (that.page == 'supplies' || that.page == 'facilities') {
    ...
    FleetDispatcher.prototype.submitFleet3 = function(force) {
      ...
      $.post(this.sendFleetUrl, params, function(response) {
        ...
        // request failed
          ...
          if (data.responseArray && data.responseArray.limitReached &&
              !data.responseArray.force) {
            errorBoxDecision(
                that.loca.LOCA_ALL_NETWORK_ATTENTION,
                that.locadyn.localBashWarning, that.loca.LOCA_ALL_YES,
                that.loca.LOCA_ALL_NO, function() {
  ...
  keepOnPlanetDialog(coords, btn) {
    ...
    if (coords) {
      container.appendChild(this.createDOM(
          ...
          this.current.coords +
              (this.current.isMoon ? ' (Moon)' : ' (Planet)')));
    ...
    let prod =
        box.appendChild(this.createDOM('div', {class: 'ogk-adjust-grid'}));

    prod.appendChild(this.createDOM(
        'span', {}, `<a class="ogl-option resourceIcon metal"></a>`));
    let metInput = prod.appendChild(this.createDOM('input', {
    ...
    prod.appendChild(this.createDOM(
        'span', {}, `<a class="ogl-option resourceIcon crystal"></a>`));
    let criInput = prod.appendChild(this.createDOM('input', {
    ...
    prod.appendChild(this.createDOM(
        'span', {}, `<a class="ogl-option resourceIcon deuterium"></a>`));

    let deutInput = prod.appendChild(this.createDOM('input', {
    ...
    let fleet =
        box.appendChild(this.createDOM('div', {class: 'ogk-bhole-grid'}));
    ...
    [202,
     ...
     214,
      ...
      fleet.appendChild(this.createDOM('a', {
        class: 'ogl-option ogl-fleet-ship ogl-fleet-' + id,
      }));
      let input = fleet.appendChild(this.createDOM('input', {
  ...
  initUnionCombat(union) {
    if (this.unionInterval) {
      ...
      this.delayDiv =
          document.querySelector('.briefing h2')
              .appendChild(this.createDOM('span', {class: 'ogk-delay'}));
      this.delayDiv2 =
          document.querySelector('#roundup ul')
              .appendChild(this.createDOM('div', {class: 'ogk-delay'}));
      this.delayDiv3 =
          document.querySelector('#continueToFleet2')
              .appendChild(this.createDOM('div', {class: 'ogk-delay'}));
      this.delayTimeDiv =
          document
              .querySelector('#fleetBriefingPart1_2 li:first-of-type .value')
              .appendChild(this.createDOM('div', {class: 'undermark'}));
      this.delayTimeDiv2 = document.querySelector('#fleet3 #arrivalTime')
                               .parentElement.appendChild(
                                   this.createDOM('div', {class: 'undermark'}));
      this.delayTimeDiv3 = document.querySelector('#fleet1 .ogl-info')
                               .appendChild(this.createDOM('div', {
                                 class: 'undermark',
  ...
  fleetDispatcher() {
    ...
    if (this.page == 'fleetdispatch' && document.querySelector('#civilships') &&
      ...
      this.onFleetSent(() => {
        ...
        let coords = document.querySelector('#galaxy').value + ':' +
            document.querySelector('#system').value + ':' + pos;

        let fuel = fleetDispatcher.getConsumption();
        let dateStr = getFormatedDate(new Date().getTime(), '[d].[m].[y]');

        let fullCoords = coords;
        if (fleetDispatcher.targetPlanet.type ==
      ...
      $('.allornonewrap .select-most').on('click', () => {
        fleetDispatcher.shipsOnPlanet.forEach((ship) => {
          let kept =
              this.json.options
                  .kept[this.current.coords + this.current.isMoon ? 'M' : 'P'] ||
              this.json.options.defaultKept;
          this.selectShips(
              ship.id, Math.max(0, ship.number - (kept[ship.id] || 0)));
        });
      });
      ...
      let svg = svgButtons.appendChild(
          this.createDOM('div', {class: 'ogi-speed-icon'}, `
        <svg x="0px" y="0px"
      ...
      svg.addEventListener('mouseover', () => {
        document.querySelectorAll('#shipsChosen .technology')
            .forEach((elem) => {
              elem.classList.add('ogi-transparent');
              let id = elem.getAttribute('data-technology');
              elem.appendChild(this.createDOM(
                  'span', {class: 'ogi-speed'},
                  fleetDispatcher.fleetHelper.shipsData[id]
      ...
      let plusSvg = svgButtons.appendChild(
          this.createDOM('div', {class: 'ogi-plus-icon'}, `
  ...
  updateServerSettings() {
    ...
    let settingsUrl = `https://s${this.universe}-${
        this.gameLang}.ogame.gameforge.com/api/serverData.xml`;
        ...
        .then((xml) => {
          ...
          this.json.trashsimSettings = {
            ...
            characterClassesEnabled:
                xml.querySelector('characterClassesEnabled').innerHTML,
            minerBonusFasterTradingShips:
                xml.querySelector('minerBonusFasterTradingShips').innerHTML,
            minerBonusIncreasedCargoCapacityForTradingShips:
                xml.querySelector(
                       'minerBonusIncreasedCargoCapacityForTradingShips')
                    .innerHTML,
            warriorBonusFasterCombatShips:
                xml.querySelector('warriorBonusFasterCombatShips').innerHTML,
            warriorBonusFasterRecyclers:
                xml.querySelector('warriorBonusFasterRecyclers').innerHTML,
            warriorBonusRecyclerFuelConsumption:
                xml.querySelector('warriorBonusRecyclerFuelConsumption')
                    .innerHTML,
            combatDebrisFieldLimit:
                xml.querySelector('combatDebrisFieldLimit').innerHTML,
          };
          this.saveData();
          return true;
        });
  ...
  topBarUtilities() {
    ...
    bar.appendChild(this.createDOM(
        'li', {}, `<a href="${boardlink}" target="_blank">Board</a>`));
    bar.appendChild(this.createDOM(
        'li', {},
        `<a href="https://proxyforgame.com/${
            this.gameLang}/ogame/calc/flight.php" target="_blank">Flight</a>`));
    bar.appendChild(this.createDOM(
        'li', {},
        `<a href="https://trashsim.universeview.be/en" target="_blank">Trash</a>`));
    bar.appendChild(this.createDOM(
        'li', {},
        `<a href="https://www.mmorpg-stat.eu/base.php?se=1&univers=_${
            this.universe}" target="_blank">Mmorpg</a>`));

    // Adding ping info
    let ping = window.performance.timing.domLoading -
        window.performance.timing.fetchStart;

    let colorClass = 'friendly';
    if (ping > 400 && ping < 800) colorClass = 'neutral';
    if (ping > 800) colorClass = 'hostile';
    $('#bar ul').prepend(this.createDOM(
        'span', {class: 'ogk-ping'},
        `<span class='${colorClass}'>${
            (ping / 1000).toFixed(1)}s</span> ping`));
  }
  ...
  eventBox() {
    let interval = setInterval(() => {
      if (document.querySelector('#eventboxLoading').style.display == 'none') {
        ...
        if (JSON.stringify(this.json.flying.ids) !=
            JSON.stringify(flying.ids)) {
          ...
          //   );
          let gone = [];
          this.json.flying.ids && this.json.flying.ids.forEach((mov) => {
            let found = false;
          ...
          let added = [];
          this.json.flying.ids && flying.ids.forEach((mov) => {
            let found = false;
    ...
    let addColors = () => {
      document.querySelectorAll('.eventFleet, .allianceAttack')
          .forEach((line) => {
    ...
    let changeSpy = () => {
      document.querySelectorAll('#eventContent .sendProbe a')
          .forEach((elem) => {
            let params = new URL(elem.href).searchParams;
            elem.href = '#';
            elem.setAttribute(
                'onClick',
                `sendShipsWithPopup(6,${params.get('galaxy')},${
                    params.get('system')},${params.get('position')},${
                    params.get(
                        'planetType')},${this.json.spyProbes}); return false;`);
          });
    };
      ...
      document.querySelectorAll('.eventFleet').forEach((line) => {
        const id = line.getAttribute('id'),
              notificationIcon = this.createDOM(
                  'td', {
                    class: 'ogl-notification ' +
                        (this.notifiy.exists(id) ? 'active' : ''),
                  },
                  null);
        notificationIcon.addEventListener('click', (item) => {
          const arrival = line.getAttribute('data-arrival-time'),
                missionFleet = line.querySelector('.missionFleet img')
                                   .getAttribute('title'),
          ...
          if (item.target.classList.contains('active')) {
            ...
            this.notifiy.add(new OGIMessage(
                id, 'Fleet', missionFleet + ' - ' + coordsOrigin,
                arrival * 1000));
          }
    ...
    let changeTimeZone = () => {
      document.querySelectorAll('.eventFleet').forEach((line) => {
        ...
        if (line.querySelector('.arrivalTime')) {
          line.querySelector('.arrivalTime').innerText =
              getFormatedDate(arrival, '[H]:[i]:[s]');
        }
      });
    };
    ...
    let addRefreshButton = () => {
      ...
      refreshBtn.addEventListener('click', () => {
        $.get(ajaxEventboxURI.replace('&asJson=1', ''), (data) => {
  ...
  addGalaxyTooltips() {
    ...
    document.querySelectorAll('.tooltipRel').forEach((sender) => {
      ...
      if (rel.indexOf('player') == 0 && rel != 'player99999') {
        ...
        sender.appendChild(this.createDOM(
            'a',
  ...
  fixRedirectGalaxy() {
    history.pushState(
        {}, null,
        `/game/index.php?page=ingame&component=galaxy&galaxy=${galaxy}&system=${
            system}`);
  }
  ...
  addGalaxyMarkers() {
    document.querySelectorAll('#galaxytable tbody .allytag')
    ...
    document.querySelectorAll('#galaxytable tbody tr')
        .forEach((element, index) => {
          let coords = galaxy + ':' + system + ':' + Number(index + 1);
          ...
          if (this.json.markers[coords]) {
            if (this.json.markers[coords].id != id) {
              ...
              element.setAttribute(
                  'data-marked', this.json.markers[coords].color);
              this.json.markers[coords].moon =
                  element.querySelector('.moon.tooltipRel') ? true : false;
            }
            this.saveData();
          }
        });
  ...
  scan() {
    ...
    document.dispatchEvent(
        new CustomEvent('ogi-galaxy', {detail: changes}), true, true);

    document.querySelectorAll(`div:not(.ogl-target-list) .ogl-stalkPlanets`)
        .forEach((reset) => {
                  ...
                  setInterval(
                      () => {
                        try {
                          ...
                          $('#jumpgate .send_all')
                              .after(this.createDOM(
                                  'span', {class: 'select-most'}));
                          $('.select-most')
                              .on('click',
                                  () => {
                                    let kept =
      ...
      if (eventList) {
        ...
        document.querySelector('.event_list')
            .appendChild(this.createDOM(
                ...
                'Flying: ' + (this.commander ? '' : '≈') + '<span class="' +
                    color + '">' + ((fleetCount / total) * 100).toFixed(0) +
                    '%</span>'));
      }
    }, 200);
  }
  ...
  welcome() {
    ...
    let head =
        container.appendChild(this.createDOM('div', {class: 'ogk-header'}));
    ...
    let shortcutsDiv = container.appendChild(this.createDOM(
        'p', {
    ...
    let ctrl = shortcutsDiv.appendChild(this.createDOM(
        'div', {
          style:
    ...
    let keyHelp =
        container.appendChild(this.createDOM('div', {class: 'ogk-keyhelp'}));
    let ctrlKey = this.createDOM(
        'div', {
          style:
              'display: flex; width: 80px;margin-left: 10px;margin-top: -2px;',
        },
        `
  ...
  chat() {
    ...
    let toggleChat = () => {
      ...
      this.saveData();
      document.querySelector('#chatBar').style.display =
          this.json.tchat ? 'block' : 'none';
    };
    ...
    let btn = document.querySelector('body').appendChild(
        this.createDOM('div', {class: 'ogk-chat icon icon_chat'}));
    if (this.json.tchat) {
      document.querySelector('#chatBar').style.display =
          this.json.tchat ? 'block' : 'none';
    }
  ...
  uvlinks() {
    if (this.page == 'messages') {
      document.querySelectorAll('.msg_actions').forEach((elem) => {
        ...
        if (keyNode) {
          let key = keyNode.getAttribute('title') ||
              keyNode.getAttribute('data-title');
          key = key.split('\'')[1];
          if (key.startsWith('sr')) {
            let link = elem.appendChild(this.createDOM('div', {
            ...
            let link = elem.appendChild(this.createDOM('a', {
  ...
  warningCommander() {
    let content = this.createDOM(
        'div', {class: 'ogl-warning-dialog overmark', style: 'padding: 25px'},
        `<div class="premium">
      <div class="officers100  commander">
            <a href="https://s${this.universe}-${
            this.gameLang}.ogame.gameforge.com/game/index.php?page=premium&openDetail=2" class="detail_button">
              <span class="ecke">
                  <span class="level">
  ...
  sideOptions() {
    ...
    let container = document.querySelector('#myPlanets') ||
    ...
    let targetList = harvestOptions.appendChild(this.createDOM('a', {
      class: 'ogl-option ogl-targetIcon tooltip',
      title: this.getTranslatedText(1),
    }));

    let search = harvestOptions.appendChild(this.createDOM('div', {
      class: 'ogl-option ogl-search-icon tooltip',
      title: 'Player search',
    }));

    let statsBtn = harvestOptions.appendChild(this.createDOM('div', {
      class: 'ogl-option ogl-statistics-icon tooltip',
      title: 'Statistics',
    }));

    // let harvestBtn = harvestOptions.appendChild(
    //   this.createDOM("div", {
    //     class: "ogl-option ogl-settings-icon tooltip",
    //     title: "Auto harvest",
    //   })
    // );
    ...
    let empireBtn;
    this.commander &&
        (empireBtn = harvestOptions.appendChild(this.createDOM('div', {
          class: 'ogl-option ogl-empire-icon tooltip',
          title: 'Overview',
        })));
    let overViewBtn = harvestOptions.appendChild(this.createDOM('div', {
    ...
    //   let list = this.openPlanetList((planet) => {
    //     this.json.autoHarvest = [
    ...
    //     let link =
    //     `?page=ingame&component=fleetdispatch&galaxy=${planet.galaxy}&system=${planet.system}&position=${planet.position}&type=${planet.type}&mission=${this.json.options.harvestMission}&oglMode=3`;

    //     window.location.href =
    //       "https://" + window.location.host + window.location.pathname +
    //       link;
    //   }, true);
    //   this.popup(false, list);
    // });
    ...
    empireBtn && empireBtn.addEventListener('click', () => {
      if (!this.commander) {
    ...
    overViewBtn.addEventListener('click', () => {
      ...
      if (this.json.options.empire) {
        document.querySelector('.ogl-overview-icon')
            .classList.remove('ogl-active');
        document.querySelectorAll('.ogl-summary, .ogl-res')
  ...
  profitGraph(profits, max, callback) {
    ...
    let spans = [];
    profits.slice().reverse().forEach((elem, index) => {
      let span = div.appendChild(this.createDOM('span', {
        style: `height: ${
            elem.profit == 0 ?
                5 :
                Math.max(10, (Math.abs(elem.profit) / max) * 60)}px`,
      ...
      span.addEventListener('click', () => {
        ...
        let contentHtml = `<strong>${
            getFormatedDate(
                elem.date.getTime(), '[d].[m].[y]')}</strong> <span class="${
            elem.profit >= 0 ? 'undermark' :
                               'overmark'}">${elem.profit >= 0 ? ' +' : ' -'}${
            getNumberFormatShort(Math.abs(elem.profit), 2)}</strong></span>`;

        if (elem.start) {
          contentHtml += `<strong>${
              getFormatedDate(elem.start.getTime(), '[d].[m].[y]')}</strong>`;
        }
  ...
  repartitionGraph(eco, tech, fleet, def) {
    ...
    let chartNode = div.appendChild(this.createDOM('canvas', {
      id: 'piechart',
      width: '200px',
      height: '150px',
    }));
  ...
  generalStats(player) {
    ...
    let globalInfo =
        content.appendChild(this.createDOM('div', {class: 'ogk-global'}));
    ...
    playerDiv.appendChild(this.createDOM(
        'p', {class: honorScore > 0 ? 'undermark' : 'overmark'},
        '(' + dotted(honorScore) + ')'));
    let stats =
        playerDiv.appendChild(this.createDOM('a', {class: 'ogl-mmorpgstats'}));
    stats.addEventListener('click', () => {
      window.open(
          this.generateMMORPGLink(player.id), '_blank',
          `location=yes,scrollbars=yes,status=yes,width=${
              screen.availWidth},height=${screen.availHeight}`);
    });
    ...
    globalInfo.appendChild(this.repartitionGraph(
        player.economy.score, player.research.score, player.military.score,
        player.def));

    globalInfo.appendChild(this.createDOM('h2', {}, player.points.position));
    globalInfo.appendChild(this.createDOM(
        'h3', {}, dotted(player.points.score) + '<small> pts</small>'));

    let detailRank = globalInfo.appendChild(
        this.createDOM('div', {class: 'ogl-detailRank'}));

    detailRank.html(`
          <div><div class="ogl-ecoIcon"></div>${
        dotted(player.economy
                   .score)} <small>pts</small><span class="ogl-ranking">#${
        player.economy.position} </span></div>
          <div><div class="ogl-techIcon"></div>${
        dotted(player.research
                   .score)} <small>pts</small><span class="ogl-ranking">#${
        player.research.position} </span></div>
          <div><div class="ogl-fleetIcon"></div>${
        dotted(player.military
                   .score)} <small>pts</small><span class="ogl-ranking">#${
        player.military.position} </span></div>
          <div><div class="ogl-fleetIcon grey"></div>${
        dotted(player.def)} <small>pts</small></div>
          `);

    let details =
        content.appendChild(this.createDOM('div', {class: 'ogk-details'}));

    let ecoDetail =
        details.appendChild(this.createDOM('div', {class: 'ogk-box'}));

    let techDetail = details.appendChild(
        this.createDOM('div', {class: 'ogk-box ogk-technos'}));

    let div =
        techDetail.appendChild(this.createDOM('div', {class: 'ogk-tech'}));
    div.appendChild(this.createDOM('span', {}, 'Hyperspace'));
    div.appendChild(this.createDOM('a', {
      class: 'ogl-option ogl-fleet-ship ogl-tech-' + 114,
    }));

    if (this.commander) {
      div.appendChild(this.createDOM(
          'span', {}, `<strong>${this.json.empire[0][114]}</strong>`));
    } else {
      div.appendChild(this.createDOM(
          'span', {}, `<strong>${this.json.apiTechData[114]}</strong>`));
    }
    ...
    [115, 117, 118, 109, 110, 111].forEach((id) => {
      ...
      fleetTech.appendChild(this.createDOM('a', {
        class: 'ogl-option ogl-fleet-ship ogl-tech-' + id,
      }));
      if (this.commander) {
        fleetTech.appendChild(this.createDOM(
            'span', {}, `<strong>${this.json.empire[0][id]}</strong>`));
      } else {
        fleetTech.appendChild(this.createDOM(
            'span', {}, `<strong>${this.json.apiTechData[id]}</strong>`));
      }
    });

    let mprod = 0, mlvl = 0, cprod = 0, clvl = 0, dprod = 0, dlvl = 0, sum = 0;
    ...
    if (this.commander) {
      ...
      for (let i in this.json.myMines) {
        ...
        cprod += this.json.myMines[i].crystalProd ||
            this.production(2, crystal, true);
        dprod += this.json.myMines[i].deuteriumProd ||
    ...
    prod.appendChild(this.createDOM(
        'span', {class: 'ogl-crystal'}, `${dotted(cprod * 24)}`));
    prod.appendChild(
        this.createDOM('span', {class: 'ogl-deut'}, `${dotted(dprod * 24)}`));

    prod.appendChild(this.createDOM('p', {}, `Week`));
    prod.appendChild(this.createDOM(
        'span', {class: 'ogl-metal'}, `${dotted(mprod * 24 * 7)}`));
    prod.appendChild(this.createDOM(
        'span', {class: 'ogl-crystal'}, `${dotted(cprod * 24 * 7)}`));
    prod.appendChild(this.createDOM(
        'span', {class: 'ogl-deut'}, `${dotted(dprod * 24 * 7)}`));
    ...
    if (this.commander) {
      let fleet =
          fleetDetail.appendChild(this.createDOM('div', {class: 'ogk-fleet'}));
      ...
      [202,
       ...
       214,
        ...
        shipDiv.appendChild(this.createDOM('a', {
          class: 'ogl-option ogl-fleet-ship ogl-fleet-' + id,
        }));
        if (id == 209) {
          cyclos = sum;
      ...
      fleetInfo.appendChild(this.createDOM(
          'span', {},
      ...
      fleetInfo.appendChild(this.createDOM(
          'span', {}, `Recycling: <strong>${dotted(rcpower)}</strong>`));
    } else {
      fleetDetail.appendChild(this.createDOM(
          'p', {class: 'ogk-commander'},
  ...
  tabs(titles, small) {
    ...
    for (let title in titles) {
      ...
      tabs.push(header.appendChild(
          this.createDOM('span', {class: 'ogl-tab'}, title)));
    }
  ...
  expeditionMessages() {
    ...
    if (this.page == 'messages') {
      ...
      if (document.querySelector('li[id=subtabs-nfFleet22].ui-state-active')) {
        let id =
            document.querySelector('li[id=subtabs-nfFleet22].ui-state-active')
        ...
        document.querySelectorAll(`div[id=${id}] li.msg`).forEach((msg) => {
          ...
          dataHelper.getExpeditionType(textContent).then((type) => {
            ...
            fleetMatches && fleetMatches.forEach((result) => {
              let split = result.split(': ');
              type = 'Fleet';
              let id = this.json.shipNames[split[0]];
              let count = Number(split[1]);
              sums.fleet[id] ? (sums.fleet[id] += count) :
                               (sums.fleet[id] = count);
            });
            // }
            ...
            if (this.json.expeditions[id].result == 'Unknown') {
              ...
              msg.querySelector('.msg_actions').appendChild(this.createDOM('a', {
                class:
      ...
      if (document.querySelector('li[id=subtabs-nfFleet21].ui-state-active')) {
        let id =
            document.querySelector('li[id=subtabs-nfFleet21].ui-state-active')
                .getAttribute('aria-controls');
        document.querySelectorAll(`div[id=${id}] li.msg`).forEach((msg) => {
          let id = msg.getAttribute('data-msg-id');
      ...
      if (document.querySelector('li[id=subtabs-nfFleet24].ui-state-active')) {
        let id =
            document.querySelector('li[id=subtabs-nfFleet24].ui-state-active')
                .getAttribute('aria-controls');
        document.querySelectorAll(`div[id=${id}] li.msg`).forEach((msg) => {
          let id = msg.getAttribute('data-msg-id');
  ...
  overview() {
    ...
    let minesBtn =
        header.appendChild(this.createDOM('span', {class: 'ogl-tab'}, 'Mines'));
  ...
  fetchAndConvertRC(messageId) {
    return fetch(
               `https://s${this.universe}-${
                   this.gameLang}.ogame.gameforge.com/game/index.php?page=messages&messageId=${
                   messageId}&tabid=21&ajax=1`)
        .then((rep) => rep.text())
        .then((str) => {
          ...
          if (lastRound) {
            combatIds.forEach((id) => {
              for (let i in lastRound.defenderLosses) {
                if (!isDefender) {
                  Object.entries(lastRound.defenderLosses[i])
                      .forEach((ship) => {
                        let shipid = ship[0];
                        let shipcount = Number(ship[1]);
                        damages[shipid] ? (damages[shipid] += shipcount) :
                ...
                if (i == id) {
                  Object.entries(lastRound.defenderLosses[i])
                      .forEach((ship) => {
                        let shipid = ship[0];
                        let shipcount = Number(ship[1]);
                        losses[shipid] ? (losses[shipid] += shipcount) :
              ...
              for (let i in lastRound.attackerLosses) {
                if (isDefender) {
                  Object.entries(lastRound.attackerLosses[i])
                      .forEach((ship) => {
                        let shipid = ship[0];
                        let shipcount = Number(ship[1]);
                        damages[shipid] ? (damages[shipid] += shipcount) :
          ...
          let cr = {
            ...
            isProbes: isProbes && json.loot.metal == 0 &&
                json.loot.crystal == 0 && json.loot.deuterium == 0 &&
                json.debris.crystalTotal < 200000,
            win: (json.result == 'defender' && isDefender) ||
  ...
  expeditionStats() {
    ...
    let renderDetails = (sums, onchange) => {
      ...
      let box = this.resourceBox(
          ...
          true, () => {
            globalDiv.empty();
            globalDiv.appendChild(this.blackHoleBox((costs) => {
    ...
    let computeRangeSums = (sums, start, stop) => {
      ...
      for (var d = new Date(start); d >= new Date(stop);
        ...
        if (sums[dateStr]) {
          ...
          for (let [type, num] of Object.entries(sums[dateStr].type)) {
            weekSums.type[type] ? (weekSums.type[type] += num) :
    ...
    content.appendChild(this.tabs({
      D: () => {
        ...
        div.appendChild(this.profitGraph(profits, max, (range, index) => {
          details.remove();
      ...
      W: () => {
        ...
        for (let i = 0; i < 12; i++) {
          let range =
              computeRangeSums(this.json.expeditionSums, start, prevMonday);
          weeks.push(range);
        ...
        div.appendChild(this.profitGraph(totals, max, (range, index) => {
          details.remove();
          details = renderDetails(range);
          div.appendChild(details);
        }));
      ...
      M: () => {
        ...
        for (let i = 0; i < 12; i++) {
          let range =
              computeRangeSums(this.json.expeditionSums, lastDay, firstDay);
          months.push(range);
          let total = getTotal(range);
          totals.push({
            profit: total,
      ...
      All: () => {
        ...
        let title =
            content.appendChild(this.createDOM('div', {class: 'ogk-date'}));
  ...
  combatStats() {
    ...
    let renderDetails = (sums, onchange) => {
      ...
      let topDiv =
          globalDiv.appendChild(this.createDOM('div', {class: 'ogk-top'}));
      ...
      sums.topCombats.forEach(async (top) => {
        ...
        topDiv.appendChild(this.createDOM(
            'div', {class: top.loot > 0 ? 'undermark' : 'overmark'},
            this.formatToUnits(top.loot)));
        topDiv.appendChild(this.createDOM(
            'div', {class: 'overmark'}, '-' + this.formatToUnits(top.losses)));
        topDiv.appendChild(this.createDOM(
            'div', {class: 'debris'}, this.formatToUnits(top.debris)));
      });
      ...
      let box = this.resourceBox(
          ...
          false, () => {
            globalDiv.empty();
            globalDiv.appendChild(this.adjustBox(sums.adjust, (adjust) => {
              let date = document.querySelector('.ogk-date strong').innerText;
              if (!this.json.combatsSums[date]) {
                this.json.combatsSums[date] = {
                  loot: [0, 0, 0],
    ...
    content.appendChild(this.tabs({
      ...
      W: () => {
        ...
        for (let i = 0; i < 12; i++) {
          let range =
              computeRangeSums(this.json.combatsSums, start, prevMonday);
          weeks.push(range);
          let total = getTotal(range);
          totals.push({
            profit: total,
      ...
      M: () => {
        ...
        for (let i = 0; i < 12; i++) {
          let range =
              computeRangeSums(this.json.combatsSums, lastDay, firstDay);
          months.push(range);
          let total = getTotal(range);
          totals.push({
            profit: total,
  ...
  blackHoleBox(onValidate) {
    ...
    if (onValidate) {
      let btn =
          box.appendChild(this.createDOM('button', {class: 'btn_blue'}, 'OK'));
      btn.addEventListener('click', () => {
        let fleet = {};
        inputs.forEach((input) => {
          let id = Number(input.getAttribute('data'));
  ...
  shipsBox(ships, minus) {
    ...
    [202, 203, 210, 204, 205, 206, 219, 207, 215, 211, 213, 218].forEach(
        (id) => {
          ...
          shipDiv.appendChild(this.createDOM(
              'span', {class: ships[id] && minus ? 'overmark' : ''},
  ...
  resourceBox(rows, am, callback) {
    ...
    if (am) {
      prod.appendChild(this.createDOM(
          'span', {}, `<a class="ogl-option resourceIcon darkmatter"></a>`));
    }
    ...
    rows.forEach((row) => {
      ...
      prod.appendChild(this.createDOM(
          'span', {class: 'ogl-metal' + (row.metal < 0 ? ' overmark' : '')},
          `${row.metal == 0 ? '-' : this.formatToUnits(row.metal)}`));
      prod.appendChild(this.createDOM(
          'span', {class: 'ogl-crystal' + (row.crystal < 0 ? ' overmark' : '')},
          `${row.crystal == 0 ? '-' : this.formatToUnits(row.crystal)}`));
      prod.appendChild(this.createDOM(
          'span', {class: 'ogl-deut' + (row.deuterium < 0 ? ' overmark' : '')},
    ...
    if (am) {
      prod.appendChild(this.createDOM(
          'span', {class: 'ogk-total'}, `${this.formatToUnits(totAm)}`));
    }
  ...
  generateMMORPGLink(playerid) {
    let lang = [
      'fr', 'de', 'en', 'es', 'pl', 'it', 'ru', 'ar', 'mx', 'tr',
      'fi', 'tw', 'gr', 'br', 'nl', 'hr', 'sk', 'cz', 'ro', 'us',
      'pt', 'dk', 'no', 'se', 'si', 'hu', 'jp', 'ba',
    ].indexOf(this.gameLang);

    return `https://www.mmorpg-stat.eu/0_fiche_joueur.php?pays=${lang}&ftr=${
        playerid}.dat&univers=_${this.universe}`;
  }

  generateHiscoreLink(playerid) {
    return `https://s${this.universe}-${
        this.gameLang}.ogame.gameforge.com/game/index.php?page=highscore&searchRelId=${
        playerid}`;
  }
  ...
  playerSearch(show, name) {
    ...
    let renderPlayerInfo = (player) => {
      ...
      let name = `<span class="ogl-${player.status}">${player.name}</span> ${
          status}
                  <a target="_self"
      ...
      let stats =
          btns.appendChild(this.createDOM('a', {class: 'ogl-mmorpgstats'}));
      let chat =
          btns.appendChild(this.createDOM('a', {class: 'icon icon_chat'}));
      ...
      detailRank.html(`
          <div><div class="ogl-totalIcon"></div> ${
          this.formatToUnits(player.points.score)} <small>pts</small></div>
          <div><div class="ogl-ecoIcon"></div> ${
          this.formatToUnits(player.economy.score)} <small>pts</small></div>
          <div><div class="ogl-techIcon"></div> ${
          this.formatToUnits(player.research.score)} <small>pts</small></div>
          <div><div class="ogl-fleetIcon"></div> ${
          this.formatToUnits(player.military.score)} <small>pts</small></div>
          <div><div class="ogl-fleetIcon grey"></div> ${
          this.formatToUnits(player.def)} <small>pts</small></div>
          <div><div class="ogl-fleetIcon orange"></div> ${
          this.formatToUnits(player.military.ships)} <small>ships</small></div>
          `);
      ...
      this.updateStalk(player.planets)
          .forEach((e) => stalkPlanets.appendChild(e));
      this.highlightTarget();
    ...
    let updatePlayerList = (players, forced) => {
      players.forEach(async (player, index) => {
        ...
        let name = this.createDOM(
            'span', {class: getStatus(player.status, noob)},
            `${player.name} ${
                player.status == '' ? '' : '(' + player.status + ') '}`);
        playerNode.appendChild(this.createDOM(
            'a', {
    ...
    let updateSearch = async (value, alliance, forced) => {
      ...
      if (value.length > 2) {
        ...
        if (possible.length == 0) {
          searchResult.appendChild(this.createDOM(
              'div', {style: 'text-align: center;'}, 'No results...'));
        }
      } else {
    ...
    let searchColumn =
        content.appendChild(this.createDOM('div', {class: 'ogl-search-col'}));

    let input = searchColumn.appendChild(this.createDOM('input', {
      type: 'search',
      placeholder: 'Player',
    }));
    ...
    setTimeout(() => {
      ...
      searchResult =
          document.querySelector('.ogl-search-content .mCSB_container');
    }, 200);
  ...
  minesOverview() {
    ...
    metalRow.appendChild(this.createDOM(
        'td', {}, '<div class="ogl-option resourceIcon metal"></div>'));
    crystalRow.appendChild(this.createDOM(
        'td', {}, '<div class="ogl-option resourceIcon crystal"></div>'));
    deutRow.appendChild(this.createDOM(
        'td', {}, '<div class="ogl-option resourceIcon deuterium"></div>'));
    nrjRow.appendChild(this.createDOM(
        'td', {}, '<div class="ogl-option resourceIcon energy"></div>'));
    ...
    this.json.empire.forEach((planet) => {
      ...
      header.appendChild(this.createDOM(
          'th', {},
          `<p>${planet.name}</p> <a href="${link}" class="ogl-fleet-coords">${
              planet.coordinates}</a> <span class="ogl-planet-fields">${
              planet.fieldUsed} / ${planet.fieldMax}</span>`));
      let td = metalRow.appendChild(this.createDOM('td'));
      td.appendChild(this.createDOM('div', {class: 'ogl-metal'}, planet[1]));
      td.appendChild(this.createDOM(
          'div', {class: 'ogl-metal'},
    ...
    td.appendChild(this.createDOM(
        'div', {class: 'ogl-metal'}, this.formatToUnits(sumhour(0))));
    td.appendChild(this.createDOM(
        'div', {class: 'ogl-metal'}, this.formatToUnits(sumday(0))));
    td.appendChild(this.createDOM(
        'div', {class: 'ogl-metal'}, this.formatToUnits(sumweek(0))));
    ...
    td.appendChild(this.createDOM(
        'div', {class: 'ogl-crystal'}, this.formatToUnits(sumhour(1))));
    td.appendChild(this.createDOM(
        'div', {class: 'ogl-crystal'}, this.formatToUnits(sumday(1))));
    td.appendChild(this.createDOM(
        'div', {class: 'ogl-crystal'}, this.formatToUnits(sumweek(1))));
    ...
    td.appendChild(this.createDOM(
        'div', {class: 'ogl-deut'}, this.formatToUnits(sumhour(2))));
    td.appendChild(this.createDOM(
        'div', {class: 'ogl-deut'}, this.formatToUnits(sumday(2))));
    td.appendChild(this.createDOM(
        'div', {class: 'ogl-deut'}, this.formatToUnits(sumweek(2))));
  ...
  generateGalaxyLink(galaxy, system, position) {
    return `?page=ingame&component=galaxy&galaxy=${galaxy}&system=${
  ...
  fleetOverview(moon) {
    ...
    this.json.empire.forEach((planet) => {
      ...
      row.appendChild(this.createDOM(
          'th', {},
          `<p>${planet.name}</p> <a class="ogl-fleet-coords" href="${link}">${
    ...
    [202,
     ...
     214,
      ...
      td.appendChild(this.createDOM(
          'span', {}, shipCount ? this.formatToUnits(shipCount) : '-'));
      row.appendChild(td);

      let th = row.appendChild(this.createDOM('th'));
      th.appendChild(this.createDOM('th', {
        class: 'ogl-option ogl-fleet-ship ogl-fleet-' + id,
      }));
  ...
  defenseOverview(moon) {
    ...
    Object.keys(shipsInfo).forEach((id) => {
      ...
      this.json.empire.forEach((planet) => {
        ...
        let valueMooon = '-';
        if (planet.moon) {
          valueMooon = planet.moon[id] == 0 ?
              '-' :
              this.formatToUnits(planet.moon[id], 2);
        }

        let td = this.createDOM(
            'td', {class: valuePLa == '-' ? 'ogl-fleet-empty' : ''}, valuePLa);
        if (moon) {
          td = this.createDOM(
              'td', {class: valueMooon == '-' ? 'ogl-fleet-empty' : ''},
              valueMooon);
        }
    ...
    if (this.page == 'fleetdispatch' && document.querySelector('#civilships') &&
      ...
      let needCargo = (fret) => {
        ...
        let amount = this.calcNeededShips({
          fret: fret,
          resources: Math.min(metal, metalAvailable) +
      ...
      let dispatch =
          document.querySelector('#shipsChosen')
      ...
      let destination =
          dispatch.appendChild(this.createDOM('div', {class: 'ogl-dest'}));
      let resDiv = dispatch.appendChild(this.createDOM('div'));
      let actions =
          resDiv.appendChild(this.createDOM('div', {class: 'ogl-transport'}));
      let coords =
          destination.appendChild(this.createDOM('div', {class: 'ogl-coords'}));

      let warning = coords.appendChild(this.createDOM('a', {
        class: 'ogl-warning tooltipRight',
        'data-title': 'Error : current planet/moon...',
      }));

      let galaxyInput = coords.appendChild(this.createDOM('input', {
        type: 'text',
        pattern: '[0-9]*',
        value: fleetDispatcher.targetPlanet.galaxy,
      }));
      let systemInput = coords.appendChild(this.createDOM('input', {
        type: 'text',
        pattern: '[0-9]*',
        value: fleetDispatcher.targetPlanet.system,
      }));
      let positionInput = coords.appendChild(this.createDOM('input', {
        type: 'text',
        pattern: '[0-9]*',
        value: fleetDispatcher.targetPlanet.position,
      }));
      let planet =
          coords.appendChild(this.createDOM('a', {class: 'ogl-planet-icon'}));
      let moon =
          coords.appendChild(this.createDOM('a', {class: 'ogl-moon-icon'}));
      let debris =
          coords.appendChild(this.createDOM('a', {class: 'ogl-debris-icon'}));
      ...
      let planetList = coords.appendChild(this.createDOM(
          'div', {class: 'ogl-homes'},
      ...
      if (unions.length != 0) {
        let unionsBtn = coords.appendChild(this.createDOM(
            'div', {class: 'ogl-union-btn'},
        ...
        unionsBtn.addEventListener('click', () => {
          ...
          for (let i in unions) {
            let union = unions[i];
            let unionDiv = container.appendChild(this.createDOM(
                'div', {class: 'ogl-quickPlanet'},
                `${union.name} [${union.galaxy}:${union.system}:${
                    union.planet}] ${union.planettype == 1 ? 'P' : 'M'}`));
      ...
      planetList.addEventListener('click', () => {
        let container = this.openPlanetList((planet) => {
          ...
          document.querySelector('.ogl-dialogOverlay')
              .classList.remove('ogl-active');
          update(true);
          fleetDispatcher.focusSubmitFleet1();
        });
      ...
      let info =
          briefing.appendChild(this.createDOM('div', {class: 'ogl-info'}));

      info.appendChild(this.createDOM('div', {}, 'Arrival'));
      let arrivalDiv =
          info.appendChild(this.createDOM('div', {class: 'ogl-arrival-time'}));
      info.appendChild(this.createDOM('div', {}, 'Duration'));
      let durationDiv =
          info.appendChild(this.createDOM('div', {class: 'ogl-duration'}));
      info.appendChild(this.createDOM('div', {}, 'Return'));
      let returnDiv =
          info.appendChild(this.createDOM('div', {class: 'ogl-return-time'}));
      returnDiv.style.visibility = 'hidden';
      info.appendChild(this.createDOM('div', {}, 'Consumption'));
      let consDiv =
          info.appendChild(this.createDOM('div', {class: 'undermark'}));

      let slider = briefing.appendChild(this.createDOM(
      ...
      $('.ogl-fleetSpeed div').on('mouseout', (event) => {
        fleetDispatcher.speedPercent =
            slider.querySelector('.ogl-active').getAttribute('data-step');
      ...
      let isDefaultMission = (index) => {
        ...
        if (index == 3) {
          if (fleetDispatcher.targetPlayerId == playerId &&
        ...
        if (index == 6 && this.json.options.expeditionMission == 6 &&
            !this.expedition) {
          return true;
        }
        return false;
      ...
      fleetDispatcher.switchToPage = (page) => {
        if (!(fleetDispatcher.currentPage == 'fleet1' && page == 'fleet3')) {
          ...
          if (fleetDispatcher.shipsToSend.length == 0) {
            document.querySelector('.ogl-dispatch .ogl-missions')
          ...
          if (missions.length == 0) {
            ...
            missionsDiv.html(
                '<span>' + fleetDispatcher.targetPlayerRankIcon +
                `<span class="status_abbr_${
                    fleetDispatcher.targetPlayerColorClass}">${
                    fleetDispatcher.targetPlayerName}</span>` +
                '</span>');
          ...
          missions.forEach((index) => {
            iconsDiv.appendChild(this.createDOM('div', {
      ...
      let fleet = JSON.stringify(fleetDispatcher.shipsToSend.map((elem) => {
        return elem.id;
      }));
      ...
      let update = (submit) => {
        if (fleetDispatcher.currentPage == 'fleet1') {
          let galaxy = clampInt(
              galaxyInput.value, 1, fleetDispatcher.fleetHelper.MAX_GALAXY,
              true);
          galaxyInput.value = galaxy;
          let system = clampInt(
              systemInput.value, 1, fleetDispatcher.fleetHelper.MAX_SYSTEM,
              true);
          systemInput.value = system;
          let position = clampInt(
              positionInput.value, 1, fleetDispatcher.fleetHelper.MAX_POSITION,
              true);
        ...
        if (submit) {
          let newFleet =
              JSON.stringify(fleetDispatcher.shipsToSend.map((elem) => {
                return elem.id;
              }));
          let newTargetPlanet = JSON.stringify(fleetDispatcher.targetPlanet);

          if (newFleet != fleet || targetPlanet != newTargetPlanet ||
              firstLoad) {
            firstLoad = false;
            warning.style.visibility = 'hidden';
            fleet = newFleet;
        ...
        durationDiv.html(
            '<strong>' + formatTime(fleetDispatcher.getDuration()) +
            '</strong>');
        consDiv.innerText =
            fleetDispatcher.getConsumption().toLocaleString('de-DE');
        if (fleetDispatcher.getConsumption() > deutAvailable) {
          consDiv.classList.add('overmark');
          if (!error) {
            warning.style.visibility = 'visible';
            warning.setAttribute(
                'data-title', 'Error : No enough deuterium...');
            document.querySelector('#continueToFleet2').style.filter =
                'hue-rotate(-50deg)';
          }
        } else {
        ...
        interval = setInterval(() => {
          arrivalDiv.html(getFormatedDate(
              new Date(serverTime).getTime() +
                  fleetDispatcher.getDuration() * 1000,
              '<strong> [G]:[i]:[s] </strong> - [d].[m]'));

          returnDiv.html(getFormatedDate(
              new Date(serverTime).getTime() +
      ...
      let btns =
          metalBtn.appendChild(this.createDOM('div', {class: 'ogl-actions'}));

      let selectMinMetal = btns.appendChild(this.createDOM('img', {
        src:
            'https://gf2.geo.gfsrv.net/cdn10/45494a6e18d52e5c60c8fb56dfbcc4.gif',
      }));
      let selectMostMetal =
          btns.appendChild(this.createDOM('a', {class: 'select-most-min'}));
      let selectMaxMetal = btns.appendChild(this.createDOM('img', {
        src:
      ...
      let crystalFiller =
          crystalBtn.appendChild(this.createDOM('input', {type: 'text'}));
      ...
      let crystalBtns =
          crystalBtn.appendChild(this.createDOM('div', {class: 'ogl-actions'}));

      let selectMinCrystal = crystalBtns.appendChild(this.createDOM('img', {
        src:
            'https://gf2.geo.gfsrv.net/cdn10/45494a6e18d52e5c60c8fb56dfbcc4.gif',
      }));
      let selectMostCrystal = crystalBtns.appendChild(
          this.createDOM('a', {class: 'select-most-min'}));
      let selectMaxCrystal = crystalBtns.appendChild(this.createDOM('img', {
        src:
            'https://gf3.geo.gfsrv.net/cdnea/fa0c8ee62604e3af52e6ef297faf3c.gif',
      }));
      ...
      let deutFiller =
          deutBtn.appendChild(this.createDOM('input', {type: 'text'}));
      ...
      let deutBtns =
          deutBtn.appendChild(this.createDOM('div', {class: 'ogl-actions'}));

      let selectMinDeut = deutBtns.appendChild(this.createDOM('img', {
        src:
            'https://gf2.geo.gfsrv.net/cdn10/45494a6e18d52e5c60c8fb56dfbcc4.gif',
      }));
      let selectMostDeut =
          deutBtns.appendChild(this.createDOM('a', {class: 'select-most-min'}));
      let selectMaxDeut = deutBtns.appendChild(this.createDOM('img', {
        src:
            'https://gf3.geo.gfsrv.net/cdnea/fa0c8ee62604e3af52e6ef297faf3c.gif',
      }));
      ...
      $('#selectMinMetal')
          .after(this.createDOM(
              'a', {id: 'selectMostMetal', class: 'select-most-min'}));
      $('#selectMinCrystal').after(this.createDOM('a', {
        id: 'selectMostCrystal',
        class: 'select-most-min',
      }));
      $('#selectMinDeuterium').after(this.createDOM('a', {
        id: 'selectMostDeuterium',
        class: 'select-most-min',
      }));

      $('#selectMaxMetal')
          .after(this.createDOM('span', {class: 'ogi-metalLeft'}, '-'));
      $('#selectMaxCrystal')
          .after(this.createDOM('span', {class: 'ogi-crystalLeft'}, '-'));
      $('#selectMaxDeuterium')
          .after(this.createDOM('span', {class: 'ogi-deuteriumLeft'}, '-'));

      $('#allresources').after(this.createDOM('a', {class: 'select-most'}));
      $('#allresources')
          .after(this.createDOM('a', {class: 'send_none'}, '<a></a>'));
      ...
      let refreshRes = () => {
        ...
        if (firstResRefresh) {
          ...
          mLeft.innerText =
              Math.max(0, metalAvailable - val).toLocaleString('de-DE');
          val = this.removeNumSeparator(
              document.querySelector('input#crystal').value);
          cLeft.innerText =
              Math.max(0, crystalAvailable - val).toLocaleString('de-DE');
          val = this.removeNumSeparator(
              document.querySelector('input#deuterium').value);
          dLeft.innerText =
              Math.max(
                      0, deutAvailable - fleetDispatcher.getConsumption() - val)
      ...
      $('#selectMostMetal').on('click', () => {
        ...
        fleetDispatcher.cargoMetal =
            Math.min(capacity, Math.max(0, metalAvailable - (kept[0] || 0)));
        fleetDispatcher.refresh();
        fleetDispatcher.focusSubmitFleet3();
      });
      $('#selectMostCrystal').on('click', () => {
        ...
        fleetDispatcher.cargoCrystal =
            Math.min(capacity, Math.max(0, crystalAvailable - (kept[1] || 0)));
        fleetDispatcher.refresh();
        fleetDispatcher.focusSubmitFleet3();
      });
      ...
      let selectMostRes =
          load.appendChild(this.createDOM('a', {class: 'select-most'}));
      let selectAllRes =
          load.appendChild(this.createDOM('a', {class: 'sendall'}));
      ...
      let settings = load.appendChild(this.createDOM(
          'div', {class: 'ogl-setting-icon'},
      ...
      let updateCargo = () => {
        let total = Number(this.removeNumSeparator(metalFiller.value)) +
        ...
        <div>
        <span class="${freeSpace >= 0 ? 'undermark' : 'overmark'}">${
                freeSpace.toLocaleString('de-DE')} </span>
        / <span> ${
                fleetDispatcher.getCargoCapacity().toLocaleString(
                    'de-DE')}</span>
        </div>`);
      ...
      if (this.json.pbFret != 0) {
        pbBtn = transport.appendChild(this.createDOM('a', {
          'tech-id': 210,
      ...
      let onResChange = (index) => {
        ...
        let deut = Math.min(
            this.removeNumSeparator(deutFiller.value), capacity,
            deutAvailable - fleetDispatcher.getConsumption());
        ...
        if (index == 2) {
          ...
          if (fleetDispatcher.getFreeCargoSpace() == 0 &&
              deutLeft.innerText != '0') {
            deutLeft.classList.add('overmark');
            deutReal.innerText = Math.max(0, fleetDispatcher.cargoDeuterium)
                                     .toLocaleString('de-DE');
          } else {
          ...
          let crystal = Math.min(
              this.removeNumSeparator(crystalFiller.value), capacity,
              crystalAvailable);
          ...
          if (fleetDispatcher.getFreeCargoSpace() == 0 &&
            ...
            crystalReal.innerText = Math.max(0, fleetDispatcher.cargoCrystal)
                                        .toLocaleString('de-DE');
          } else {
          ...
          let metal = Math.min(
              this.removeNumSeparator(metalFiller.value), capacity,
              metalAvailable);

          fleetDispatcher.cargoMetal = Math.min(
              metal,
              fleetDispatcher.cargoMetal + fleetDispatcher.getFreeCargoSpace());
          metalLeft.innerText = (metalAvailable - fleetDispatcher.cargoMetal)
                                    .toLocaleString('de-DE');
          ...
          if (fleetDispatcher.getFreeCargoSpace() == 0 &&
            ...
            metalReal.innerText =
                Math.max(0, fleetDispatcher.cargoMetal).toLocaleString('de-DE');
          } else {
      ...
      if (this.mode == 2 || this.mode == 1) {
        if (this.mode == 1) {
          ...
          let coords = fleetDispatcher.targetPlanet.galaxy + ':' +
              fleetDispatcher.targetPlanet.system + ':' +
              fleetDispatcher.targetPlanet.position;
          coords += fleetDispatcher.targetPlanet.type ==
                  fleetDispatcher.fleetHelper.PLANETTYPE_MOON ?
              'M' :
              'P';
      ...
      shipsOnPlanet.forEach((ship) => {
        if (ship.id == 202 || ship.id == 203) {
          ...
          let span = this.createDOM(
              'span', {class: 'ogl-needed'}, amount.toLocaleString('de-DE'));
          document.querySelector(`.technology[data-technology="${ship.id}"]`)
              .appendChild(span);
  ...
  harvest() {
    ...
    let btnAction = (event, coords, type) => {
      ...
      let link = `?page=ingame&component=fleetdispatch&galaxy=${
          coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${
          type}&mission=${this.json.options.harvestMission}&oglMode=1`;
    ...
    this.planetList.forEach((planet) => {
      let coords =
          planet.querySelector('.planet-koords').textContent.split(':');

      if (this.current.coords != coords.join(':') || this.current.isMoon) {
        let btn = planet.querySelector('.planetlink')
                      .appendChild(this.createDOM(
                          'div', {class: 'ogl-harvest ogl-planet'}));
      ...
      if (moon) {
        ...
        let btn = planet.querySelector('.moonlink')
                      .appendChild(this.createDOM(
                          'button', {class: 'ogl-harvest ogl-moon'}));
  ...
  openPlanetList(callcback) {
    let container = this.createDOM(
        'div', {class: 'ogl-dialogContainer ogl-quickLinks'}
    ...
    let buildButton = (planet, id, galaxy, system, position, type) => {
      ...
      if (type == 1)
        div.classList.add('ogl-quickPlanet');
      else
        div.classList.add('ogl-quickMoon');

      div.addEventListener('click', () => callcback(data));

      if ((planet == this.current.planet && !this.current.isMoon &&
           type == 1) ||
    ...
    this.planetList.forEach((planet) => {
      ...
      let btn = buildButton(
          planet,
          new URL(planet.querySelector('.planetlink').href)
              .searchParams.get('cp'),
          coords[0], coords[1], coords[2], 1);

      btn.html(`[${coords.join(':')}] ${
      ...
      if (planet.querySelector('.moonlink')) {
        let btn = buildButton(
            planet,
            new URL(planet.querySelector('.moonlink').href)
                .searchParams.get('cp'),
            coords[0], coords[1], coords[2], 3);

        btn.html('<figure class="planetIcon moon"></figure>');
      } else
        container.appendChild(this.createDOM('div'));
    });
  ...
  autoHarvest() {
    ...
    if (this.page == 'fleetdispatch') {
      let nextElement = this.current.planet.nextElementSibling ||
      ...
      // ignore source
      if (nextElement.querySelector('.planet-koords').textContent ==
      ...
      if (this.current.isMoon && this.mode == 5 &&
      ...
      if ((this.current.isMoon && this.mode == 3) ||
          (this.current.isMoon && this.mode == 5)) {
        if (nextElement.querySelector('.moonlink')) {
          id = new URL(nextElement.querySelector('.moonlink').href)
                   .searchParams.get('cp');
        }
      }
      ...
      if (this.mode == 3) {
        ...
        destination =
            nextElement.querySelector('.planet-koords').textContent.split(':');
        if (!this.current.isMoon) type = 3;
      }

      let link = `?page=ingame&component=fleetdispatch&galaxy=${
          destination[0]}&system=${destination[1]}&position=${
          destination[2]}&type=${type}&mission=${mission}&cp=${id}&oglMode=${
          this.mode}`;
      link =
          'https://' + window.location.host + window.location.pathname + link;

      let needed = document.querySelector(`.technology[data-technology="${
          this.json.options.fret}"] .ogl-needed`);
      ...
      let pCoords =
          this.current.isMoon ? this.current.coords + 'M' : this.current.coords;
      let sent = false;
  ...
  expedition() {
    if (this.page == 'fleetdispatch') {
      dataHelper.getPlayer(playerId).then((player) => {
        ...
        maxTotal = this.playerClass == PLAYER_CLASS_EXPLORER ?
            maxTotal * 3 * this.json.speed :
            maxTotal * 2;

        let maxPT = Math.max(
            minPT, this.calcNeededShips({fret: 202, resources: maxTotal}));
        let maxGT = Math.max(
            minGT, this.calcNeededShips({fret: 203, resources: maxTotal}));

        if (!document.querySelector('#allornone .allornonewrap')) return;

        let ptExpe = document.querySelector('#allornone .secondcol')
                         .appendChild(this.createDOM(
                             'button', {class: 'ogl-prefab ogl-ptexpe'}));
        let gtExpe = document.querySelector('#allornone .secondcol')
                         .appendChild(this.createDOM(
                             'button', {class: 'ogl-prefab ogl-gtexpe'}));
        ...
        [ptExpe, gtExpe].forEach((btn) => {
          btn.addEventListener('click', () => {
            ...
            prio.forEach((shipID) => {
              if (bigship == 0 &&
                  document.querySelector(`.technology[data-technology="${
                                             shipID}"] .amount`)
            ...
            shipsOnPlanet.forEach((ship) => {
              if (ship.id == type)
                this.selectShips(ship.id, count);
              else if (ship.id == bigship)
                this.selectShips(ship.id, 1);
              else if (ship.id == 210)
                this.selectShips(ship.id, 1);
              else if (ship.id == 219)
                this.selectShips(ship.id, 1);
  ...
  quickPlanetList() {
    if (this.page == 'fleetdispatch' && fleetDispatcher) {
      ...
      let btn =
          document.querySelector('#shortcuts span')
  ...
  activitytimers() {
    ...
    this.planetList.forEach((planet) => {
      ...
      let pTimer = planet.querySelector('.planetlink')
                       .appendChild(this.createDOM('div', {
      ...
      if (planet.querySelector('.moonlink')) {
        let mTimer = planet.querySelector('.moonlink')
                         .appendChild(this.createDOM('div', {
                           class: 'ogl-timer ogl-short ogl-medium',
                           'data-timer': Math.min(
                               Math.round((now - timers[1]) / 60000), 60),
                         }));

        if (this.json.options.activitytimers && value != 60 && value >= 15) {
          planet.querySelector('.moonlink')
              .appendChild(this.createDOM(
                  'div', {class: 'activity showMinutes'}, value));
        }
  ...
  async getEmpireInfo() {
    ...
    return fetch(
               `https://s${this.universe}-${
                   this.gameLang}.ogame.gameforge.com/game/index.php?page=standalone&component=empire`,
               {signal: abortController.signal})
        .then((rep) => rep.text())
        .then((str) => {
          let planets = JSON.parse(str.substring(
          ...
          if (hasMoon) {
            return fetch(
                       `https://s${this.universe}-${
                           this.gameLang}.ogame.gameforge.com/game/index.php?page=standalone&component=empire&planetType=1`,
                       {signal: abortController.signal})
                .then((rep) => rep.text())
                .then((str) => {
                  let moons =
                      JSON.parse(str.substring(
                                     str.indexOf('createImperiumHtml') + 47,
                                     str.indexOf('initEmpire') - 16))
                          .planets;
  ...
  getFlyingRes() {
    ...
    document.querySelectorAll('.eventFleet').forEach((line) => {
      ...
      let coords =
          line.querySelector('.coordsOrigin > a').innerText.trim().slice(1, -1);

      let destCoords =
          line.querySelector('.destCoords > a').innerText.trim().slice(1, -1);
      ...
      if (type == 4 || (back && !(id - 1 in uniques) && !(id - 2 in uniques))) {
        ...
        div.querySelectorAll('td[colspan="2"]').forEach((tooltip) => {
          ...
          if (id) {
            ...
            fleetCount[id] ? (fleetCount[id] += count) :
                             (fleetCount[id] = count);
          } else {
            if (!planets[coords]) {
  ...
  hasActivityChanged(oldAct, newAct) {
    return (
        (oldAct == 0 && newAct > 0) || (oldAct > 0 && newAct == 0) ||
        (oldAct < 61 && newAct == 61));
  }
  ...
  updateresourceDetail() {
    ...
    let mSumP = 0, cSumP = 0, dSumP = 0;
    let mSumM = 0, cSumM = 0, dSumM = 0;

    let emulatedEmpire = [];
    !this.commander && this.planetList.forEach((planet) => {
      let coords = planet.querySelector('.planet-koords').textContent;
      ...
      let crystal = 0;
      let deuterium = 0;
      let metProd = 0, criProd = 0, deutProd = 0, diff = 0;
    ...
    empire.forEach((planet) => {
      ...
      let isaFullM =
          ...
          '' :
          ' ogl-afull';
      let isaFullC = planet.crystalStorage - planet.crystal >
              planet.production.hourly[1] * 2 ?
          '' :
          ' ogl-afull';
      let isaFullD = planet.deuteriumStorage - planet.deuterium >
      ...
      sumNodes[1].querySelector('.ogl-metal').innerText =
          this.formatToUnits(this.json.flying.metal);
      sumNodes[1].querySelector('.ogl-crystal').innerText =
          this.formatToUnits(this.json.flying.crystal);
      sumNodes[1].querySelector('.ogl-deut').innerText =
          this.formatToUnits(this.json.flying.deuterium);

      sumNodes[2].querySelector('.ogl-metal').innerText =
          this.formatToUnits(mSumP + mSumM + this.json.flying.metal);
      sumNodes[2].querySelector('.ogl-crystal').innerText =
          this.formatToUnits(cSumP + cSumM + this.json.flying.crystal);
      sumNodes[2].querySelector('.ogl-deut').innerText =
  ...
  resourceDetail() {
    ...
    !this.isMobile && rechts.addEventListener('mouseover', () => {
    ...
    !this.isMobile && rechts.addEventListener('mouseout', (e) => {
    ...
    flying.appendChild(this.createDOM(
        'span', {class: 'ogl-metal'},
        this.formatToUnits(this.json.flying.metal)));
    flying.appendChild(this.createDOM(
        'span', {class: 'ogl-crystal'},
    ...
    flyingSum.appendChild(this.createDOM(
        'div', {class: 'ogl-sum-symbol'},
    ...
    empire.forEach((elem) => {
      ...
      let isaFullC =
          ...
          '' :
          ' ogl-afull';
      let isaFullD = elem.deuteriumStorage - elem.deuterium >
              elem.production.hourly[2] * 2 ?
          '' :
          ' ogl-afull';
      ...
      divPla.appendChild(this.createDOM(
          'span', {class: 'ogl-metal' + isFullM + isaFullM},
          this.formatToUnits(elem.metal)));
      divPla.appendChild(this.createDOM(
          'span', {class: 'ogl-crystal' + isFullC + isaFullC},
          this.formatToUnits(elem.crystal)));
      divPla.appendChild(this.createDOM(
          'span', {class: 'ogl-deut' + isFullD + isaFullD},
      ...
      planet.querySelector('.planetlink')
          .parentNode.insertBefore(
              divPla, planet.querySelector('.planetlink').nextSibling);
      ...
      if (elem.moon) {
        ...
        divMoon.appendChild(this.createDOM(
            'span', {class: 'ogl-metal'}, this.formatToUnits(elem.moon.metal)));
        divMoon.appendChild(this.createDOM(
            'span', {class: 'ogl-crystal'},
    ...
    divPlaSum.appendChild(this.createDOM(
        'span', {class: 'ogl-metal'}, this.formatToUnits(mSumP)));
    divPlaSum.appendChild(this.createDOM(
        'span', {class: 'ogl-crystal'}, this.formatToUnits(cSumP)));
    ...
    divMoonSum.appendChild(this.createDOM(
        'span', {class: 'ogl-metal'}, this.formatToUnits(mSumM)));
    divMoonSum.appendChild(this.createDOM(
        'span', {class: 'ogl-crystal'}, this.formatToUnits(cSumM)));
    ...
    sumres.appendChild(this.createDOM(
        'span', {class: 'ogl-metal'},
        this.formatToUnits(mSumP + mSumM + this.json.flying.metal)));
    sumres.appendChild(this.createDOM(
        'span', {class: 'ogl-crystal'},
  ...
  updateInfo() {
    ...
    document.querySelector('#countColonies')
  ...
  targetList(show) {
    let renderTagetList = () => {
      ...
      let header =
          div.appendChild(this.createDOM('div', {class: 'ogk-controls'}));
      ...
      ['red', 'yellow', 'green', 'blue', 'violet', 'gray'].forEach((color) => {
        ...
        toggle.addEventListener('click', () => {
          this.json.options.hiddenTargets[color] =
              this.json.options.hiddenTargets[color] ? false : true;
          this.saveData();
          ...
          content.querySelectorAll(`[data-marked="${color}"]`)
              .forEach((planet) => {
                if (this.json.options.hiddenTargets[color])
                  planet.classList.add('ogl-colorHidden');
                else
                  planet.classList.remove('ogl-colorHidden');
              });
      ...
      let filterTabs = header.appendChild(this.createDOM('div', {
        class: 'ogl-tabList',
        style: 'margin-bottom: 5px;',
      }));
      let tabG = filterTabs.appendChild(this.createDOM(
          'div', {class: 'ogl-tab' + (!galaxy ? ' ogl-active' : '')}, 'Gs'));
      ...
      let tabS = filterTabs.appendChild(this.createDOM(
          'div', {class: 'ogl-tab' + (!system ? ' ogl-active' : '')}, 'Ss'));
      ...
      // popup content
      let content = div.appendChild(this.createDOM('div', {
      ...
      let checkEmpty = (galaxy, system) => {
        for (let g = 1; g <= 10; g++) {
          if (galaxy) {
            ...
            if (children)
              header.querySelector(`.ogl-tab[data-galaxy="${g}"]`)
        ...
        for (let s = 0; s < step * 10; s += step) {
          if (system) {
            let children = content.querySelector(
                `[data-galaxy="${this.json.targetTabs.g}"][data-system="${
                    s}"]:not(.ogl-colorHidden)`);

            if (children)
              header.querySelector(`.ogl-tab[data-system="${s}"]`)
                  .classList.remove('ogl-isEmpty');
            else
              header.querySelector(`.ogl-tab[data-system="${s}"]`)
                  .classList.add('ogl-isEmpty');
      ...
      for (let i = 0; i < step * 10; i += step) {
        ...
        sTab.addEventListener('click', (event) => {
          if (!system) return;
          header.querySelectorAll('.ogl-tab[data-system].ogl-active')
      ...
      // galaxy buttons
      for (let i = 1; i <= 10; i++) {
        let gTab = galaxyTabList.appendChild(this.createDOM(
            'div', {class: 'ogl-tab', 'data-galaxy': i}, 'G' + i));
        ...
        gTab.addEventListener('click', (event) => {
          if (!galaxy) return;
          header.querySelectorAll('.ogl-tab[data-galaxy]')
              .forEach((e) => e.classList.remove('ogl-active'));
          event.target.classList.add('ogl-active');
      ...
      keys.forEach((coords) => {
        ...
        if (this.json.markers[coords]) {
          let a = this.renderPlanet(
              coords, false, false, this.json.markers[coords].moon);
          ...
          if (this.json.options
                  .hiddenTargets[this.json.markers[coords].color]) {
    ...
    if (show) {
      ...
      document.querySelector('#rechts').children[0].appendChild(
          renderTagetList());
    } else {
  ...
  addMarkerUI(coords, parent, id) {
    ...
    ['red', 'yellow', 'green', 'blue', 'violet', 'gray'].forEach((color) => {
      let circle =
          div.appendChild(this.createDOM('div', {'data-marker': color}));
      ...
      circle.addEventListener('click', () => {
        div.querySelectorAll('div[data-marker]')
            .forEach((e) => e.classList.remove('ogl-active'));
        dataHelper.getPlayer(id).then((player) => {
          ...
          document.querySelector('.ogl-targetIcon')
  ...
  sendMessage(id) {
    if (this.tchat) {
      ...
      document.location = `https://s${this.universe}-${
          this.gameLang}.ogame.gameforge.com/game/index.php?page=chat&playerId=${
          id}`;
    }
  }
  ...
  generateIgnoreLink(playerId) {
    return `https://s${this.universe}-${
        this.gameLang}.ogame.gameforge.com/game/index.php?page=ignorelist&action=1&id=${
        playerId}`;
  }
  generateBuddyLink(playerId) {
    return `https://s${this.universe}-${
        this.gameLang}.ogame.gameforge.com/game/index.php?page=ingame&component=buddies&action=7&id=${
        playerId}&ajax=1`;
  }
  ...
  stalk(sender, player, delay) {
    ...
    let render = (player) => {
      ...
      <h1>${player.name} 
        <a href="${
          this.generateHiscoreLink(player.id) || ''}" class="ogl-ranking">
          #${player.points.position || 'b'}
        </a>
      </h1>
      ...
      let actions =
          content.appendChild(this.createDOM('div', {class: 'ogi-actions'}));
      actions.html(`
      <a href="${
          this.generateIgnoreLink(player.id)}" class="icon icon_against"></a>
      <a href="${
          this.generateBuddyLink(
              player.id)}" class="icon icon_user overlay buddyrequest"></a>`);

      let msgBtn =
          actions.appendChild(this.createDOM('a', {class: 'icon icon_chat'}));
      msgBtn.addEventListener('click', () => {
        this.sendMessage(player.id);
      });

      let actBtn = actions.appendChild(this.createDOM(
          'a', {style: 'margin-left: 10px', class: 'ogl-text-btn'}, '&#9888'));
        ...
        if (this.page != 'galaxy') {
          let coords =
              document
                  ...
                  .split(':');
          location.href =
              `?page=ingame&component=galaxy&galaxy=${coords[0]}&system=${
                  coords[1]}&position=${coords[2]}&id=${player.id}`;
        }
      ...
      let date = content.appendChild(this.createDOM('span', {
        style: 'margin-top: 2px;',
        class: 'ogl-right ogl-date',
      }));
      content.appendChild(this.createDOM('hr'));
      let detailRank =
          content.appendChild(this.createDOM('div', {class: 'ogl-detailRank'}));
      content.appendChild(this.createDOM('hr'));
      let list = content.appendChild(this.createDOM('div', {
        class: 'ogl-stalkPlanets',
        'player-id': player.id,
      }));
      ...
      let sideStalk =
          content.appendChild(this.createDOM('a', {class: 'ogl-pin'}));

      sideStalk.addEventListener('click', () => this.sideStalk(player.id));

      let stats =
          content.appendChild(this.createDOM('a', {class: 'ogl-mmorpgstats'}));
      stats.addEventListener('click', () => {
      ...
      this.page == 'galaxy' ? (pos = {bottom: pos < 4, top: pos > 4}) :
                              (pos = {});
      ...
      <div><div class="ogl-fleetIcon orange"></div> ${
          this.formatToUnits(
              player.military.ships)} <small>ships</small></div>`);
    };
  ...
  renderPlanet(coords, main, scanned, moon, deleted) {
    ...
    let planetDiv =
        a.appendChild(this.createDOM('div', {class: 'ogl-planet-div'}));
    let planetIcon =
        planetDiv.appendChild(this.createDOM('div', {class: 'ogl-planet'}));

    let panel = planetDiv.appendChild(
        this.createDOM('div', {class: 'ogl-planet-hover'}));
    let plaspy =
        panel.appendChild(this.createDOM('button', {class: 'icon_eye'}));
    let plaFleet = panel.appendChild(this.createDOM('div', {class: 'ogl-atk'}));

    plaspy.addEventListener('click', (e) => {
      sendShipsWithPopup(
          6, coords[0], coords[1], coords[2], 0, this.json.spyProbes);
      e.stopPropagation();
    });
    plaFleet.addEventListener('click', (e) => {
      window.location.href = `?page=ingame&component=fleetdispatch&galaxy=${
          coords[0]}&system=${coords[1]}&position=${coords[2]}&type=1`;
      e.stopPropagation();
    });
    ...
    panel =
        moonDiv.appendChild(this.createDOM('div', {class: 'ogl-moon-hover'}));
    plaFleet = panel.appendChild(this.createDOM('div', {class: 'ogl-atk'}));
    plaspy = panel.appendChild(this.createDOM('button', {class: 'icon_eye'}));

    plaspy.addEventListener('click', (e) => {
      sendShipsWithPopup(
          6, coords[0], coords[1], coords[2], 3, this.json.spyProbes);
      e.stopPropagation();
    });
    plaFleet.addEventListener('click', (e) => {
      window.location.href = `?page=ingame&component=fleetdispatch&galaxy=${
          coords[0]}&system=${coords[1]}&position=${coords[2]}&type=3`;
      e.stopPropagation();
    });

    a.addEventListener('click', () => {
      let link = `?page=ingame&component=galaxy&galaxy=${coords[0]}&system=${
          coords[1]}&position=${coords[2]}`;
      link =
          'https://' + window.location.host + window.location.pathname + link;
  ...
  updateStalk(planets) {
    ...
    sorted.forEach((planet) => {
      ...
      let moonDiv =
          a.appendChild(this.createDOM('div', {class: 'ogl-moon-div'}));
      moonDiv.appendChild(this.createDOM('div', {class: 'ogl-moon-act'}));

      let mIcon =
          moonDiv.appendChild(this.createDOM('div', {class: 'ogl-moon'}));
  ...
  sideStalk(playerid) {
    ...
    if (last) {
      ...
      sideStalk = document.querySelector('#links').appendChild(
          this.createDOM('div', {class: 'ogl-sideStalk'}));
      ...
      dataHelper.getPlayer(playerid).then((player) => {
        sideStalk.appendChild(this.createDOM(
            'div', {class: 'ogi-title ' + getStatus(player.status)},
            player.name));

        sideStalk.appendChild(this.createDOM('hr'));

        let container = sideStalk.appendChild(this.createDOM('div', {
          class: 'ogl-stalkPlanets',
          'player-id': player.id,
        }));
        ...
        let index = 0;
        let first = true;
        actBtn && actBtn.addEventListener('click', () => {
        ...
        watchlistBtn && watchlistBtn.addEventListener('click', () => {
          sideStalk.empty();
          sideStalk.appendChild(
              this.createDOM('div', {class: 'title'}, 'Historic'));
          sideStalk.appendChild(this.createDOM('hr'));
          this.json.sideStalk.slice().reverse().forEach((id) => {
            dataHelper.getPlayer(id).then((player) => {
              let playerDiv = sideStalk.appendChild(
                  this.createDOM('div', {class: 'ogl-player'}));
              playerDiv.appendChild(this.createDOM(
                  'span', {class: getStatus(player.status)}, player.name));
...
`);
  ...
  spyTable() {
    if (this.page == 'fleetdispatch' && this.mode == 4) {
      let link = 'https://' + window.location.host + window.location.pathname +
          '?page=messages';
    ...
    // let old = ogame.messages.doInitAction;
    // let first = true;
    // ogame.messages.doInitAction = (tab) => {
    //   old(tab);
    //   alert();
    //   //   if (first) return;
    //   //   document.querySelector(".ogl-spyTable") &&
    ...
    if (document.querySelector('#tabs-nfFleets.ui-state-active')) {
      messages = document.querySelectorAll(
          '#' +
          document.querySelector('#subtabs-nfFleet20')
              .getAttribute('aria-controls') +
          ' .msg');
    } else {
      messages = document.querySelectorAll(
          '#' +
          document.querySelector('#tabs-nfFavorites')
              .getAttribute('aria-controls') +
          ' .msg');
    }
    ...
    messages.forEach((msg) => {
      ...
      let cleanDate =
          new Date(`${rawDate[2]}-${rawDate[1]}-${rawDate[0]} ${rawDate[3]}`);
      let deltaDate = Date.now() - cleanDate;
      let mins = deltaDate / 60000;
      let hours = mins / 60;

      if (!parseInt(
              data[0].querySelectorAll('span.fright')[0].textContent.match(
                  /\d+/)))
        return;
      ...
      report.status = data[0]
                          ...
                          .textContent.replace(/&nbsp;/g, '')
                          .trim();
      report.spy = msg.querySelector('a[onclick*="sendShipsWithPopup"]')
                       .getAttribute('onclick');
      report.activity =
          parseInt(data[0].querySelectorAll('span.fright')[0].textContent.match(
              /\d+/)[0]);
      report.coords =
          /\[.*\]/g.exec(msg.querySelector('.msg_title').innerHTML)[0].slice(
              1, -1);
      ...
      report.fleet = data[4].querySelectorAll('span').length > 0 ?
          this.cleanValue(data[4]
                              .querySelectorAll('span.ctn')[0]
                              .textContent.replace(/(\D*)/, '')
                              .split(' ')[0]) :
          'No Data';
      report.defense = data[4].querySelectorAll('span').length > 1 ?
          this.cleanValue(data[4]
      ...
      report.metal = this.cleanValue(
          data[2].querySelectorAll('.resspan')[0].textContent.replace(
              /(\D*)/, ''));
      report.crystal = this.cleanValue(
          data[2].querySelectorAll('.resspan')[1].textContent.replace(
              /(\D*)/, ''));
      report.deut = this.cleanValue(
          data[2].querySelectorAll('.resspan')[2].textContent.replace(
              /(\D*)/, ''));
      report.total = report.metal + report.crystal + report.deut;
      report.renta = Math.round((report.total * report.loot) / 100);

      report.apiKey = msg.querySelector('.icon_apikey').getAttribute('title') ||
    ...
    if (this.reportList != []) {
      // alert(document.querySelector(".ogl-spyTable"));
  ...
  sortTable(arr) {
    ...
    let tab, pagination;
    if (document.querySelector('#tabs-nfFleets.ui-state-active')) {
      tab = document.querySelector('#subtabs-nfFleet20')
                .getAttribute('aria-controls');
      pagination = document.querySelector('#' + tab + ' .pagination');
      console.log(pagination);
    } else {
      tab = document.querySelector('#tabs-nfFavorites')
                .getAttribute('aria-controls');
      pagination = document.querySelector('#' + tab + ' .pagination');
      console.log(pagination);
    }
    ...
    // eye icon, toggle report table
    let enableTable = tableOptions.appendChild(this.createDOM('button', {
    ...
    enableTable.addEventListener('click', () => {
      this.json.options.spyTableEnable =
          this.json.options.spyTableEnable ? false : true;
      this.saveData();
      document.location.reload();
    });

    let appendOption = tableOptions.appendChild(this.createDOM('button', {
    ...
    appendOption.addEventListener('click', () => {
      this.json.options.spyTableAppend =
          this.json.options.spyTableAppend ? false : true;
      this.saveData();
      document.location.reload();
    });

    // trash icon, toggle auto delete
    let autoDelete = tableOptions.appendChild(this.createDOM('button', {
    ...
    autoDelete.addEventListener('click', () => {
      this.json.options.autoDeleteEnable =
          this.json.options.autoDeleteEnable ? false : true;
      this.saveData();
      document.location.reload();
    });
    ...
    let sc = cargoChoice.appendChild(this.createDOM('div', {
      class: 'ogl-option ogl-fleet-ship ogl-fleet-202',
    }));
    let lc = cargoChoice.appendChild(this.createDOM('div', {
      class: 'ogl-option ogl-fleet-ship ogl-fleet-203',
    }));
    let pf = cargoChoice.appendChild(this.createDOM('div', {
      class: 'ogl-option ogl-fleet-ship ogl-fleet-219',
    }));
    ...
    if (this.json.pbFret != 0) {
      let pb = cargoChoice.appendChild(this.createDOM('div', {
        class: 'ogl-option ogl-fleet-ship ogl-fleet-210',
      }));
      pb.addEventListener('click', () => updateDefaultShip(210));
    }

    let cargo = header.appendChild(this.createDOM(
        'th', {},
        `<span style="display: flex;" class="ogl-option ogl-fleet-ship ogl-fleet-${
            this.json.options.spyFret}"></span>`));

    cargo.addEventListener(
        'mouseover', () => this.tooltip(cargo, cargoChoice, false, false, 50));
...
`;
      ...
      let date = line.appendChild(this.createDOM(
          'td', {class: 'tooltipLeft ogl-date', title: dateDetail}, dateText));
      if (report.activity <= 15)
        date.classList.add('ogl-danger');
      else if (report.activity < 60)
        date.classList.add('ogl-care');
      else
        date.classList.add('ogl-good');

      let moonContent =
          report.type == 3 ? `<figure class="planetIcon moon"></figure>` : '';

      let coords = line.appendChild(this.createDOM('td'));
      coords.appendChild(this.createDOM(
          'a', {href: report.coordsLink}, report.coords + moonContent));
      ...
      let link = name.appendChild(this.createDOM(
          'a', {class: status[report.status]},
...
`;
      ...
      let total = line.appendChild(this.createDOM(
          'td', {class: 'tooltipLeft ogl-lootable', title: totalDetail},
          this.formatToUnits(report.renta)));

      if (this.json.options.rvalLimit <=
      ...
      total.style.background =
          `linear-gradient(to right, rgba(255, 170, 204, 0.63) ${
              report.resRatio[0]}%, rgba(115, 229, 255, 0.78) ${
              report.resRatio[0]}%
, rgba(115, 229, 255, 0.78) ${
              report.resRatio[0] +
              report.resRatio[1]}%, rgb(166, 224, 176) ${report.resRatio[2]}%)`;
      ...
      if (report.defense == 0 && report.fleet == 0 &&
      ...
      let fleetLink = `?page=ingame&component=fleetdispatch&galaxy=${
          splittedCoords[0]}&system=${splittedCoords[1]}&position=${
          splittedCoords[2]}&type=${report.type}&mission=1&am${shipId}=${
          shipCount}&oglMode=4`;

      let ship = line.appendChild(this.createDOM('td'));
      ship.appendChild(this.createDOM(
          'a', {
      ...
      colors.appendChild(this.createDOM('div', {
        class: 'ogl-colors',
      ...
      dataHelper.getPlayer(report.name).then((player) => {
        ...
        if (this.json.markers[report.coords]) {
          ...
          line.setAttribute(
              'data-marked', this.json.markers[report.coords].color);
        }
      });

      let opt =
          line.appendChild(this.createDOM('td', {class: 'ogl-spyOptions'}));

      opt.appendChild(this.createDOM('button', {
        class: 'icon icon_maximize overlay',
        href: report.detail,
      }));
      let simulateBtn =
          opt.appendChild(this.createDOM('a', {class: 'ogl-text-btn'}, 'T'));
      ...
      attackBtn.addEventListener('click', () => {
        let fleetLink = `?page=ingame&component=fleetdispatch&galaxy=${
            splittedCoords[0]}&system=${splittedCoords[1]}&position=${
            splittedCoords[2]}&type=${report.type}&mission=1&oglMode=4`;
      ...
      let deleteBtn =
          opt.appendChild(this.createDOM('button', {class: 'icon icon_trash'}));
      ...
      if (this.json.options.autoDeleteEnable && report.fleet == 0 &&
          Math.round((report.total * report.loot) / 100) <
              this.json.options.rvalLimit) {
        deleteBtn.click();
      }
    ...
    let handleLock = (coords, planet) => {
      ...
      if (missing) {
        ...
        div.html(`
          ...
          <hr>
          <div class="ogl-metal">M: ${
            this.formatToUnits(Math.max(0, missing[0]))}</div>
          <div class="ogl-crystal">C: ${
            this.formatToUnits(Math.max(0, missing[1]))}</div>
          <div class="ogl-deut">D: ${
            this.formatToUnits(Math.max(0, missing[2]))}</div>
          <hr>
          `);
        let deleteBtn = div.appendChild(this.createDOM('div', {
          style: 'width: 75px;',
        ...
        btn.addEventListener('click', () => {
          ...
          let link = `?page=ingame&component=fleetdispatch&galaxy=${
              splittedCoords[0]}&system=${splittedCoords[1]}&position=${
              splittedCoords[2].slice(0, -1)}&type=${type}&mission=${
              this.json.options.harvestMission}&oglMode=2`;
  ...
  highlightTarget() {
    ...
    if (document.querySelector('#galaxy_input').value == coords[0] &&
      ...
      let target = document.querySelectorAll(
          '#galaxytable tbody tr')[parseInt(coords[2]) - 1];
  ...
  checkInputs() {
    ...
    if (this.page == 'fleetdispatch') {
      document.querySelectorAll('form[name="shipsChosen"] input')
    ...
    document.querySelectorAll('input.ogl-formatInput').forEach((input) => {
      ...
      if (input.value)
        input.value = parseInt(this.removeNumSeparator(input.value, true))
                          .toLocaleString('de-DE');
    });
  }
  ...
  formatToUnits(value) {
    ...
    return (
        (neg ? '-' : '') +
        (value / Math.pow(10, order * 3)).toFixed(precision) + suffix);
  }
  ...
  // Depracated -- needs to be remove in next release (currently used to
  // transition)
  production(id, lvl, total, coords) {
    ...
    if (this.commander) {
      ...
      let energyMet =
          this.consumption(1, mines.metal.replace('(', '').replace(')', ''));
      let energyCri =
          this.consumption(2, mines.crystal.replace('(', '').replace(')', ''));
      let energyDeut = this.consumption(
          3, mines.deuterium.replace('(', '').replace(')', ''));
    ...
    let maxCrawlerCount = (Number(mines.metal) + Number(mines.crystal) +
                           Number(mines.deuterium)) *
        8;
    if (this.geologist) {
      maxCrawlerCount = maxCrawlerCount * 1.1;
    }

    let crawlerBonus = Math.min(mines.crawlers, maxCrawlerCount) *
    ...
    // Useless since commender should be taking over
    if (this.allOfficers) {
  ...
  research(id, lvl, labs, technocrat, explorer) {
    ...
    if (id == 199) {
      ...
      cost[0] = 4000 * Math.pow(1.75, lvl - 1);
      cost[1] = 8000 * Math.pow(1.75, lvl - 1);
      cost[2] = 4000 * Math.pow(1.75, lvl - 1);
    }

    let time = (cost[0] + cost[1]) / (1000 * labs) / this.json.speed /
        this.json.researchDivisor;
  ...
  keyboardActions() {
    let closeDialog = () => {
      ...
      let btn = document.querySelector('.ogl-dialog .btn_blue.save') ||
    ...
    let actionSkip = () => {
      ...
      if (this.current.isMoon) {
        cp = new URL(nextElement.querySelector('.moonlink').href)
                 .searchParams.get('cp');
      } else {
        cp = new URL(nextElement.querySelector('.planetlink').href)
                 .searchParams.get('cp');
      }
  ...
  tooltip(sender, content, autoHide, side, timer) {
    ...
    if (!tooltip) {
      ...
      close =
          tooltip.appendChild(this.createDOM('a', {class: 'close-tooltip'}));
    ...
    this.tooltipTimer =
        setTimeout(() => tooltip.classList.add('ogl-active'), timer);
  ...
  popup(header, content) {
    ...
    if (!dialog) {
      dialog =
          overlay.appendChild(this.createDOM('div', {class: 'ogl-dialog'}));
      let close =
          dialog.appendChild(this.createDOM('div', {class: 'close-tooltip'}));
      close.addEventListener('click', () => {
        if (this.json.welcome) {
          ...
          if (this.playerClass == 0) {
            window.location.href = `https://s${this.universe}-${
                this.gameLang}.ogame.gameforge.com/game/index.php?page=ingame&component=characterclassselection`;
          } else {
            window.location.href = `https://s${this.universe}-${
                this.gameLang}.ogame.gameforge.com/game/index.php?page=ingame&component=overview`;
          }
    ...
    let top = dialog.querySelector('header') ||
        dialog.appendChild(this.createDOM('header'));
    let body = dialog.querySelector('.ogl-dialogContent') ||
  ...
  trashsimTooltip(container, fleetinfo) {
    ...
    let btn =
        ctn.appendChild(this.createDOM('div', {class: 'ogk-trashsim tooltip'}));
      ...
      let addTooltip = () => {
        ...
        positions.forEach((position) => {
          if (!position.classList.contains('ogi-ready')) {
            ...
            if (countDiv) {
              let count = countDiv.getAttribute('title') ||
                  countDiv.getAttribute('data-title');
              count = count.split(':')[1].trim();
              countDiv.html(`<span class="ogi-highscore-ships">(${
                  count})</span> ${countDiv.innerText}`);
            }
            ...
            if (mail) {
              ...
              dataHelper.getPlayer(id).then((p) => {
                if (p.status && p.status.includes('v')) {
                  playerDiv.html(`<span class="status_abbr_vacation">${
                      p.name}</span> ${status}`);
                }
      ...
      initHighscoreContent = () => {
        ...
        if (userWantsFocus) {
          if ($('#position' + searchPosition).length > 0) {
            let top =
                Math.max(0, $('#position' + searchPosition).offset().top - 200);
            scrollTo(0, top);
          }
        }
        $('.changeSite').change(function() {
          ...
          $('#stat_list_content')
              .html(
                  '<div class="ajaxLoad">' + LocalizationStrings.loading +
                  '</div>');
          ajaxCall(
              highscoreContentUrl + '&category=' + currentCategory +
                  '&type=' + currentType + '&site=' + value,
              '#stat_list_content', initHighscoreContent);
        });
  ...
  utilities() {
      ...
      for (let id in data) {
        let infos = `
            <div class="ogl-fleetInfo">
                ...
                <hr>
                <div><span>Fret</span>${
            data[id].cargoCapacity.toLocaleString('de-DE')}</div>
                <div><span>${this.getTranslatedText(19)}</span>${
            data[id].speed.toLocaleString('de-DE')}</div>
                <div><span>Conso</span>${
            data[id].fuelConsumption.toLocaleString('de-DE')}</div>
            </div>`;

        let ship =
            document.querySelector(`.technology[data-technology="${id}"]`);
    ...
    if (this.page == 'movement') {
      ...
      document.querySelectorAll('.fleetDetails').forEach((fleet) => {
        ...
        let originCoords =
            fleet.querySelector('.originCoords').innerText.slice(1, -1);
        this.planetList.forEach((planet) => {
          let coords = planet.querySelector('.planet-koords').textContent;
        ...
        fleet.appendChild(this.createDOM(
            'a', {class: `ogl-mission-icon ogl-mission-${type}`}));
        ...
        let destination =
            fleet.querySelector('.destinationCoords a').innerText.slice(1, -1);
        ...
        details.appendChild(this.createDOM(
            'div', {class: 'ogk-ships-count'},
            this.formatToUnits(fleetCount) + ' ships'));

        if (!fleet.querySelector('.reversal')) return;

        let back = fleet.querySelector('.reversal a').title ||
            fleet.querySelector('.reversal a').getAttribute('data-title');
        let splitted = back.split('|')[1]
                           .replace('<br>', '/')
                           .replace(/:|\./g, '/')
                           .split('/');
        ...
        let lastTimer = new Date(
                            backDate.year, backDate.month, backDate.day,
                            backDate.h, backDate.m, backDate.s)
                            .getTime();

        let content =
            details.appendChild(this.createDOM('div', {class: 'ogl-date'}));
        let date;

        let updateTimer = () => {
          lastTimer += 1000;
          date = new Date(lastTimer);
          content.html(getFormatedDate(
              date.getTime(), '<strong> [G]:[i]:[s] </strong> - [d].[m]'));
        };
      ...
      if (lastFleetBtn) {
        ...
        let backlast = document.querySelector('.fleetStatus')
                           .appendChild(this.createDOM(
                               'span', {class: 'reload ogl-backLast'}, `
  ...
  settings() {
    ...
    dataDiv.appendChild(
        this.createDOM('div', {class: 'ogi-checkbox'}, `Bug reporting 
        <a target="_blank" href="https://discord.gg/9aMdQgk"> Here </span>`));
    dataDiv.appendChild(
        this.createDOM('div', {class: 'ogi-checkbox'}, `Feature request  
        <a target="_blank" href="https://discord.gg/9aMdQgk"> Here </span>`));

    dataDiv.appendChild(this.createDOM(
        'p', {
          style:
    ...
    if (this.json.timezoneDiff != 0) {
      let spanZone = dataDiv.appendChild(this.createDOM(
          'span', {
            style:
      ...
      let timeZoneCheck =
          spanZone.appendChild(this.createDOM('input', {type: 'checkbox'}));
    ...
    let fileHandler = dataBtns.appendChild(this.createDOM('input', {
      id: 'file',
      name: 'file',
      class: 'inputfile',
      type: 'file',
    ...
    dataBtns.appendChild(
      ...
      let span = settingDiv.appendChild(this.createDOM(
          'span', {
            style:
      ...
      let missionDiv =
          span.appendChild(this.createDOM('div', {style: 'display:flex'}));
      let none = missionDiv.appendChild(this.createDOM('a', {
        class: 'icon icon_against',
        style: 'margin-top: 2px;margin-right: 5px;',
      }));
      let own3 = missionDiv.appendChild(this.createDOM('div', {
        class: `ogl-mission-icon ogl-mission-3 ${
            this.json.options.harvestMission == 3 ? 'ogl-active' : ''}`,
      }));
      let own4 = missionDiv.appendChild(this.createDOM('div', {
      ...
      span = settingDiv.appendChild(this.createDOM(
          'span', {
            style:
                'display: flex;justify-content: space-between; align-items: center;',
          },
      ...
      missionDiv =
          span.appendChild(this.createDOM('div', {style: 'display:flex'}));
      none = missionDiv.appendChild(this.createDOM('a', {
        class: 'icon icon_against',
        style: 'margin-top: 2px;margin-right: 5px;',
      }));
      let other3 = missionDiv.appendChild(this.createDOM('div', {
        class: `ogl-mission-icon ogl-mission-3 ${
            this.json.options.foreignMission == 3 ? 'ogl-active' : ''}`,
      }));
      let other1 = missionDiv.appendChild(this.createDOM('div', {
      ...
      let expe15 = missionDiv.appendChild(this.createDOM('div', {
        class: `ogl-mission-icon ogl-mission-15 ${
            this.json.options.expeditionMission == 15 ? 'ogl-active' : ''}`,
      }));
      let expe6 = missionDiv.appendChild(this.createDOM('div', {
      ...
      let timerCheck =
          span.appendChild(this.createDOM('input', {type: 'checkbox'}));
      ...
      let rvalInput = span.appendChild(this.createDOM('input', {
        type: 'text',
