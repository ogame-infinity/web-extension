...
class OGInfinity {
  ...
  init() {
    ...
    this.json.options.spyTableEnable =
      this.json.options.spyTableEnable !== false;
    this.json.options.spyTableAppend =
      this.json.options.spyTableAppend !== false;
    this.json.options.compactViewEnable =
      this.json.options.compactViewEnable !== false;
    this.json.options.autoDeleteEnable =
      this.json.options.autoDeleteEnable === true;
      ...
      technologyDetails.show = function (technologyId) {
        ...
        $.ajax({
          ...
          if (
            ...
            const lvlFromTo = titleDiv.appendChild(
              that.createDOM('div', {}, '')
            );
            titleDiv.appendChild(that.createDOM('div', {}, 'Missing'));
            const helpNode = document
            ...
            if (that.page == 'research') {
              ...
              const lvls = that.getRobotsNanites(
                technologyId,
                baseLvl,
                initTime
              );
        ...
        if (
          ...
          gone.forEach((movement) => {
            ...
            if (this.json.myRes[movement.dest]) {
              ...
              this.planetList.forEach((planet) => {
                const coords =
                  planet.querySelector('.planet-koords').textContent;
  ...
  addGalaxyMarkers() {
    document
      ...
      .forEach((element, index) => {
        const moon = !!element.parentNode.querySelector(
          '.cellMoon .tooltipRel'
        );
      ...
      .forEach((row, index) => {
        ...
        if (playerDiv) {
          ...
          if (
            ...
            ptreJSON[coords].mv = !!row.querySelector(
              'span[class*="vacation"]'
            );
      ...
      if (eventList) {
        ...
        document.querySelector('.event_list').appendChild(
          this.createDOM(
            ...
            `Flying: ${this.commander ? '' : '≈'}<span class="${color}">${(
              (fleetCount / total) *
              100
            ).toFixed(0)}%</span>`
          )
  ...
  sideOptions() {
    const harvestOptions = this.createDOM('div', {
      class: 'ogl-harvestOptions',
    });
    const container =
  ...
  tabs(titles, small) {
    ...
    const header = body.appendChild(
      this.createDOM('div', { class: 'ogl-tabs' })
    );
    const tabs = [];
    let first;
  ...
  betterFleetDispatcher() {
    if (
      ...
      const needCargo = (fret) => {
        ...
        const deut = Number(
          deutFiller.value.split(LocalizationStrings.thousandSeperator).join('')
        );
      ...
      this.overwriteFleetDispatcher('stopLoading', false, () => {
        ...
        if (auxAjaxFailed) {
          ...
          missionsDiv.html(
            `<span>${fleetDispatcher.targetPlayerRankIcon}<span class="status_abbr_${fleetDispatcher.targetPlayerColorClass}">${fleetDispatcher.targetPlayerName}</span>` +
              '</span>'
          );
      ...
      let update = (submit) => {
        ...
        if (
          this.current.coords ==
          `${fleetDispatcher.targetPlanet.galaxy}:${fleetDispatcher.targetPlanet.system}:${fleetDispatcher.targetPlanet.position}`
        ) {
          if (
            this.current.isMoon &&
            fleetDispatcher.targetPlanet.type ==
      ...
      const crystalLeft = crystalBtn.appendChild(
        this.createDOM('span', {}, '-')
      );
      const crystalReal = crystalBtn.appendChild(
        this.createDOM('span', { class: 'ogk-real-cargo ogk-crystal' }, '-')
    ...
    const renderTagetList = () => {
      ...
      tabG.addEventListener('click', () => {
        if (this.json.targetTabs.g == -1) {
          ...
          galaxy = false;
          this.saveData();
          const active = header.querySelector(
            '.ogl-tab[data-galaxy].ogl-active'
          );
      ...
      tabS.addEventListener('click', () => {
        if (this.json.targetTabs.s == -1) {
          ...
          this.saveData();
          const active = header.querySelector(
            '.ogl-tab[data-system].ogl-active'
          );
          if (active) active.classList.remove('ogl-active');
  ...
  spyTable() {
    if (this.page == 'fleetdispatch' && this.mode == 4) {
      const link = `https://${window.location.host}${window.location.pathname}?page=messages`;
    ...
    if (document.querySelector('#tabs-nfFleets.ui-state-active')) {
      messages = document.querySelectorAll(
        `#${document
          ...
          .getAttribute('aria-controls')} .msg`
      );
    } else {
      messages = document.querySelectorAll(
        `#${document
  ...
  sortTable(arr) {
    ...
    arr.forEach(async (report, index) => {
      ...
      const name = line.appendChild(
        this.createDOM('td', { class: 'ogl-name' })
      );
      const status = {
        '': 'status_abbr_active',
  ...
  formatToUnits(value) {
    ...
    return (
      (neg ? '-' : '') + (value / 10 ** (order * 3)).toFixed(precision) + suffix
    );
  }
  ...
  production(id, lvl, total, coords) {
    ...
    const miner =
      (this.playerClass == PLAYER_CLASS_MINER ? 1 : 0) * 0.25 * prod;
    let totalProd =
  ...
  building(id, lvl, robotic, nanite) {
    ...
    if (id == 15 || id == 36 || id == 43 || id == 42 || id == 41) {
      time = ((cost[0] + cost[1]) / 2500) * (1 / (1 + robotic)) * 0.5 ** nanite;
    }
  ...
  settings() {
    ...
    const srvDatas = dataDiv.appendChild(
      this.createDOM(
        ...
        `Server Settings : <br/>Top Score : ${this.formatToUnits(
          this.json.topScore
        )}<br/>Eco Speed : ${this.json.speed}<br/>Fleet Speed War: ${
          this.json.speedFleetWar
        }<br/>Fleet Speed Peaceful: ${
          this.json.speedFleetPeaceful
        }<br/>Fleet Speed Holding: ${this.json.speedFleetHolding}`
      )
    );
    const srvDatasBtn = this.createDOM(
      'button',
      { class: 'btn_blue' },
      'Update'
    );
