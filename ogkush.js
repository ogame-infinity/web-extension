...
let UNIVERSVIEW_LANGS = ["en", "cs", "es", "fr", "de", "da", "hr", "it", "hu", "nl", "pl", "pt", "ro", "ru", "sk", "sv", "tr", "el", "zh", "ko"];
...
class OGInfinity {
    ...
    if (this.page == "research" || this.page == "supplies" || this.page == "facilities" || this.page == "shipyard" || this.page == "defenses") {
      ...
      let updateResearchDetails = (technoId, baselvl, tolvl) => {
        ...
        if (that.page == "supplies") {
          ...
          let prodDiv =
            (document.querySelector(".narrow") && document.querySelector(".ogk-production")) || document.querySelector(".narrow").appendChild(that.createDOM("li", { class: "ogk-production" }));
          ...
          if (consDiv) {
            ...
            prodDiv.html(
              `<strong>Production :</strong> <span class="value">${parseInt(baseProd).toLocaleString("de-DE")} <span class="bonus"> (+${parseInt(baseProd - currentProd).toLocaleString(
                "de-DE"
              )})</span></span>`
            );
          }
  ...
  initUnionCombat(union) {
    if (this.unionInterval) {
      ...
      this.delayDiv2 = document.querySelector("#naviActions").appendChild(this.createDOM("div", { class: "ogk-delay" }));
  ...
  topBarUtilities() {
    ...
    bar.appendChild(this.createDOM("li", {}, `<a href="https://trashsim.universeview.be/${this.univerviewLang}" target="_blank">Trash</a>`));
  ...
  eventBox() {
    ...
    let changeSpy = () => {
      document.querySelectorAll("#eventContent .sendProbe a").forEach((elem) => {
        ...
        elem.setAttribute(
          "onClick",
          `sendShipsWithPopup(6,${params.get("galaxy")},${params.get("system")},${params.get("position")},${params.get("planetType")},${this.json.spyProbes}); return false;`
        );
      });
    };
    let addHover = () => {
  ...
  scan() {
    ...
    document.querySelectorAll("#galaxycomponent .galaxyRow.ctContentRow").forEach((row, index) => {
      ...
      if (playerDiv) {
        ...
        if (this.json.options.ptreTK && playerId > -1 && (this.json.sideStalk.indexOf(playerId) > -1 || this.markedPlayers.indexOf(playerId) > -1)) {
        ...
        if (sided.length != 0) {
          ...
          this.activities[coords] = this.getActivity(row);
          changes.push({ id: sided[0].parentElement.getAttribute("player-id"), moon: row.querySelector(".cellMoon .tooltipRel") ? true : false, coords: coords, deleted: true });
        }
      }
    });
  ...
  async ptreActivityUpdate(ptreJSON, systemCoords) {
    for (const coords of Object.keys(ptreJSON)) {
      ...
      let mainPlanet = pl.planets.find((obj) => obj.isMain === true);
  ...
  welcome() {
    ...
    let shortcutsDiv = container.appendChild(
      this.createDOM("p", { class: "ogk-tips friendly", style: "display: flex;justify-content: space-between;font-size: revert" }, "Oh, and here are some quick tips: ")
    );
        ...
        if (keyNode) {
          ...
          if (key.startsWith("sr")) {
            ...
            link.addEventListener("click", () => {
              ...
              window.open(`https://trashsim.universeview.be/${this.univerviewLang}?SR_KEY=${key}#prefill=${base64}`, "_blank");
            });
    ...
    profits
      ...
      .forEach((elem, index) => {
        let span = div.appendChild(
          this.createDOM("span", { style: `height: ${elem.profit == 0 ? 5 : Math.max(10, (Math.abs(elem.profit) / max) * 60)}px`, class: elem.profit >= 0 ? "" : "ogk-minus" })
        );
        spans.push(span);
        span.addEventListener("click", () => {
          spans.forEach((elem) => elem.classList.remove("ogk-active"));
          span.classList.add("ogk-active");
          let contentHtml = `<strong>${getFormatedDate(elem.date.getTime(), "[d].[m].[y]")}</strong> <span class="${elem.profit >= 0 ? "undermark" : "overmark"}">${
            elem.profit >= 0 ? " +" : " -"
  ...
  generalStats(player) {
    ...
    detailRank.html(
      `\n          <div><div class="ogl-ecoIcon"></div>${dotted(player.economy.score)} <small>pts</small><span class="ogl-ranking">#${
        player.economy.position
      } </span></div>\n          <div><div class="ogl-techIcon"></div>${dotted(player.research.score)} <small>pts</small><span class="ogl-ranking">#${
  ...
  async ptreAction(frame, player) {
    ...
    this.getJSON(`https://ptre.chez.gg/scripts/oglight_get_player_infos.php?team_key=${this.json.options.ptreTK}&pseudo=${cleanPlayerName}&player_id=${player.id}&input_frame=${frame}`, (result) => {
      if (result.code == 1) {
        ...
        container.innerHTML = `\n                            <h3>${
          this.gameLang == "fr" ? "Meilleur Rapport" : "Best Report"
        ...
        document.querySelectorAll(`div[id=${id}] li.msg`).forEach((msg) => {
            ...
            if (this.json.expeditions[id].result == "Unknown") {
              msg.querySelector(".ogl-unknown-warning") ||
                msg
                  ...
                  .appendChild(this.createDOM("div", { class: "ogl-unknown-warning" }, "Unknown expedition message... <a href='https://discord.gg/8Y4SWup'> Help me find them all</a>"));
            } else if (this.json.expeditions[id].busy) {
              msg.querySelector(".ogl-warning") ||
                msg.querySelector(".msg_actions").appendChild(this.createDOM("a", { class: "ogl-warning tooltip", "data-title": "Warning : expedition position is getting weak..." }));
            }
          ...
          dataHelper.getExpeditionType(textContent).then((type) => {
            ...
            if (this.json.expeditions[id].result == "Unknown") {
              ...
              msg
                .querySelector(".msg_actions")
                .appendChild(this.createDOM("a", { class: "ogl-warning tooltipRight ogl-tooltipReady ogl-tooltipInit", "data-title": "Warning : Expedition position is weak..." }));
            }
            msg.classList.add("ogk-" + this.json.expeditions[id].result.toLowerCase());
            this.saveData();
          });
    ...
    content.appendChild(
      this.tabs({
        ...
        All: () => {
          ...
          let contentHtml = `<strong>${getFormatedDate(this.dateStrToDate(minDate).getTime(), "[d].[m].[y]")}</strong> <span class="${total > 0 ? "undermark" : "overmark"}">${
            total > 0 ? " +" : " -"
  ...
  generateMMORPGLink(playerid) {
    let lang = ["fr", "de", "en", "es", "pl", "it", "ru", "ar", "mx", "tr", "fi", "tw", "gr", "br", "nl", "hr", "sk", "cz", "ro", "us", "pt", "dk", "no", "se", "si", "hu", "jp", "ba"].indexOf(
      this.gameLang
    );
  ...
  minesStats() {
    ...
    this.planetList.forEach((planet) => {
      ...
      td.appendChild(
        this.createDOM("div", { class: "ogl-crystal" }, this.formatToUnits((this.json.myMines[coords].crystalProd || this.production(2, this.json.myMines[coords].crystal, true)) * 24 * 7))
      );
      if (current) td.classList.add("ogl-current");
      td = deutRow.appendChild(this.createDOM("td"));
      ...
      td.appendChild(
        this.createDOM("div", { class: "ogl-deut" }, this.formatToUnits((this.json.myMines[coords].deuteriumProd || this.production(3, this.json.myMines[coords].deuterium, true)) * 24 * 7))
      );
      if (current) td.classList.add("ogl-current");
      td = nrjRow.appendChild(this.createDOM("td"));
      let diff = this.json.myMines[coords].energy;
      td.appendChild(this.createDOM("div", { class: "ogl-energy " + (diff >= 0 ? "undermark" : "overmark"), style: "font-size: 18px;" }, this.formatToUnits(diff)));
      if (current) td.classList.add("ogl-current");
    });
    mlvl = mlvl / sum;
    clvl = clvl / sum;
  ...
  buildDispatcherUI() {
    ...
    if (unions.length != 0) {
      let unionsBtn = coords.appendChild(
        this.createDOM("div", { class: "ogl-union-btn" }, '<img src="https://gf3.geo.gfsrv.net/cdn56/2ff25995f98351834db4b5aa048c68.gif" height="16" width="16"></img>')
      );
      unionsBtn.addEventListener("click", () => {
        ...
        for (let i in unions) {
          ...
          let unionDiv = container.appendChild(
            this.createDOM("div", { class: "ogl-quickPlanet" }, `${union.name} [${union.galaxy}:${union.system}:${union.planet}] ${union.planettype == 1 ? "P" : "M"}`)
          );
  ...
  betterFleetDispatcher() {
    if (this.page == "fleetdispatch" && document.querySelector("#civilships") && fleetDispatcher.shipsOnPlanet.length != 0) {
      ...
      let coords = destination.appendChild(this.createDOM("div", { class: "ogl-coords" }));
      let warning = coords.appendChild(this.createDOM("a", { class: "ogl-warning tooltipRight", "data-title": "Error : current planet/moon..." }));
      let galaxyInput = coords.appendChild(this.createDOM("input", { id: "galaxyInput", type: "text", pattern: "[0-9]*", value: fleetDispatcher.targetPlanet.galaxy }));
      let systemInput = coords.appendChild(this.createDOM("input", { id: "systemInput", type: "text", pattern: "[0-9]*", value: fleetDispatcher.targetPlanet.system }));
      let positionInput = coords.appendChild(this.createDOM("input", { id: "positionInput", type: "text", pattern: "[0-9]*", value: fleetDispatcher.targetPlanet.position }));
      let planet = coords.appendChild(this.createDOM("a", { class: "ogl-planet-icon" }));
      let moon = coords.appendChild(this.createDOM("a", { class: "ogl-moon-icon" }));
      let debris = coords.appendChild(this.createDOM("a", { class: "ogl-debris-icon" }));
      planet.addEventListener("click", () => {
      ...
      this.overwriteFleetDispatcher("stopLoading", false, () => {
        ...
        if (auxAjaxFailed) {
          ...
          missionsDiv.html(
            "<span>" + fleetDispatcher.targetPlayerRankIcon + `<span class="status_abbr_${fleetDispatcher.targetPlayerColorClass}">${fleetDispatcher.targetPlayerName}</span>` + "</span>"
          );
      ...
      let update = (submit) => {
        ...
        interval = setInterval(() => {
          ...
          returnDiv.html(
            getFormatedDate(new Date(serverTime).getTime() + 2 * fleetDispatcher.getDuration() * 1e3 + (fleetDispatcher.mission == 15 ? 3600 : 0) * 1e3, "<strong> [G]:[i]:[s] </strong> - [d].[m]")
          );
        }, 100);
        onResChange(2);
        onResChange(1);
  ...
  harvest() {
    ...
    this.planetList.forEach((planet) => {
      ...
      if (this.current.coords != coords.join(":") || this.current.isMoon) {
        let btn = planet.querySelector(".planetlink .planetPic").addEventListener("click", (event) => btnAction(event, coords, 1));
      }
  ...
  spyTable() {
    ...
    messages.forEach((msg) => {
      ...
      if (this.json.options.ptreTK && msg.querySelector(".espionageDefText")) {
        let id = msg.getAttribute("data-msg-id");
        let tmpHTML = this.createDOM("div", {}, msg.querySelector("span.player").getAttribute("title") || msg.querySelector("span.player").getAttribute("data-title"));
    ...
    if (Object.keys(ptreJSON).length > 0) {
      fetch("https://ptre.chez.gg/scripts/oglight_import_player_activity.php", { method: "POST", body: JSON.stringify(ptreJSON) })
        .then((response) => response.json())
        .then((data) => {
          if (data.code != 1) {
  ...
  sortTable(arr) {
    ...
    arr.forEach(async (report, index) => {
      ...
      simulateBtn.addEventListener("click", () => {
        ...
        let json = { 0: [{ class: this.playerClass, research: apiTechData, planet: { galaxy: coords[0], system: coords[1], position: coords[2] } }] };
        let base64 = btoa(JSON.stringify(json));
        window.open(`https://trashsim.universeview.be/${this.univerviewLang}?SR_KEY=${report.apiKey}#prefill=${base64}`, "_blank");
      });
  ...
  sideLock(add) {
    ...
    let handleLock = (coords, planet) => {
      ...
      if (missing) {
        ...
        btn.addEventListener("click", () => {
          let type = moon ? 3 : 1;
          let link = `?page=ingame&component=fleetdispatch&galaxy=${splittedCoords[0]}&system=${splittedCoords[1]}&position=${splittedCoords[2].slice(0, -1)}&type=${type}&mission=${
  ...
  async pantrySync(pantryKey, action = 1) {
    ...
    let loadPantrySync = this.createDOM("div", { id: `ogi-pantry-sync`, class: "ogi-loader-container" });
    ...
    if (action === 0) {
      ...
      const expeSumsRequest = fetch(`https://getpantry.cloud/apiv1/pantry/${pantryKey}/basket/${this.universe}-${this.gameLang}-expeditionSums`, {
        priority: "low",
        method: "POST",
        headers: pantryHeaders,
      ...
      const combatSumsRequest = fetch(`https://getpantry.cloud/apiv1/pantry/${pantryKey}/basket/${this.universe}-${this.gameLang}-combatSums`, {
        priority: "low",
        method: "POST",
        headers: pantryHeaders,
      ...
      await Promise.allSettled([targetsRequest, otherSettingsRequest, expeRequest, expeSumsRequest, combatRequest, combatSumsRequest, harvestRequest]).then(async (requestsPromises) => {
        ...
        if (success) {
          this.json.expeditionSums = { ...this.json.expeditionSums, ...cloudConsolidated.expeditionSums };
          this.json.expeditions = { ...this.json.expeditions, ...cloudConsolidated.expeditions };
          this.json.combatsSums = { ...this.json.combatsSums, ...cloudConsolidated.combatsSums };
          this.json.combats = { ...this.json.combats, ...cloudConsolidated.combats };
          this.json.harvests = { ...this.json.harvests, ...cloudConsolidated.harvests };
  ...
  showToast(text, type = "info", icon = "info", title = null, duration = 3500) {
    ...
    let toastHtml = this.createDOM("div", { class: `ogi-toast ogi-toast-${type}` });
    let toastContainer = this.createDOM("div", { class: "ogi-toast-container" });
    let toastBody = this.createDOM("span", { class: "ogi-toast-body" });
    let toastLogoContainer = this.createDOM("span", { class: "ogi-toast-logo" });
  ...
  trashsimTooltip(container, fleetinfo) {
    ...
    btn.addEventListener("click", () => {
      ...
      let base64 = btoa(JSON.stringify(json));
      window.open(`https://trashsim.universeview.be/${this.univerviewLang}?#prefill=${base64}`, "_blank");
    });
  }
  betterHighscore() {
    if (this.page == "highscore") {
  ...
  utilities() {
    ...
    if (this.page == "fleetdispatch") {
      ...
      let slider = this.createDOM("div", { class: "ogl-fleetSpeed", style: "margin-top: 10px; margin-left: 10px; margin-right: 10px; display: flex; grid-column: 1/3;" });
  ...
  settings() {
    ...
    dataDiv.appendChild(this.createDOM("div", { class: "ogi-checkbox" }, `Bug reporting & contributing\n        <a target="_blank" href="https://discord.gg/9aMdQgk"> Here </span>`));
    ...
    if (this.json.timezoneDiff != 0) {
      let spanZone = dataDiv.appendChild(
        this.createDOM("span", { style: "display: flex;justify-content: space-between; align-items: center;margin-bottom: 10px" }, "Change clocks to local time zone")
      );
    ...
    dataDiv.appendChild(
      this.createDOM("h1", {}, `Data management <span style="font-weight: 100;color: white; float:right"> <strong class="${size.total > 4 ? "overmark" : "undermark"}"> ${size.total}</strong>  / 5 Mb`)
    );
    dataDiv.appendChild(this.createDOM("h1", {}, ``));
    let expeditionsBox = dataDiv.appendChild(
      this.createDOM("div", { class: "ogi-checkbox" }, `<label for="expeditions">Expeditions data</label>\n        <input type="checkbox" id="expeditions" name="expeditions">`)
    );
    ...
    let cacheBox = dataDiv.appendChild(
      this.createDOM("div", { class: "ogi-checkbox" }, `<label for="temp">Cache and Temp data</label>\n        <input type="checkbox" id="temp" name="temp" checked>`)
    );
    ...
    let dataSpan = dataDiv.appendChild(this.createDOM("span", {}, '<a href="https://getpantry.cloud/" target="_blank">Pantry</a> Key '));
    let pantryInput = dataSpan.appendChild(
      this.createDOM("input", { type: "password", class: "ogl-pantryKey tooltip", value: this.json.options.pantryKey ?? "", placeholder: "XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX" })
    );
    dataDiv.appendChild(this.createDOM("hr"));
    ...
    let ptreInput = span.appendChild(this.createDOM("input", { type: "password", class: "ogl-ptreTeamKey tooltip", value: this.json.options.ptreTK ?? "", placeholder: "TM-XXXX-XXXX-XXXX-XXXX" }));
    settingDiv.appendChild(this.createDOM("hr"));
