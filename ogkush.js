const dataHelper = (function () {
    ...
    function expedition(message) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const payload = { requestId: rid, message };
    ...
    function Get(id) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const payload = { requestId: rid, id };
    ...
    function filter(name, alliance) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const payload = { requestId: rid, name, alliance };
            window.dispatchEvent(new CustomEvent('ogi-filter', { detail: payload }));
        });
    }

    return { getExpeditionType: expedition, getPlayer: Get, filter };
}());
const dotted = (value) => parseInt(value).toLocaleString(separatorLang);
const redirect = localStorage.getItem('ogl-redirect');
if (redirect && redirect.indexOf('https') > -1) {
    localStorage.setItem('ogl-redirect', false);
    window.location.href = redirect;
}

const commaSeparator = ['en-US', 'en-GB'];
...
class OGInfinity {
    ...
    init() {
        ...
        this.json.options.dispatcher = this.json.options.dispatcher === true;
        this.json.options.sideStalkVisible = this.json.options.sideStalkVisible !== false;
        this.json.options.eventBoxExps = this.json.options.eventBoxExps !== false;
        this.json.options.eventBoxKeep = this.json.options.eventBoxKeep === true;
        this.json.options = this.json.options || {};
        this.json.options.empire = this.json.options.empire === true;
        this.json.options.targetList = this.json.options.targetList === true;
        ...
        this.json.options.activitytimers = this.json.options.activitytimers === true;
        this.json.options.planetIcons = this.json.options.planetIcons === true;
        this.json.options.disableautofetchempire = this.json.options.disableautofetchempire === true;
        this.json.options.autofetchempire = this.json.options.disableautofetchempire !== true;
        ...
        this.json.options.spyTableEnable = this.json.options.spyTableEnable !== false;
        this.json.options.spyTableAppend = this.json.options.spyTableAppend !== false;
        this.json.options.compactViewEnable = this.json.options.compactViewEnable !== false;
        this.json.options.autoDeleteEnable = this.json.options.autoDeleteEnable === true;
        ...
        this.json.options.timeZone = this.json.options.timeZone !== false;
    ...
    start() {
        ...
        if (this.page == 'fleetdispatch') {
            if (!this.json.shipNames) {
                this.json.shipNames = {};
                for (const id in fleetDispatcher.fleetHelper.shipsData) {
                    const { name } = fleetDispatcher.fleetHelper.shipsData[id];
        ...
        const storage = this.getLocalStorageSize();
                    ...
                    success(data) {
                        if (!data.response) {
                            ...
                            if (typeof data.newAjaxToken !== 'undefined') {
                                ...
                                const page = rawURL.searchParams.get('component')
                  || this.rawURL.searchParams.get('page');
                                if (page === 'galaxy') {
                                    const phalanxElements = document.querySelectorAll(
    ...
    timeZone() {
        ...
        if (this.json.options.timeZone) {
            timeDiff += this.json.timezoneDiff * 1e3;
        }
        const hourDiff = this.json.timezoneDiff / 60 / 60;
        hourDiff != 0
      && $('.ogk-ping').prepend(
          this.createDOM(
              'span',
    ...
    overviewDates() {
        document
            ...
            .forEach((timer) => {
                ...
                const newDate = new Date(Date.now() + timeLeft);
    ...
    minesLevel() {
        const coords = this.current.coords + (this.current.isMoon ? 'M' : 'P');
        if (this.page == 'overview') {
            let maxTemp;
            const splits = textContent[3].split('°C').join('').split(' ');
        ...
        let metalProd; let crystalProd; let
            deuteriumProd;
        tooltips.forEach((elem, i) => {
            const tooltip = this.createDOM('div', {});
            tooltip.html(elem);
            const lines = tooltip.querySelectorAll('tr');
        ...
        this.planetList.forEach((planet) => {
          ...
          || '?';
            ...
            const div = this.createDOM('div', { class: 'ogl-mines' });
    ...
    technoDetail() {
      ...
      || this.page == 'lfbuildings'
            ...
            const currentEnergy = this.removeNumSeparator(
            ...
            const technocrat = !!document.querySelector('.technocrat.on');
            let labs = 0;
            const that = this;
            const updateResearchDetails = (technoId, baselvl, tolvl) => {
                const durationDiv = document.querySelector('.build_duration');
                const timeDiv = document.querySelector('.build_duration time');
                const timeSumDiv = durationDiv.querySelector('.build_duration .ogk-sum')
          || durationDiv.appendChild(that.createDOM('time', { class: 'ogk-sum' }));
                const resSum = [0, 0, 0, 0];
                let timeSum = 0;
            ...
            || document
                ...
                .appendChild(that.createDOM('li', { class: 'ogk-production' }));
                    ...
                    if (consDiv) {
                        ...
                        const pos = this.current.coords.split(':')[2];
                        const currentProd = that.minesProduction(
                            technoId,
                            baselvl - 1,
                            pos,
                            temp,
                        );
                        const baseProd = that.minesProduction(technoId, tolvl, pos, temp);
                        const baseCons = that.consumption(technoId, baselvl - 1);
                        const currentCons = that.consumption(technoId, tolvl);
                        const diff = currentEnergy - (currentCons - baseCons);
                        consDiv.html(
                    ...
                    if (energyDiv) {
                        const currentProd = that.production(technoId, baselvl - 1, false);
                        const baseProd = that.production(technoId, tolvl, false);
                        energyDiv.html(
                ...
                const missing = [];
                if (techno.cost[0] != 0) {
                    const metal = document.querySelector('.costs .metal');
                ...
                if (techno.cost[1] != 0) {
                    const crystal = document.querySelector('.costs .crystal');
                ...
                if (techno.cost[2] != 0) {
                    const deuterium = document.querySelector('.costs .deuterium');
                ...
                if (techno.cost[3] != 0) {
                    const energy = document.querySelector('.costs .energy');
                    if (energy) {
                        ...
                        const tooltip = document.querySelector('#energy_box').getAttribute('title')
              || document.querySelector('#energy_box').getAttribute('data-title');
                        const div = that.createDOM('div');
                        div.html(tooltip);
                        const prod = div.querySelectorAll('span')[1].innerText.substring(1);
                ...
                lockListener = () => {
                    const coords = that.current.coords + (that.current.isMoon ? 'M' : 'P');
            ...
            technologyDetails.show = function (technologyId) {
                ...
                const elemTechnologyDetailsWrapper = $('#technologydetails_wrapper');
                const elemTechnologyDetailsContent = $('#technologydetails_content');
                const elemTechnologyDetails = $('#technologydetails');
                elemTechnologyDetailsWrapper.toggleClass('slide-up', true);
                elemTechnologyDetailsWrapper.toggleClass('slide-down', false);
                const locationIndicator = elemTechnologyDetailsContent.ogameLoadingIndicator();
                ...
                $.ajax({
                    ...
                    const anchor = $('header[data-anchor=technologyDetails]');
                    ...
                    const costDiv = document.querySelector('.costs');
                    const titleDiv = costDiv.appendChild(
                        that.createDOM('div', { class: 'ogk-titles' }),
                    );
                    const tree = document.querySelector('.technology_tree');
                    const clone = tree.cloneNode(true);
                    tree.style.display = 'none';
                    clone.innerText = '';
                    document.querySelector('.description').appendChild(clone);
                    const timeDiv = document.querySelector('.build_duration time');
                    const baseTime = getTimeFromString(timeDiv.getAttribute('datetime'));
                    if (
                        [
                        ...
                        const helpNode = document
                            .querySelector('.txt_box .details')
                            .cloneNode(true);
                        infoDiv.appendChild(helpNode);
                        const input = document.querySelector('.build_amount input');
                        const updateShipDetails = (value) => {
                            const missing = [];
                            resDivs.forEach((div, index) => {
                                if (!div) return;
                                const tot = value * baseCost[index];
                                const min = Math.min(0, currentRes[index] - tot);
                                missing[index] = min;
                            ...
                            if (technologyId == 212) {
                                const diff = Number(currentEnergy) + value * base;
                                energyDiv.html(
              ...
              && document
                  ...
                  .addEventListener('click', () => {
                      updateShipDetails(Number(input.getAttribute('max')));
                        ...
                        let tolvl = baseLvl;
                        const lvl = titleDiv.appendChild(
                            that.createDOM('div', {}, `Lvl <strong>${baseLvl}</strong>`),
                        );
                        const lvlFromTo = titleDiv.appendChild(that.createDOM('div', {}, ''));
                        titleDiv.appendChild(that.createDOM('div', {}, 'Missing'));
                        const helpNode = document
                            .querySelector('.txt_box .details')
                            .cloneNode(true);
    ...
    onFleetSent(callback) {
        FleetDispatcher.prototype.submitFleet2 = function (force) {
            ...
            this.startLoading();
            $.post(this.sendFleetUrl, params, (response) => {
    ...
    keepOnPlanetDialog(coords, btn) {
        ...
        const container = this.createDOM('div');
        if (coords) {
        ...
        const box = this.createDOM('div', { class: 'ogk-keep-dialog' });
        box.appendChild(this.createDOM('h1', {}, 'Resources to keep on planets'));
        const prod = box.appendChild(
        ...
        const metInput = prod.appendChild(
        ...
        const criInput = prod.appendChild(
            this.createDOM('input', {
        ...
        const deutInput = prod.appendChild(
            this.createDOM('input', {
        ...
        btn.addEventListener('click', () => {
            ...
            inputs.forEach((input) => {
                const id = Number(input.getAttribute('data'));
                const amount = this.removeNumSeparator(input.value);
                if (amount > 0) {
                    kept[id] = amount;
                }
            });
      ...
      && fleetDispatcher.shipsOnPlanet.length != 0
            ...
            this.onFleetSent(() => {
                ...
                let coords = `${document.querySelector('#galaxy').value
                    ...
                    pos}`;
                const fuel = fleetDispatcher.getConsumption();
                const dateStr = getFormatedDate(new Date().getTime(), '[d].[m].[y]');
          ...
          == fleetDispatcher.fleetHelper.PLANETTYPE_PLANET
            ...
            $('.allornonewrap .select-most').on('click', () => {
                fleetDispatcher.shipsOnPlanet.forEach((ship) => {
                    const kept = this.json.options.kept[
                ...
                const elem = document.querySelector('.ogl-planet-icon.ogl-active')
          || document.querySelector('.ogl-moon-icon.ogl-active')
          || document.querySelector('.ogl-debris-icon.ogl-active');
                if (elem) elem.click();
            });
            const svgButtons = this.createDOM('div', { class: 'ogl-dispatch-icons' });
            $('#civil').append(svgButtons);
            const svg = svgButtons.appendChild(
                this.createDOM(
                    'div',
            ...
            svg.addEventListener('mouseover', () => {
                document
                    ...
                    .forEach((elem) => {
                        ...
                        const id = elem.getAttribute('data-technology');
                        elem.appendChild(
                            this.createDOM(
                                'span',
                                { class: 'ogi-speed' },
            ...
            const plusSvg = svgButtons.appendChild(
                this.createDOM(
                    'div',
                    { class: 'ogi-plus-icon' },
    ...
    updateServerSettings() {
        ...
        const settingsUrl = `https://s${this.universe}-${this.gameLang}.ogame.gameforge.com/api/serverData.xml`;
    ...
    topBarUtilities() {
        const boardlink = `https://board.${this.gameLang}.ogame.gameforge.com/`;
        ...
        const ping = window.performance.timing.domLoading
    ...
    eventBox() {
                ...
                && movement.metal + movement.crystal + movement.deuterium != 0)
                        ...
                        if (this.json.myRes[movement.dest]) {
                            ...
                            if (found) {
                                ...
                                const tot = movement.metal + movement.crystal + movement.deuterium;
        ...
        const addOptions = () => {
            ...
            const div = header.appendChild(this.createDOM('div', {}));
            div.appendChild(this.createDOM('span', {}, 'Keep'));
            const keep = div.appendChild(this.createDOM('input', { type: 'checkbox' }));
            if (this.json.options.eventBoxKeep) keep.checked = true;
            div.appendChild(this.createDOM('span', {}, 'Expeditions'));
            const exps = div.appendChild(this.createDOM('input', { type: 'checkbox' }));
        ...
        const addColors = () => {
            document
                ...
                .forEach((line) => {
                    const origin = line.querySelector('.coordsOrigin a');
                    const dest = line.querySelector('.destCoords a');
                    const mission = line.getAttribute('data-mission-type');
                    const debrisD = line.querySelector('.destFleet .tf');
                    const moonD = line.querySelector('.destFleet .moon');
                    ...
                    const debrisO = line.querySelector('.originFleet .tf');
                    const moonO = line.querySelector('.originFleet .moon');
                    if (debrisO) {
        ...
        const changeSpy = () => {
            document
                ...
                .forEach((elem) => {
                    const params = new URL(elem.href).searchParams;
                    elem.href = '#';
                    elem.setAttribute(
                        'onClick',
        ...
        const addHover = () => {
            document.querySelectorAll('.eventFleet').forEach((line) => {
                const previous = Number(line.getAttribute('id').replace('eventRow-', '')) - 1;
                const next = Number(line.getAttribute('id').replace('eventRow-', '')) + 1;
                const previousNode = document.querySelector(`#eventRow-${previous}`);
                const nextNode = document.querySelector(`#eventRow-${next}`);
                const { opacity } = line.style;
        ...
        let addRefreshButton = () => {
            const refreshBtn = this.createDOM('a', { class: 'icon icon_reload' });
    ...
    expeditionImpact(show) {
        if (show) {
            document
                ...
                .forEach((elem) => {
                    const previous = Number(elem.getAttribute('id').replace('eventRow-', '')) - 1;
                    const previousNode = document.querySelector(`#eventRow-${previous}`);
                    if (previousNode) {
    ...
    addGalaxyTooltips() {
        document.querySelectorAll('.tooltipRel').forEach((sender) => {
            const rel = sender.getAttribute('rel');
            if (rel.indexOf('player') == 0 && rel != 'player99999') {
                const id = rel.replace('player', '');
                const content = document.querySelector(`#${rel}`);
                const rank = content.querySelector('.rank a');
                sender.appendChild(
                    this.createDOM(
                        'a',
    ...
    onGalaxyUpdate() {
        ...
        const dc = displayContentGalaxy;
        displayContentGalaxy = (b) => {
            dc(b);
            const json = $.parseJSON(b);
            if (!this.keepTooltip) {
    ...
    addGalaxyMarkers() {
        document
            ...
            .forEach((element, index) => {
                const moon = !!element.parentNode.querySelector('.cellMoon .tooltipRel');
                const playerDiv = element.parentNode.querySelector(
          ...
          || 99999;
                ...
                const colors = this.createDOM('div', {
                    class: 'ogl-colors',
                    'data-coords': coords,
                    'data-context': 'galaxy',
                });
        ...
        document
            ...
            .forEach((element, index) => {
                ...
                const coords = `${galaxy}:${system}:${Number(index + 1)}`;
                const playerDiv = element.querySelector(
                    '.cellPlayerName > span.tooltipRel',
                );
                const id = playerDiv && playerDiv.getAttribute('rel')
                    ? playerDiv.getAttribute('rel').replace('player', '')
                    : null;
    ...
    getActivity(row) {
        const planet = row.children[1];
        const moon = row.children[3];
        let planetAct = -1;
        let moonAct = -1;
        if (planet) {
            if (planet.querySelector('.activity.minute15')) {
                ...
                const timer = planet.querySelector('.activity.showMinutes');
        ...
        if (moon.children.length != 0) {
            if (moon.querySelector('.activity.minute15')) {
                ...
                const timer = moon.querySelector('.activity.showMinutes');
    ...
    updateSideActivity(planet, act) {
        ...
        const planetAct = planet.querySelector('.ogl-planet-act');
        const moonAct = planet.querySelector('.ogl-moon-act');
        if (!planetAct) return;
    ...
    // Stalk = planet list of  pinned target
        dataHelper.getPlayer(stalk.getAttribute('player-id')).then((player) => {
            player.planets.forEach((planet) => {
                ...
                const olds = stalk.querySelectorAll('a');
                let max;
                let maxCoords;
                let found = false;
                let coords;
    ...
    scan() {
        const sided = document.querySelectorAll('.ogl-stalkPlanets > a');
        ...
        const data = {};
        const ptreJSON = {};
        const baseCords = `${galaxy}:${system}`;
        const secureCoords = `${document.getElementById('galaxy_input').value
        }:${
            document.getElementById('system_input').value}`;
        const doubleCheckCoords = document.querySelector('.ogl-colors')?.dataset?.coords;
        if (
      ...
      || (doubleCheckCoords && doubleCheckCoords !== `${baseCords}:1`)
        ...
        document
            ...
            .forEach((row, index) => {
                const coords = `${baseCords}:${Number(index + 1)}`;
                ...
                if (playerDiv) {
                    ...
                    const playerId = playerDiv.getAttribute('id').replace('player', '');
                    const planetId = planetDiv ? planetDiv.dataset.planetId : -1;
                    const moonId = moonDiv ? moonDiv.dataset.moonId : -1;
                    const name = playerDiv.querySelector('span:first-of-type').innerText;

                    changes.push({
                        id: playerId,
                        name,
                        planetId,
                        moon: !!row.querySelector('.cellMoon .tooltipRel'),
                        moonId,
                        coords,
                    });

                    const sided = document.querySelectorAll('.ogl-stalkPlanets');
                    if (sided.length != 0) {
                        sided.forEach((side) => {
            ...
            && (this.json.sideStalk.indexOf(playerId) > -1
              || this.markedPlayers.indexOf(playerId) > -1)
                        ...
                        const planetActivity = row.querySelector(
                            ...
                            : row
                                ...
                                ?.textContent.trim() || 60;
                        const moonActivity = row.querySelector(
                    ...
                    const sided = document.querySelectorAll(
        ...
        if (Object.keys(ptreJSON).length > 0) {
            const systemCoords = [galaxy, system];
        ...
        data.serverTime = serverTime && typeof serverTime.getTime !== 'undefined'
            ? serverTime.getTime()
            : null;
    ...
    async ptreActivityUpdate(ptreJSON, systemCoords) {
        for (const coords of Object.keys(ptreJSON)) {
            ...
            const mainPlanet = pl.planets.find((obj) => obj.isMain === true);
    ...
    async flyingFleet() {
        ...
        document.querySelectorAll('.eventFleet').forEach((line) => {
            ...
            const back = line.getAttribute('data-return-flight');
        ...
        const per = (fleetCount / total) * 100;
        let color = 'friendly';
        if (per >= 70) color = 'neutral';
        if (per >= 90) color = 'hostile';
        const inter = setInterval(() => {
            const current = document.querySelector('.ogk-flying-per');
    ...
    welcome() {
        const container = this.createDOM('div', { class: 'ogk-welcome' });
        const head = container.appendChild(
        ...
        const shortcutsDiv = container.appendChild(
            this.createDOM(
                'p',
                {
        ...
        const ctrl = shortcutsDiv.appendChild(
            this.createDOM(
                'div',
                {
                    style:
        ...
        const keyHelp = container.appendChild(
            this.createDOM('div', { class: 'ogk-keyhelp' }),
        );
        const ctrlKey = this.createDOM(
            'div',
            {
        ...
        const heart = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path style="fill:#C80909" d="M12 4.435c-1.989-5.399-12-4.597-12 3.568 0 4.068 3.06 9.481 12 14.997 8.94-5.516 12-10.929 12-14.997 0-8.118-10-8.999-12-3.568z"/></svg>';
        container.appendChild(
            this.createDOM(
                'div',
                { class: 'ogk-love' },
    ...
    chat() {
        ...
        const oldfunc = ogame.chat.loadChatLogWithPlayer;
            ...
            document.querySelectorAll('.msg_actions').forEach((elem) => {
                ...
                if (keyNode) {
                    ...
                    if (key.startsWith('sr')) {
                        ...
                        const apiTechData = {};
                        for (const id in this.json.apiTechData) {
                        ...
                        link.addEventListener('click', () => window.open(
    ...
    sideOptions() {
        const harvestOptions = this.createDOM('div', { class: 'ogl-harvestOptions' });
        const container = document.querySelector('#myPlanets')
      || document.querySelector('#myWorlds');
        container.prepend(harvestOptions);
        const syncOption = harvestOptions.appendChild(
            this.createDOM('div', {
        ...
        const targetList = harvestOptions.appendChild(
            this.createDOM('a', {
                class: 'ogl-option ogl-targetIcon tooltip',
                title: this.getTranslatedText(1),
            }),
        );
        const search = harvestOptions.appendChild(
            this.createDOM('div', {
                class: 'ogl-option ogl-search-icon tooltip',
                title: 'Player search',
            }),
        );
        const statsBtn = harvestOptions.appendChild(
            this.createDOM('div', {
      ...
      && (empireBtn = harvestOptions.appendChild(
        ...
        const overViewBtn = harvestOptions.appendChild(
            this.createDOM('div', {
    ...
    profitGraph(profits, max, callback) {
        const content = this.createDOM('div', { class: 'ogk-profit' });
        const title = content.appendChild(
    ...
    repartitionGraph(eco, tech, fleet, def) {
        const div = this.createDOM('div', { class: 'ogk-repartition' });
        const chartNode = div.appendChild(
    ...
    winGraph(win, draw, count) {
        const div = this.createDOM('div', { class: 'ogk-win' });
        const chartNode = div.appendChild(
            this.createDOM('canvas', {
                id: 'piechart',
                width: '200px',
    ...
    generalStats(player) {
        const content = this.createDOM('div', { class: 'ogk-stats' });
        const globalInfo = content.appendChild(
        ...
        const playerDiv = globalInfo.appendChild(this.createDOM('h1'));
        ...
        const stats = playerDiv.appendChild(
        ...
        const detailRank = globalInfo.appendChild(
        ...
        const details = content.appendChild(
            this.createDOM('div', { class: 'ogk-details' }),
        );
        const ecoDetail = details.appendChild(
            this.createDOM('div', { class: 'ogk-box' }),
        );
        const techDetail = details.appendChild(
            this.createDOM('div', { class: 'ogk-box ogk-technos' }),
        );
        const div = techDetail.appendChild(
        ...
        const fleetTech = techDetail.appendChild(
            this.createDOM('div', { class: 'ogk-tech' }),
        );
        ...
        clvl /= sum;
        dlvl /= sum;
        const prod = ecoDetail.appendChild(
        ...
        prod.appendChild(this.createDOM('span'));
        const innerAstro = prod.appendChild(
            this.createDOM('span', {
                style:
        ...
        const innerEnergy = prod.appendChild(
            this.createDOM('span', {
                style:
          'display: flex; align-items: center; margin-left: auto; margin-top: 10px;',
            }),
        ...
        const innerPlasma = prod.appendChild(
            this.createDOM('span', {
                style:
          'display: flex; align-items: center; margin-left: auto; margin-top: 10px;',
            }),
        ...
        const fleetDetail = details.appendChild(
            this.createDOM('div', { class: 'ogk-box' }),
        );
        if (this.commander) {
            const fleet = fleetDetail.appendChild(
            ...
            [
                ...
                const flyingCount = flying.fleet[id];
                let sum = 0;
                ...
                totalSum += sum;
                const shipDiv = fleet.appendChild(this.createDOM('div'));
                shipDiv.appendChild(
                    this.createDOM('a', {
                        class: `ogl-option ogl-fleet-ship ogl-fleet-${id}`,
                    }),
            ...
            const fleetInfo = fleetDetail.appendChild(
                this.createDOM('div', { class: 'ogk-fleet-info' }),
            );
            const apiBtn = fleetInfo.appendChild(
            ...
            const rcpower = (((this.json.empire[0][114] * 5) / 100) * 2e4 + 2e4) * cyclos;
            fleetInfo.appendChild(
                this.createDOM(
                    'span',
                    {},
    ...
    tabs(titles, small) {
        const body = this.createDOM('div');
        const header = body.appendChild(this.createDOM('div', { class: 'ogl-tabs' }));
        const tabs = [];
        let first;
    ...
    async statistics() {
        const showStats = async () => {
            const player = await dataHelper.getPlayer(playerId);
    ...
    async ptreAction(frame, player) {
        ...
        const container = this.createDOM('div', { class: 'ptreContent' });
        ...
        const cleanPlayerName = encodeURIComponent(player.name);
        this.getJSON(
            ...
            (result) => {
                if (result.code == 1) {
                    const arrData = result.activity_array.succes == 1
                        ? JSON.parse(result.activity_array.activity_array)
                        : null;
                    const checkData = result.activity_array.succes == 1
                    ...
                    if (result.activity_array.succes == 1) {
                        arrData.forEach((line, index) => {
                            if (!isNaN(line[1])) {
                                ...
                                let title;
                                const checkValue = Math.max(0, 100 - dotValue);
    ...
    cleanupMessages() {
        for (const [id, result] of Object.entries(this.json.expeditions)) {
            if (
                !result.favorited
        ...
        for (const [id, result] of Object.entries(this.json.combats)) {
            if (
                !result.favorited
        ...
        for (const [id, result] of Object.entries(this.json.harvests)) {
    ...
    expeditionMessages() {
        const normalized = ['Metal', 'Crystal', 'Deuterium', 'AM'];
        const ressources = this.json.resNames;
        if (!this.combats) this.combats = {};
        if (!this.expeditionsIds) this.expeditionsIds = {};
        let cyclosName = '';
        for (const i in this.json.shipNames) {
        ...
        && document.querySelectorAll('div li.msg').forEach((msg) => {
            ...
            const date = msg.querySelector('.msg_date').innerText;
            if (
                    ...
                    dataHelper.getExpeditionType(textContent).then((type) => {
                        ...
                        ressources.forEach((res, i) => {
                            if (textContent.includes(res)) {
                                const regex = new RegExp('[0-9]{1,3}(.[0-9]{1,3})*', 'gm');
                        ...
                        const fleetMatches = textContent.match(/.*: [1-9].*/gm);
                        fleetMatches
            ...
            if (document.querySelector('li[id=subtabs-nfFleet21].ui-state-active')) {
                ...
                document.querySelectorAll(`div[id=${id}] li.msg`).forEach((msg) => {
                    const id = msg.getAttribute('data-msg-id');
                    const isCR = msg.querySelector('.msg_actions .icon_nf_link');
                    if (!isCR) {
                    ...
                    if (id in this.json.combats) {
                        ...
                        this.fetchAndConvertRC(id).then((cr) => {
                            if (cr === null) return;
                            const date = getFormatedDate(cr.timestamp, '[d].[m].[y]');
                            if (cr.coordinates.position == 16) {
                                ...
                                for (const [key, value] of Object.entries(cr.losses)) {
                ...
                document.querySelectorAll(`div[id=${id}] li.msg`).forEach((msg) => {
                    ...
                    if (id in this.json.harvests) {
                        if (this.json.harvests[id].coords.split(':')[2] == 16) {
                            msg.classList.add('ogk-expedition');
                        } else if (this.json.harvests[id].combat) {
                            msg.classList.add('ogk-combat');
                        } else {
                    ...
                    if (coords) {
                        const content = msg.querySelector('.msg_content').innerText;
                        coords = coords.innerText.slice(1, -1);
                        const matches = content.match(/[0-9.]*[0-9]/gm);
                        const met = Number(matches[matches.length - 2].split('.').join(''));
                        const cri = Number(matches[matches.length - 1].split('.').join(''));
                        let combat = false;
                        if (coords.split(':')[2] == 16) {
                            msg.classList.add('ogk-expedition');
                        } else if (content.includes(cyclosName)) {
                            msg.classList.add('ogk-harvest');
                        } else {
    ...
    overview(forcePlanetView = false) {
        const header = this.createDOM('div', { class: 'ogl-tabs' });
        const fleetBtn = header.appendChild(
            this.createDOM('span', { class: 'ogl-tab ogl-active' }, 'Fleet'),
        );
        const defBtn = header.appendChild(
            this.createDOM('span', { class: 'ogl-tab' }, 'Defense'),
        );
        const minesBtn = header.appendChild(
    ...
    fetchAndConvertRC(messageId) {
        ...
        return fetch(url)
            ...
            .then((str) => {
                ...
                const begin = str.indexOf(beginText) + 6;
                const end = str.indexOf('\');');
                const json = JSON.parse(str.substr(begin, end - begin));
                const combatIds = [];
                let isProbes = true;
                let isDefender = false;
                for (const i in json.attacker) {
                    for (const j in json.attacker[i].shipDetails) {
                        if (j != 210) {
                            isProbes = false;
                        }
                    }
                ...
                const damages = {};
                const losses = {};
                const lastRound = json.combatRounds[json.combatRounds.length - 1];
                if (lastRound) {
                    combatIds.forEach((id) => {
                        for (const i in lastRound.defenderLosses) {
                            if (!isDefender) {
                                Object.entries(lastRound.defenderLosses[i]).forEach((ship) => {
                                    const shipid = ship[0];
                                    const shipcount = Number(ship[1]);
                                    damages[shipid]
                        ...
                        for (const i in lastRound.attackerLosses) {
                            if (isDefender) {
    ...
    expeditionStats() {
        const ressources = ['Metal', 'Crystal', 'Deuterium', 'AM'];
        const content = this.createDOM('div', { class: 'ogk-stats-content' });
        const renderDetails = (sums, onchange) => {
            const content = this.createDOM('div', { class: 'ogk-stats' });
            const globalDiv = content.appendChild(
                this.createDOM('div', { class: 'ogk-global' }),
            );
            let numExpe = 0;
            ...
            const losses = this.getFleetCost(sums.losses);
            const fleetRes = this.getFleetCost(sums.fleet);
            const box = this.resourceBox(
                [
                    {
                        title: 'Found',
                        metal: sums.found[0],
                ...
                () => {
                    ...
                    globalDiv.appendChild(
                        this.blackHoleBox((costs) => {
                            const date = document.querySelector('.ogk-date strong').innerText;
        ...
        const computeRangeSums = (sums, start, stop) => {
            const weekSums = {
                found: [0, 0, 0, 0],
                harvest: [0, 0],
                losses: {
            ...
            for (
                ...
                const dateStr = getFormatedDate(new Date(d).getTime(), '[d].[m].[y]');
                if (sums[dateStr]) {
                    ...
                    for (const [type, num] of Object.entries(sums[dateStr].type)) {
                        weekSums.type[type]
        ...
        const getTotal = (sums) => {
            let total = 0;
            const fleet = this.getFleetCost(sums.fleet);
            const losses = this.getFleetCost(sums.losses);
        ...
        content.appendChild(
            this.tabs({
                D: () => {
                    ...
                    let max = 0;
                    for (let i = 0; i < 12; i++) {
                        const dateStr = getFormatedDate(date.getTime(), '[d].[m].[y]');
                        const sums = this.json.expeditionSums[dateStr] || sum;
                        const profit = sums ? getTotal(sums) : 0;
                ...
                All: () => {
                    const keys = Object.keys(this.json.expeditionSums).sort(
    ...
    combatStats() {
        ...
        const renderDetails = (sums, onchange) => {
            ...
            const topDiv = globalDiv.appendChild(
            ...
            sums.topCombats.forEach(async (top) => {
                ...
                const player = await dataHelper.getPlayer(top.ennemi);
        ...
        content.appendChild(
            this.tabs({
                D: () => {
                    ...
                    for (let i = 0; i < 12; i++) {
                        const dateStr = getFormatedDate(date.getTime(), '[d].[m].[y]');
                        const sums = this.json.combatsSums[dateStr] || sum;
                        const profit = sums ? getTotal(sums) : 0;
                        if (Math.abs(profit) > max) max = profit;
                        profits.push({
                            date: new Date(date.getTime()),
                ...
                All: () => {
                    const keys = Object.keys(this.json.combatsSums).sort(
                        (a, b) => this.dateStrToDate(a) - this.dateStrToDate(b),
                    );
                    const minDate = keys[0];
                    const maxDate = keys[keys.length - 1];
    ...
    blackHoleBox(onValidate) {
        const box = this.createDOM('div', { class: 'ogk-box ogk-small' });
        const fleet = box.appendChild(
            this.createDOM('div', { class: 'ogk-bhole-grid' }),
        );
        const inputs = [];
        ...
        if (onValidate) {
            ...
            btn.addEventListener('click', () => {
                ...
                const cost = this.getFleetCost(fleet);
                onValidate(cost);
            });
        }
        return box;
    ...
    shipsBox(ships, minus) {
        const fleetDetail = this.createDOM('div', { class: 'ogk-box' });
        const fleet = fleetDetail.appendChild(
            this.createDOM('div', { class: 'ogk-fleet' }),
        );
    ...
    resourceBox(rows, am, callback) {
        const box = this.createDOM('div', { class: 'ogk-box' });
        const prod = box.appendChild(this.createDOM('div', { class: 'ogk-grid' }));
        ...
        const sums = [0, 0, 0];
        rows.forEach((row) => {
            const p = prod.appendChild(this.createDOM('p', {}, row.title));
            if (row.edit) {
                p.appendChild(
                    this.createDOM(
                        'strong',
    ...
    playerSearch(show, name) {
        const renderPlayerInfo = (player) => {
            ...
            this.saveData();
            const planetsColumn = this.createDOM('div', { class: 'ogl-planets-col' });
            const controlRow = planetsColumn.appendChild(
            ...
            const btns = controlRow.appendChild(this.createDOM('div'));

            if (this.json.options.ptreTK) {
                const ptreLink = btns.appendChild(
            ...
            const stats = btns.appendChild(
                this.createDOM('a', { class: 'ogl-mmorpgstats' }),
            );
            const pinBtn = btns.appendChild(this.createDOM('a', { class: 'ogl-pin' }));

            const chat = btns.appendChild(
            ...
            const detailRank = planetsColumn.appendChild(
                this.createDOM('div', { class: 'ogl-detailRank' }),
            );
            const dotted = (value) => parseInt(value).toLocaleString(separatorLang);
            detailRank.html(
            ...
            const stalkPlanets = this.createDOM('div', {
            ...
            this.highlightTarget();
            const updateTime = planetsColumn.appendChild(
        ...
        let activeId; let
            activeNode;
        const updatePlayerList = (players, forced) => {
            players.forEach(async (player, index) => {
                ...
                let noob = false;
                const self = await dataHelper.getPlayer(playerId);
                const currentScore = self.points.score;
                if (currentScore > 5e5) {
                ...
                const playerNode = this.createDOM('div', { class: 'ogl-player-div' });
                const name = this.createDOM(
                    'span',
                ...
                const alliNode = playerNode.appendChild(
                ...
                playerNode.addEventListener('click', () => {
                    ...
                    dataHelper.getPlayer(player.id).then((pl) => {
                        const div = content.querySelector('.ogl-planets-col');
                        if (div) div.remove();
        ...
        let updateSearch = async (value, alliance, forced) => {
            searchResult.empty();
            if (value.length > 2) {
                const possible = await dataHelper.filter(value, alliance);
                possible.sort((a, b) => {
                    if (alliance) {
        ...
        const searchColumn = content.appendChild(
    ...
    minesOverview() {
        const content = this.createDOM('div', { class: 'ogl-mines-content' });
        const table = content.appendChild(
            this.createDOM('table', { class: 'ogl-fleet-table' }),
        );
        const header = table.appendChild(this.createDOM('tr'));
        header.appendChild(this.createDOM('th'));
        const metalRow = table.appendChild(this.createDOM('tr'));
        const crystalRow = table.appendChild(this.createDOM('tr'));
        const deutRow = table.appendChild(this.createDOM('tr'));
        const nrjRow = table.appendChild(this.createDOM('tr'));
        ...
        this.json.empire.forEach((planet) => {
            ...
            const link = `?page=ingame&component=supplies&cp=${planet.id}`;
            header.appendChild(
                this.createDOM(
                    'th',
                    {},
            ...
            const diff = planet.production.hourly[3];
            td.appendChild(
                this.createDOM(
                    'div',
        ...
        const sumlvl = (key) => this.json.empire.reduce((a, b) => a + Number(b[key]), 0);
        const sumhour = (key) => this.json.empire.reduce(
            (a, b) => a + Number(b.production.hourly[key]),
            0,
        );
        const sumday = (key) => this.json.empire.reduce((a, b) => a + Number(b.production.daily[key]), 0);
        const sumweek = (key) => this.json.empire.reduce(
    ...
    minesStats() {
        ...
        || this.production(3, this.json.myMines[coords].deuterium, true);
            ...
            if (current) td.classList.add('ogl-current');
            td = nrjRow.appendChild(this.createDOM('td'));
            const diff = this.json.myMines[coords].energy;
            td.appendChild(
                this.createDOM(
                    'div',
                    {
    ...
    fleetOverview(moon) {
        const content = this.createDOM('div', { class: 'ogl-fleet-content' });
        const table = this.createDOM('table', { class: 'ogl-fleet-table' });
        let row = this.createDOM('tr');
        const td = this.createDOM('th');
        const planetIcon = this.createDOM('span', {
            class: `ogl-planet ${!moon ? 'ogl-active' : ''}`,
        });
        const moonIcon = this.createDOM('span', {
        ...
        row.appendChild(td);
        this.json.empire.forEach((planet) => {
            const link = `?page=ingame&component=fleetdispatch&cp=${
        ...
        [
            ...
            const shipCount = flying.fleet[id];
            ...
            row.appendChild(td);
            const th = row.appendChild(this.createDOM('th'));
            th.appendChild(
                this.createDOM('th', {
                    class: `ogl-option ogl-fleet-ship ogl-fleet-${id}`,
                }),
          ...
          += moon && planet.moon ? Number(planet.moon[id]) : Number(planet[id]);
                const valuePLa = planet[id] == 0 ? '-' : this.formatToUnits(planet[id]);
                let valueMooon = '-';
                if (planet.moon) {
    ...
    defenseOverview(moon) {
        ...
        row.appendChild(td);
        this.json.empire.forEach((planet) => {
            const link = `?page=ingame&component=defenses&cp=${
    ...
    buildDispatcherUI() {
        ...
        const destination = dispatch.appendChild(
            this.createDOM('div', { class: 'ogl-dest' }),
        );
        const resDiv = dispatch.appendChild(this.createDOM('div'));
        const actions = resDiv.appendChild(
            this.createDOM('div', { class: 'ogl-transport' }),
        );
        const coords = destination.appendChild(
            this.createDOM('div', { class: 'ogl-coords' }),
        );
        const warning = coords.appendChild(
            this.createDOM('a', {
                class: 'ogl-warning tooltipRight',
                'data-title': 'Error : current planet/moon...',
            }),
        );
        const galaxyInput = coords.appendChild(
            this.createDOM('input', {
                type: 'text',
                pattern: '[0-9]*',
        ...
        const systemInput = coords.appendChild(
            this.createDOM('input', {
                type: 'text',
                pattern: '[0-9]*',
        ...
        const positionInput = coords.appendChild(
            this.createDOM('input', {
                type: 'text',
                pattern: '[0-9]*',
        ...
        const planet = coords.appendChild(
            this.createDOM('a', { class: 'ogl-planet-icon' }),
        );
        const moon = coords.appendChild(
            this.createDOM('a', { class: 'ogl-moon-icon' }),
        );
        const debris = coords.appendChild(
        ...
        const planetList = coords.appendChild(
            this.createDOM(
                'div',
                { class: 'ogl-homes' },
        ...
        if (unions.length != 0) {
            const unionsBtn = coords.appendChild(
                this.createDOM(
                    'div',
                    { class: 'ogl-union-btn' },
            ...
            unionsBtn.addEventListener('click', () => {
                ...
                for (const i in unions) {
                    const union = unions[i];
                    const unionDiv = container.appendChild(
                        this.createDOM(
                            'div',
        ...
        planetList.addEventListener('click', () => {
            const container = this.openPlanetList((planet) => {
        ...
        $('.ogl-fleetSpeed div').on('mouseout', (event) => {
            ...
            const middle = deutLeft.classList.contains('middlemark');
            update(false);
            if (middle) {
                deutLeft.classList.add('middlemark');
            }
        });
        const missionsDiv = destination.appendChild(
            this.createDOM('div', { class: 'ogl-missions' }),
        );
        missionsDiv.html('<span style="color: #9099a3"> No missions... </span>');
        const resFiller = actions.appendChild(
            this.createDOM('div', { class: 'ogl-res-filler' }),
        );
        const metalBtn = resFiller.appendChild(this.createDOM('div'));
        metalBtn.appendChild(
            this.createDOM('div', { class: 'resourceIcon metal' }),
        );
        const metalFiller = metalBtn.appendChild(
            this.createDOM('input', { type: 'text' }),
        );
        const metalLeft = metalBtn.appendChild(this.createDOM('span', {}, '-'));
        const metalReal = metalBtn.appendChild(
            this.createDOM('span', { class: 'ogk-real-cargo ogk-metal' }, '-'),
        );
        const btns = metalBtn.appendChild(
            this.createDOM('div', { class: 'ogl-actions' }),
        );
        const selectMinMetal = btns.appendChild(
            this.createDOM('img', {
                src: 'https://gf2.geo.gfsrv.net/cdn10/45494a6e18d52e5c60c8fb56dfbcc4.gif',
            }),
        );
        const selectMostMetal = btns.appendChild(
            this.createDOM('a', { class: 'select-most-min' }),
        );
        const selectMaxMetal = btns.appendChild(
            this.createDOM('img', {
                src: 'https://gf3.geo.gfsrv.net/cdnea/fa0c8ee62604e3af52e6ef297faf3c.gif',
            }),
        );
        const crystalBtn = resFiller.appendChild(this.createDOM('div'));
        crystalBtn.appendChild(
            this.createDOM('div', { class: 'resourceIcon crystal' }),
        );
        const crystalFiller = crystalBtn.appendChild(
            this.createDOM('input', { type: 'text' }),
        );
        const crystalLeft = crystalBtn.appendChild(this.createDOM('span', {}, '-'));
        const crystalReal = crystalBtn.appendChild(
            this.createDOM('span', { class: 'ogk-real-cargo ogk-crystal' }, '-'),
        );
        const crystalBtns = crystalBtn.appendChild(
            this.createDOM('div', { class: 'ogl-actions' }),
        );
        const selectMinCrystal = crystalBtns.appendChild(
            this.createDOM('img', {
                src: 'https://gf2.geo.gfsrv.net/cdn10/45494a6e18d52e5c60c8fb56dfbcc4.gif',
            }),
        );
        const selectMostCrystal = crystalBtns.appendChild(
            this.createDOM('a', { class: 'select-most-min' }),
        );
        const selectMaxCrystal = crystalBtns.appendChild(
            this.createDOM('img', {
                src: 'https://gf3.geo.gfsrv.net/cdnea/fa0c8ee62604e3af52e6ef297faf3c.gif',
            }),
        );
        const deutBtn = resFiller.appendChild(this.createDOM('div'));
        deutBtn.appendChild(
            this.createDOM('div', { class: 'resourceIcon deuterium' }),
        );
        const deutFiller = deutBtn.appendChild(
            this.createDOM('input', { type: 'text' }),
        );
        let deutLeft = deutBtn.appendChild(this.createDOM('span', {}, '-'));
        const deutReal = deutBtn.appendChild(
            this.createDOM('span', { class: 'ogk-real-cargo ogk-deut' }, '-'),
        );
        const deutBtns = deutBtn.appendChild(
            this.createDOM('div', { class: 'ogl-actions' }),
        );
        const selectMinDeut = deutBtns.appendChild(
            this.createDOM('img', {
                src: 'https://gf2.geo.gfsrv.net/cdn10/45494a6e18d52e5c60c8fb56dfbcc4.gif',
            }),
        );
        const selectMostDeut = deutBtns.appendChild(
            this.createDOM('a', { class: 'select-most-min' }),
        );
        const selectMaxDeut = deutBtns.appendChild(
            this.createDOM('img', {
                src: 'https://gf3.geo.gfsrv.net/cdnea/fa0c8ee62604e3af52e6ef297faf3c.gif',
            }),
        );
        ...
        const load = this.createDOM('div', { class: 'ogl-cargo' });
        const selectMostRes = load.appendChild(
            this.createDOM('a', { class: 'select-most' }),
        );
        const selectAllRes = load.appendChild(
            this.createDOM('a', { class: 'sendall' }),
        );
        const selectNoRes = load.appendChild(
            this.createDOM('a', { class: 'send_none' }, '<a></a>'),
        );
        ...
        const bar = load.appendChild(this.createDOM('div', {}));
        bar.html(
        ...
        const settings = load.appendChild(
            this.createDOM(
                'div',
        ...
        const transport = actions.appendChild(
            this.createDOM('div', { class: 'ogl-res-transport' }),
        );
        const ptBtn = transport.appendChild(
            this.createDOM('a', {
                'tech-id': 202,
                class: 'ogl-option ogl-fleet-ship ogl-fleet-202',
            }),
        );
        const ptNum = transport.appendChild(this.createDOM('span', {}, '-'));
        const gtBtn = transport.appendChild(
            this.createDOM('a', {
                'tech-id': 203,
                class: 'ogl-option ogl-fleet-ship ogl-fleet-203',
            }),
        );
        const gtNum = transport.appendChild(this.createDOM('span', {}, '-'));
        const pfBtn = transport.appendChild(
            this.createDOM('a', {
                'tech-id': 219,
                class: 'ogl-option ogl-fleet-ship ogl-fleet-219',
            }),
        );
        const pfNum = transport.appendChild(this.createDOM('span', {}, '-'));
        const cyBtn = transport.appendChild(
            this.createDOM('a', {
                'tech-id': 209,
                class: 'ogl-option ogl-fleet-ship ogl-fleet-209',
            }),
        );
        const cyNum = transport.appendChild(this.createDOM('span', {}, '-'));
        let pbBtn;
        let pbNum;
        ...
        const onTargetChange = function () {
            const galaxy = clampInt(
                galaxyInput.value,
                1,
    ...
    betterFleetDispatcher() {
      ...
      && fleetDispatcher.shipsOnPlanet.length != 0
            ...
            const metalAvailable = Math.max(0, fleetDispatcher.metalOnPlanet);
            const crystalAvailable = Math.max(0, fleetDispatcher.crystalOnPlanet);
            const deutAvailable = Math.max(0, fleetDispatcher.deuteriumOnPlanet);
            const fleetPageParameters = new URLSearchParams(window.location.search);
        ...
        && fleetPageParameters.has('mission')
            ...
            const needCargo = (fret) => {
                ...
                const amount = this.calcNeededShips({
                    fret,
                    resources:
            ...
            const trySubmitFleet1 = fleetDispatcher.trySubmitFleet1.bind(fleetDispatcher);
            ...
            this.overwriteFleetDispatcher('stopLoading', false, () => {
                const missions = fleetDispatcher.getAvailableMissions();
                const warning = document.getElementsByClassName(
                    'ogl-warning tooltipRight',
                )[0];
                const missionsDiv = document.getElementsByClassName('ogl-missions')[0];
                let iconsDiv;
                if (auxAjaxFailed) {
                    missionsDiv.html(
          ...
          || !fleetDispatcher.hasShipsSelected()
            ...
            || (fleetDispatcher.currentPage == 'fleet2' && missions.length > 0)
                    ...
                    $('.ogl-mission-icon').on('click', (e) => {
                        ...
                        const missionClick = Number(e.target.getAttribute('mission'));
            ...
            missionsDiv.html('<span style="color: #9099a3"> No missions... </span>');
            const switchToPage = fleetDispatcher.switchToPage.bind(fleetDispatcher);
            const refresh = fleetDispatcher.refresh.bind(fleetDispatcher);
            const resetShips = fleetDispatcher.resetShips.bind(fleetDispatcher);
            const selectShip = fleetDispatcher.selectShip.bind(fleetDispatcher);
            ...
            && this.json.options.harvestMission == 3
                        ...
                        return true;
                    } if (this.json.options.foreignMission == 3) {
                        return true;
                    }
                }
                if (
          ...
          && !this.expedition
            ...
            fleetDispatcher.switchToPage = (page) => {
                if (!(fleetDispatcher.currentPage == 'fleet1' && page == 'fleet3')) {
                    ...
                    if (fleetDispatcher.currentPage == 'fleet3') {
                        if (fleetDispatcher.mission == 0) {
                            const missionIcons = document.querySelectorAll('#missions .on a');
                            let mission = 0;
            ...
            const { displayErrors } = fleetDispatcher;
            let error;
            ...
            let update = (submit) => {
                ...
                if (submit) {
                    const newFleet = JSON.stringify(
                        fleetDispatcher.shipsToSend.map((elem) => elem.id),
                    );
                    const newTargetPlanet = JSON.stringify(fleetDispatcher.targetPlanet);
                    if (
                        newFleet != fleet
          ...
          == `${fleetDispatcher.targetPlanet.galaxy
            ...
            document
                ...
                .addEventListener('keyup', (e) => {
                    const val = that.removeNumSeparator(
                        document.querySelector('input#metal').value,
                    );
                    const capacity = fleetDispatcher.getFreeCargoSpace();
            ...
            let refreshRes = () => {
                const mLeft = document.querySelector('.res_wrap .ogi-metalLeft');
                const cLeft = document.querySelector('.res_wrap .ogi-crystalLeft');
                const dLeft = document.querySelector('.res_wrap .ogi-deuteriumLeft');
            ...
            $('#selectMostMetal').on('click', () => {
                const capacity = fleetDispatcher.getFreeCargoSpace();
                const cargo = Math.min(capacity, metalAvailable - (kept[0] || 0));
                fleetDispatcher.cargoMetal = Math.min(
            ...
            const updateCargo = () => {
                const total = Number(this.removeNumSeparator(metalFiller.value))
          + Number(this.removeNumSeparator(crystalFiller.value))
          + Number(this.removeNumSeparator(deutFiller.value));
                const freeSpace = fleetDispatcher.getCargoCapacity() - total;
                bar.html(
                    `<div class="fleft bar_container" data-current-amount="0" data-capacity="0">\n        <div class="filllevel_bar"></div>\n        </div>\n        <div>\n        <span class="${
                ...
                const filler = document.querySelector('.ogl-cargo .filllevel_bar');
            ...
            const updateShips = (e) => {
                const amount = e.target.nextElementSibling.getAttribute('amount');
            ...
            let onResChange = (index) => {
                const capacity = fleetDispatcher.getCargoCapacity();
                if (capacity == 0) {
            ...
            && deutLeft.innerText != '0'
                        ...
                        const currentDeut = deutAvailable - fleetDispatcher.getConsumption();
            ...
            && crystalLeft.innerText != '0'
                        ...
                        crystalReal.innerText = crystalAvailable.toLocaleString(separatorLang);
                    }
                } else if (index == 0) {
        ...
        if (this.page == 'fleetdispatch' && document.querySelector('#shipChosen')) {
            shipsOnPlanet.forEach((ship) => {
                if (ship.id == 202 || ship.id == 203) {
                    ...
                    const total = min.metal + min.crystal + min.deut;
                    const amount = this.calcNeededShips({
                        fret: ship.id,
                        resources: total,
                    });
    ...
    harvest() {
        const btnAction = (event, coords, type) => {
            event.preventDefault();
            event.stopPropagation();
            const link = `?page=ingame&component=fleetdispatch&galaxy=${coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${type}&mission=${this.json.options.harvestMission}&oglMode=1`;
    ...
    openPlanetList(callcback) {
        ...
        const buildButton = (planet, id, galaxy, system, position, type) => {
            const data = {
                id,
                galaxy,
                system,
            ...
            const div = container.appendChild(this.createDOM('div'));
    ...
    autoHarvest() {
        ...
        && !nextElement.querySelector('.moonlink')
            ...
            let destination;
            let type = 1;
            const mission = this.json.options.harvestMission;
        ...
        || (this.current.isMoon && this.mode == 5)
            ...
            const pCoords = this.current.isMoon
            ...
            dataHelper.getPlayer(playerId).then((player) => {
                ...
                [ptExpe, gtExpe].forEach((btn) => {
                    btn.addEventListener('click', () => {
                        ...
                        const prio = [218, 213, 211, 215, 207];
                        let bigship = 0;
                        prio.forEach((shipID) => {
                            if (
                                bigship == 0
    ...
    activitytimers() {
        ...
        this.planetList.forEach((planet) => {
            const coords = planet.querySelector('.planet-koords').textContent;
            const timers = this.json.myActivities[coords] || [0, 0];
            let value = Math.min(Math.round((now - timers[0]) / 6e4), 60);
            const pTimer = planet.querySelector('.planetlink').appendChild(
                this.createDOM('div', {
            ...
            if (planet.querySelector('.moonlink')) {
                const mTimer = planet.querySelector('.moonlink').appendChild(
                    this.createDOM('div', {
                        class: 'ogl-timer ogl-short ogl-medium',
    ...
    getFlyingRes() {
        ...
        const planets = {};
        document.querySelectorAll('.eventFleet').forEach((line) => {
            const tooltip = line.querySelector('.tooltip');
            const id = Number(line.getAttribute('id').split('-')[1]);
            const back = line.getAttribute('data-return-flight') != 'false';
            const type = line.getAttribute('data-mission-type');
            const time = line.getAttribute('data-arrival-time');
            const coords = line
                .querySelector('.coordsOrigin > a')
                .innerText.trim()
                .slice(1, -1);
            const destCoords = line
                .querySelector('.destCoords > a')
                .innerText.trim()
                .slice(1, -1);
            const destIsMoon = document.querySelector('.destFleet .moon');
            const originIsMoon = document.querySelector('.originFleet .moon');
            const origin = coords + (originIsMoon ? 'M' : 'P');
            ...
            if (type == 4 || (back && !(id - 1 in uniques) && !(id - 2 in uniques))) {
                ...
                div.querySelectorAll('td[colspan="2"]').forEach((tooltip) => {
                    ...
                    const name = tooltip.innerText.trim().slice(0, -1);
                    const id = this.json.shipNames[name];
                    if (id) {
    ...
    updateresourceDetail() {
        ...
        let cSumM = 0;
        let dSumM = 0;
        const emulatedEmpire = [];
        !this.commander
      && this.planetList.forEach((planet) => {
          const coords = planet.querySelector('.planet-koords').textContent;
          const id = planet.getAttribute('id').replace('planet-', '');
          const moon = planet.querySelector('.moonlink');
          let metal = 0;
          let crystal = 0;
          let deuterium = 0;
          ...
          let deutProd = 0;
          let diff = 0;
          const res = this.json.myRes[`${coords}P`];
          const moonRes = this.json.myRes[`${coords}M`];
          if (res) {
              metal = res.metal;
              crystal = res.crystal;
              deuterium = res.deuterium;
              const mines = this.json.myMines[coords];
        ...
        let { empire } = this.json;
        if (emulatedEmpire.length != 0) {
            empire = emulatedEmpire;
        }
        empire.forEach((planet) => {
            const planetNode = document.querySelector(`div[id=planet-${planet.id}]`);
            const isFullM = planet.metalStorage - planet.metal > 0 ? '' : ' ogl-full';
            const isFullC = planet.crystalStorage - planet.crystal > 0 ? '' : ' ogl-full';
            const isFullD = planet.deuteriumStorage - planet.deuterium > 0 ? '' : ' ogl-full';
            const isaFullM = planet.metalStorage - planet.metal > planet.production.hourly[0] * 2
                ? ''
                : ' ogl-afull';
            const isaFullC = planet.crystalStorage - planet.crystal > planet.production.hourly[1] * 2
                ? ''
                : ' ogl-afull';
            const isaFullD = planet.deuteriumStorage - planet.deuterium
        > planet.production.hourly[2] * 2
                ? ''
                : ' ogl-afull';
            const metalRess = planetNode.querySelectorAll('.ogl-metal');
            const crystalRess = planetNode.querySelectorAll('.ogl-crystal');
            const deutRess = planetNode.querySelectorAll('.ogl-deut');
            ...
            const sumNodes = document.querySelectorAll('.ogl-summary');
    ...
    resourceDetail() {
      ...
      && rechts.addEventListener('mouseover', () => {
          const rect = rechts.getBoundingClientRect();
          if (rect.width + rect.x > window.innerWidth) {
              const diff = rect.width + rect.x - window.innerWidth;
      ...
      && rechts.addEventListener('mouseout', (e) => {
        ...
        const flying = this.createDOM('div', { class: 'ogl-res' });
        flying.appendChild(
            this.createDOM(
                'span',
                { class: 'ogl-metal' },
        ...
        const flyingSum = this.createDOM('div', {
      ...
      && this.planetList.forEach((planet) => {
        ...
        empire.forEach((elem) => {
            const planet = list.querySelector(`div[id=planet-${elem.id}]`);
            if (!planet) return;
            const isFullM = elem.metalStorage - elem.metal > 0 ? '' : ' ogl-full';
            const isFullC = elem.crystalStorage - elem.crystal > 0 ? '' : ' ogl-full';
            const isFullD = elem.deuteriumStorage - elem.deuterium > 0 ? '' : ' ogl-full';
            const isaFullM = elem.metalStorage - elem.metal > elem.production.hourly[0] * 2
                ? ''
                : ' ogl-afull';
            const isaFullC = elem.crystalStorage - elem.crystal > elem.production.hourly[1] * 2
                ? ''
                : ' ogl-afull';
            const isaFullD = elem.deuteriumStorage - elem.deuterium > elem.production.hourly[2] * 2
                ? ''
                : ' ogl-afull';
            const divPla = this.createDOM('div', { class: 'ogl-res' });
            ...
            if (elem.moon) {
                const divMoon = this.createDOM('div', { class: 'ogl-res' });
        ...
        const divPlaSum = this.createDOM('div', { class: 'ogl-res' });
        ...
        const divMoonSum = this.createDOM('div', { class: 'ogl-res' });
        ...
        const sumPlanet = this.createDOM('div', {
            class: 'smallplanet smaller ogl-summary',
        });
        ...
        const moonSumSymbol = sumPlanet.appendChild(
            this.createDOM('div', { class: 'ogl-sum-symbol' }, 'Σ'),
        );
        sumPlanet.appendChild(divMoonSum);
        list.appendChild(sumPlanet);
        list.appendChild(flyingSum);
        const sum = this.createDOM('div', {
            class: 'smallplanet smaller ogl-summary',
        });
        const sumres = this.createDOM('div', { class: 'ogl-res' });
        sumres.appendChild(
            this.createDOM(
                'span',
                { class: 'ogl-metal' },
    ...
    updateInfo() {
        ...
        this.isLoading = true;
        const svg = '<svg width="80px" height="30px" viewBox="0 0 187.3 93.7" preserveAspectRatio="xMidYMid meet">\n                <path stroke="#3c536c" id="outline" fill="none" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10"\n                  d="M93.9,46.4c9.3,9.5,13.8,17.9,23.5,17.9s17.5-7.8,17.5-17.5s-7.8-17.6-17.5-17.5c-9.7,0.1-13.3,7.2-22.1,17.1\n                    c-8.9,8.8-15.7,17.9-25.4,17.9s-17.5-7.8-17.5-17.5s7.8-17.5,17.5-17.5S86.2,38.6,93.9,46.4z" />\n                <path id="outline-bg" opacity="0.1" fill="none" stroke="#eee" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" d="\n                M93.9,46.4c9.3,9.5,13.8,17.9,23.5,17.9s17.5-7.8,17.5-17.5s-7.8-17.6-17.5-17.5c-9.7,0.1-13.3,7.2-22.1,17.1\n                c-8.9,8.8-15.7,17.9-25.4,17.9s-17.5-7.8-17.5-17.5s7.8-17.5,17.5-17.5S86.2,38.6,93.9,46.4z" />\n\t\t\t\t      </svg>';
        document
    ...
    targetList(show) {
        const renderTagetList = () => {
            let galaxy = this.json.targetTabs.g != -1;
            let system = this.json.targetTabs.s != -1;
            const div = this.createDOM('div', { class: 'ogl-target-list' });
            const header = div.appendChild(
                this.createDOM('div', { class: 'ogk-controls' }),
            );
            const markers = header.appendChild(this.createDOM('div'));
            [
                'red',
                'orange',
                'yellow',
                ...
                const toggle = this.createDOM('div', {
            ...
            const filterTabs = header.appendChild(
                this.createDOM('div', {
                    class: 'ogl-tabList',
                    style: 'margin-bottom: 5px;',
                }),
            );
            const tabG = filterTabs.appendChild(
                this.createDOM(
                    'div',
            ...
            tabG.addEventListener('click', () => {
                if (this.json.targetTabs.g == -1) {
                    ...
                    galaxy = false;
                    this.saveData();
                    const active = header.querySelector('.ogl-tab[data-galaxy].ogl-active');
            ...
            const tabS = filterTabs.appendChild(
                this.createDOM(
                    'div',
            ...
            tabS.addEventListener('click', () => {
                if (this.json.targetTabs.s == -1) {
                    ...
                    system = false;
                    this.saveData();
                    const active = header.querySelector('.ogl-tab[data-system].ogl-active');
                    if (active) active.classList.remove('ogl-active');
            ...
            const galaxyTabList = header.appendChild(
                this.createDOM('div', { class: 'ogl-tabList ogl-galaxyTabList' }),
            );
            const systemTabList = header.appendChild(
                this.createDOM('div', { class: 'ogl-tabList ogl-systemTabList' }),
            );
            const planetList = content.appendChild(
            ...
            let checkEmpty = (galaxy, system) => {
                for (let g = 1; g <= 10; g++) {
                    if (galaxy) {
                        const children = content.querySelector(
            ...
            for (const coords in this.json.markers) {
                if (this.json.markers[coords] == '') {
                    delete this.json.markers[coords];
                    this.markedPlayers = this.getMarkedPlayers(this.json.markers);
                }
            }
            const keys = Object.keys(this.json.markers).sort((a, b) => {
                const coordsA = a
                    .split(':')
            ...
            for (let i = 0; i < step * 10; i += step) {
                const sTab = systemTabList.appendChild(
            ...
            for (let i = 1; i <= 10; i++) {
                const gTab = galaxyTabList.appendChild(
        ...
        if (show) {
            ...
            const list = document.querySelector('.ogl-target-list');
            if (list) {
                list.remove();
                document.querySelector('#planetList').style.display = 'block';
                document.querySelector('#countColonies').style.display = 'block';
    ...
    addMarkerUI(coords, parent, id) {
        const div = this.createDOM('div', { class: 'ogl-colorChoice' });
        [
            'red',
            'orange',
            'yellow',
    ...
    stalk(sender, player, delay) {
        ...
        const render = (player) => {
            finalPlayer = player;
            const content = this.createDOM('div');
            content.html(
            ...
            const actions = content.appendChild(
            ...
            const msgBtn = actions.appendChild(
                this.createDOM('a', { class: 'icon icon_chat' }),
            );
          ...
          && first.getAttribute('data-coords')
            == active.getAttribute('data-coords')
                ...
                const splits = next.getAttribute('data-coords').split(':');
            ...
            const date = content.appendChild(
                this.createDOM('span', {
            ...
            const detailRank = content.appendChild(
                this.createDOM('div', { class: 'ogl-detailRank' }),
            );
            content.appendChild(this.createDOM('hr'));
            const list = content.appendChild(
                this.createDOM('div', {
                    class: 'ogl-stalkPlanets',
                    'player-id': player.id,
                }),
            );
            const count = content.appendChild(
                this.createDOM('div', { class: 'ogl-fullGrid ogl-right' }),
            );
            const sideStalk = content.appendChild(
                this.createDOM('a', { class: 'ogl-pin' }),
            );
            sideStalk.addEventListener('click', () => this.sideStalk(player.id));
            const stats = content.appendChild(
                this.createDOM('a', { class: 'ogl-mmorpgstats' }),
            );
            stats.addEventListener('click', () => {
            ...
            if (this.json.options.ptreTK) {
                const ptreLink = content.appendChild(
                    this.createDOM('a', { class: 'ogl-ptre' }),
                );
                ptreLink.textContent = 'P';
                ptreLink.addEventListener('click', () => {
            ...
            const planets = this.updateStalk(player.planets, player.id);
    ...
    // debugger;
        ...
        const planetDiv = a.appendChild(
            this.createDOM('div', { class: 'ogl-planet-div' }),
        );
        const planetIcon = planetDiv.appendChild(
        ...
        const targeted = this.json.markers[coords.join(':')];
        if (targeted) {
    ...
    updateStalk(planets) {
        ...
        sorted.forEach((planet) => {
            ...
            const targeted = this.json.markers[planet.coords];
            if (targeted) {
                a.classList.add('ogl-marked');
                a.setAttribute('data-marked', targeted.color);
            } else {
    ...
    sideStalk(playerid) {
        ...
        const last = this.json.sideStalk[this.json.sideStalk.length - 1];
        if (last) {
            playerid = last;
            ...
            let actBtn; let watchlistBtn; let
                ptreBtn;
            ...
            dataHelper.getPlayer(playerid).then((player) => {
                ...
                const container = sideStalk.appendChild(
                    this.createDOM('div', {
                        class: 'ogl-stalkPlanets',
                        'player-id': player.id,
                    }),
                );
                const planets = this.updateStalk(player.planets);
          ...
          && actBtn.addEventListener('click', () => {
              ...
              if ($('#galaxyLoading').is(':visible')) return;
              const active = sideStalk.querySelectorAll('a.ogl-active');
              let next = active.length > 0
          ...
          && watchlistBtn.addEventListener('click', () => {
              ...
              this.json.sideStalk
                  ...
                  .forEach((id) => {
                      dataHelper.getPlayer(id).then((player) => {
                          const playerDiv = sideStalk.appendChild(
    ...
    checkDebris() {
        if (this.page == 'galaxy') {
            ...
            document.querySelectorAll('.cellDebris').forEach((element) => {
                const debris = element.querySelector('.ListLinks');
                if (debris && !debris.classList.contains('ogl-debrisReady')) {
                    ...
                    debris.querySelectorAll('.debris-content').forEach((resources) => {
                        const value = this.removeNumSeparator(
            ...
            if (expeBox && !expeBox.classList.contains('ogl-done')) {
                const content = expeBox.querySelectorAll('.ListLinks li');
                expeBox.classList.add('ogl-done');
                const scouts = content[2];
                const action = content[3];
                const res = [
    ...
    spyTable() {
        ...
        const tab = document.querySelector('#tabs-nfFleets.ui-state-active')
      ...
      || document.querySelector('.ogl-spyTable')
        ...
        messages.forEach((msg) => {
            ...
            || msg.querySelector('span.player').getAttribute('data-title'),
                ...
                const date = msg.querySelector('.msg_date').innerText;
                const timestamp = this.dateStrToDate(date).getTime();
                ptreJSON[id] = {};
        ...
        || !msg.querySelector('.msg_content .resspan')
            ...
            msg.classList.add('ogl-reportReady');
            const data = msg.querySelectorAll('.compacting');
            const rawDate = msg.querySelector('.msg_date').textContent.split(/\.| /g);
            const cleanDate = new Date(
                `${rawDate[2]}-${rawDate[1]}-${rawDate[0]} ${rawDate[3]}`,
            );
            const deltaDate = Date.now() - cleanDate;
            const mins = deltaDate / 6e4;
            const hours = mins / 60;
            if (
                !parseInt(
            ...
            report.fleet = data[5].querySelectorAll('span').length > 0
                ? this.cleanValue(
                    data[5]
                ...
                : 'No Data';
            report.defense = data[5].querySelectorAll('span').length > 1
                ? this.cleanValue(
                    data[5]
    ...
    sortTable(arr) {
        arr.sort((a, b) => {
            ...
            } if (this.json.options.spyFilter == 'DATE') {
                return a.deltaDate - b.deltaDate;
            } if (this.json.options.spyFilter == 'COORDS') {
                return a.tmpCoords - b.tmpCoords;
            } if (this.json.options.spyFilter == 'FLEET') {
                return b.fleet - a.fleet;
            } if (this.json.options.spyFilter == 'DEF') {
        ...
        const tableOptions = this.createDOM('div', { class: 'ogl-tableOptions' });
        ...
        const cargoChoice = this.createDOM('div', { class: 'ogk-cargo' });
        const sc = cargoChoice.appendChild(
            this.createDOM('div', {
                class: 'ogl-option ogl-fleet-ship ogl-fleet-202',
            }),
        );
        const lc = cargoChoice.appendChild(
            this.createDOM('div', {
                class: 'ogl-option ogl-fleet-ship ogl-fleet-203',
            }),
        );
        const pf = cargoChoice.appendChild(
            this.createDOM('div', {
                class: 'ogl-option ogl-fleet-ship ogl-fleet-219',
            }),
        );
        const updateDefaultShip = (id) => {
        ...
        if (this.json.pbFret != 0) {
            const pb = cargoChoice.appendChild(
                this.createDOM('div', {
                    class: 'ogl-option ogl-fleet-ship ogl-fleet-210',
                }),
            );
        ...
        const cargo = header.appendChild(
            this.createDOM(
                'th',
                {},
        ...
        arr.forEach(async (report, index) => {
            const line = this.createDOM('tr');
            table.appendChild(line);
            const indexDiv = line.appendChild(this.createDOM('td', {}, index + 1));
            if (report.new) {
            ...
            const dateText = `${this.timeSince(report.cleanDate)}<br>`;
            const date = line.appendChild(
                this.createDOM(
                    'td',
            ...
            const moonContent = report.type == 3 ? '<figure class="planetIcon moon"></figure>' : '';
            const coords = line.appendChild(this.createDOM('td'));
            coords.appendChild(
                this.createDOM(
                    'a',
            ...
            const name = line.appendChild(this.createDOM('td', { class: 'ogl-name' }));
            const status = {
                '': 'status_abbr_active',
        ...
        <= Math.round((report.total * report.loot) / 100)
            ...
            const shipId = this.json.options.spyFret;
            let shipCount;
            if (
                report.defense == 0
        && report.fleet == 0
        && this.json.options.spyFret == 210
            ...
            const ship = line.appendChild(this.createDOM('td'));
            ship.appendChild(
                this.createDOM(
                    'a',
                    {
            ...
            const colorsContainer = line.appendChild(this.createDOM('td'));
            const colors = colorsContainer.appendChild(
                this.createDOM('div', {
                    class: 'ogl-colors',
            ...
            const simulateBtn = opt.appendChild(
                this.createDOM('a', { class: 'ogl-text-btn' }, 'T'),
            );
            if (this.json.options.ptreTK) {
                const ptreBtn = opt.appendChild(
            ...
            const attackBtn = opt.appendChild(
            ...
            const deleteBtn = opt.appendChild(
            ...
            deleteBtn.addEventListener('click', (element) => {
                ...
                // debugger;
                this.autoQueue.enqueue(() => this.deleteMSg(msgId).then((res) => {
                    line.remove();
    ...
    sideLock(add) {
        ...
        const handleLock = (coords, planet) => {
            ...
            if (missing) {
                ...
                const deleteBtn = div.appendChild(
                    this.createDOM('div', {
                        style: 'width: 75px;',
                ...
                btn.addEventListener('click', () => {
                    const type = moon ? 3 : 1;
                    const link = `?page=ingame&component=fleetdispatch&galaxy=${
                        splittedCoords[0]
    ...
    preselectShips() {
        if (this.page == 'fleetdispatch') {
            fleetDispatcher.shipsOnPlanet.forEach((ship) => {
                const param = this.rawURL.searchParams.get(`am${ship.id}`);
                if (param) {
    ...
    getMarkedPlayers(markerList) {
        const playerList = [];
        const markerListLength = Object.keys(markerList).length;

        if (markerList) {
            Object.keys(markerList).forEach((key, index) => {
    ...
    calcNeededShips(options) {
        ...
        const type = options.fret || this.json.options.fret;
        let fret;
        if (type == 202) {
    ...
    calcAvailableFret(shipAmount) {
        const fret = this.json.options.fret == 203 ? this.json.gtFret : this.json.ptFret;
    ...
    async getObjLastElements(obj, elementsToReturn) {
        ...
        const nbElements = keyList.length;
        const startIndex = nbElements - elementsToReturn;
        const currentObj = {};
        if (startIndex <= 0) {
            return obj;
        }
    ...
    async checkPantrySync(pantryKey) {
        ...
        const lastLocalSync = this.json.pantrySync;
        ...
        if (syncRequest?.ok) {
            ...
            const responseText = await syncRequest?.text();
            if (
                !syncRequest
        ...
        const lastPantryTry = sessionStorage.getItem('lastPantryTry')
    ...
    async pantrySync(pantryKey, mainSyncObj, action = 1) {
        ...
        const loadIcon = this.createDOM('span', { class: 'ogi-loader' });
        const loadPantrySync = this.createDOM('div', {
            id: 'ogi-pantry-sync',
            class: 'ogi-loader-container',
        });
        const loaderText = this.createDOM('span', { class: 'ogi-loader-text' });
        ...
        if (action === 0) {
            const expeJsonObj = {};
            const expeSumsJsonObj = {};
            const combatJsonObj = {};
            const combatsSumsJsonObj = {};
            const harvestJsonObj = {};
            const mainSyncJsonObj = {};
            const pantrySync = { pantrySync: Date.now() };
            ...
            Promise.allSettled([
                ...
                for (let i = 0; i < requestsPromises.length; i++) {
                    if (requestsPromises[i].status === 'rejected') {
                        ...
                        const response = requestsPromises[i].value;
            ...
            await Promise.allSettled([
                ...
                if (success) {
                    ...
                    this.saveData();
                    console.info('[OGInfinity] - Pantry synchronisation complete');
                    const toastText = 'OGInfinity - Pantry synchronisation complete.';
    ...
    showToast(text, type = 'info', icon = 'info', title = null, duration = 3500) {
        ...
        const toastHtml = this.createDOM('div', {
            class: `ogi-toast ogi-toast-${type}`,
        });
        const toastContainer = this.createDOM('div', {
            class: 'ogi-toast-container',
        });
        const toastBody = this.createDOM('span', { class: 'ogi-toast-body' });
        const toastLogoContainer = this.createDOM('span', {
            class: 'ogi-toast-logo',
        });
        const toastLogo = this.createDOM('div', { class: 'material-icons' });
        toastLogo.textContent = icon;
        toastBody.textContent = text;
        if (title) {
            const toastTitle = this.createDOM('span', { class: 'ogi-toast-title' });
    ...
    formatToUnits(value) {
        ...
        return (
            (neg ? '-' : '')
      + (value / 10 ** (order * 3)).toFixed(precision)
      + suffix
        );
    }
    ...
    consumption(id, lvl) {
        const baseCons = {
            1: 10, 2: 10, 3: 20, 12: 10,
        };
        const cons = baseCons[id] * lvl * 1.1 ** lvl;
    ...
    minesProduction(id, lvl, position, temp) {
        ...
        let prod = baseProd[id] * lvl * 1.1 ** lvl;
        if (id == 3) {
            prod *= 1.44 - 0.004 * temp;
        }
        if (id == 12) {
            prod = 30 * lvl * (1.05 + tech113 * 0.01) ** lvl;
        }
        prod *= positionBonus;
        if (id == 1 || id == 2 || id == 3) {
            prod *= this.json.speed;
        }
    ...
    production(id, lvl, total, coords) {
        ...
        const position = coords.split(':')[2];
        let prodFactor = 1;
        let tech113; let tech122; let
            temp;
        const mines = this.json.myMines[coords];
        if (this.commander) {
            ...
            const energyMet = this.consumption(
                1,
                mines.metal.replace('(', '').replace(')', ''),
            );
            const energyCri = this.consumption(
                2,
                mines.crystal.replace('(', '').replace(')', ''),
            );
            const energyDeut = this.consumption(
                3,
                mines.deuterium.replace('(', '').replace(')', ''),
            );
            const energy = energyMet + energyCri + energyDeut;
            if (mines.energy < 0) {
                prodFactor = (energy + mines.energy) / energy;
            }
        }
        const percentBonus = 0;
        let maxCrawlerCount = (Number(mines.metal) + Number(mines.crystal) + Number(mines.deuterium))
      * 8;
        if (this.geologist) {
            maxCrawlerCount *= 1.1;
        }
        const crawlerBonus = Math.min(mines.crawlers, maxCrawlerCount)
      * (this.playerClass == PLAYER_CLASS_MINER ? 0.003 : 0.002);
        ...
        const plasmaFactor = { 1: 0.01, 2: 0.0066, 3: 0.0033 };
        let initProd = 0;
        if (id == 1) {
            initProd += 30;
        } else if (id == 2) {
        ...
        if (total) prod *= prodFactor;
        prod *= positionBonus;
        tech122 = 12;
        const plasma = prod * plasmaFactor[id] * tech122;
        const crawlers = prod * crawlerBonus;
        const geologist = prod * geologistFactor;
        const bonus = prod * percentBonus;
        const miner = (this.playerClass == PLAYER_CLASS_MINER ? 1 : 0) * 0.25 * prod;
        let totalProd = initProd + prod + geologist + plasma + crawlers + miner + bonus;
        if (id == 1 || id == 2 || id == 3) {
            prod *= this.json.speed;
            totalProd *= this.json.speed;
    ...
    research(id, lvl, labs, technocrat, explorer) {
        ...
        if (id == 199) {
            ...
            cost[0] = Math.ceil((4e3 * 1.75 ** (lvl - 1)) / 100) * 100;
            cost[1] = Math.ceil((8e3 * 1.75 ** (lvl - 1)) / 100) * 100;
            cost[2] = Math.ceil((4e3 * 1.75 ** (lvl - 1)) / 100) * 100;
            cost[0] = 4e3 * 1.75 ** (lvl - 1);
            cost[1] = 8e3 * 1.75 ** (lvl - 1);
            cost[2] = 4e3 * 1.75 ** (lvl - 1);
        }
    ...
    getRobotsNanites(id, lvl, time) {
        for (let i = 0; i < 15; i++) {
            for (let j = 0; j < 25; j++) {
                const newTime = this.building(id, lvl, j, i).time;
                if (
                    time
    ...
    getLabs(id, lvl, time, technocrat, explorer) {
        for (let i = 0; i < 300; i++) {
            const newTime = this.research(id, lvl, i, technocrat, explorer).time;
            if (
    ...
    building(id, lvl, robotic, nanite) {
        ...
        let time = (cost[0] + cost[1])
      / (2500 * Math.max(4 - lvl / 2, 1) * (1 + robotic) * 2 ** nanite);
        if (id == 15 || id == 36 || id == 43 || id == 42 || id == 41) {
            time = ((cost[0] + cost[1]) / 2500)
        * (1 / (1 + robotic))
        * 0.5 ** nanite;
        }
    ...
    keyboardActions() {
        const closeDialog = () => {
            const overlay = document.querySelector('.ogl-dialogOverlay.ogl-active');
            const btn = document.querySelector('.ogl-dialog .btn_blue.save')
    ...
    popup(header, content) {
        ...
        if (!dialog) {
            ...
            const close = dialog.appendChild(
        ...
        const top = dialog.querySelector('header')
      || dialog.appendChild(this.createDOM('header'));
        const body = dialog.querySelector('.ogl-dialogContent')
    ...
    trashsimTooltip(container, fleetinfo) {
        const ctn = container.appendChild(
            this.createDOM('div', { style: 'display:flex;justify-content:center;' }),
        );
        const btn = ctn.appendChild(
            this.createDOM('div', { class: 'ogk-trashsim tooltip' }),
        );
        const p = this.createDOM('div');
        p.html(fleetinfo);
        const ships = {};
        const t = p.querySelectorAll('.fleetinfo tr');
        Array.from(p.querySelectorAll('.fleetinfo tr')).forEach((elem) => {
            if (elem.children[1]) {
                const name = elem.children[0].innerText.slice(0, -1);
                const count = elem.children[1].innerText.split('.').join('');
                if (count > 0) {
                    ships[this.json.shipNames[name]] = { count };
                }
            }
        });
        const apiTechData = {};
    ...
    betterHighscore() {
        if (this.page == 'highscore') {
            const addTooltip = () => {
                const positions = document.querySelectorAll('#ranks tbody tr');
                positions.forEach((position) => {
                    if (!position.classList.contains('ogi-ready')) {
                        position.classList.add('ogi-ready');
                        const playerDiv = position.querySelector('.playername');
                        const countDiv = position.querySelector('.score.tooltip');
                        if (countDiv) {
                ...
                || countDiv.getAttribute('data-title');
                        ...
                        const mail = position.querySelector('.sendMail');
                        if (mail) {
                            const id = mail.getAttribute('data-playerid');
            ...
            initHighscoreContent = () => {
                const active = document.querySelector('.stat_filter.active');
                let type = 0;
                if (active) {
                ...
                const positionCell = $('#ranks thead .score');
            ...
            const type = this.rawURL.searchParams.get('type');
            if (type) {
    ...
    betterAPITooltip(sender) {
        if (sender.classList.contains('icon_apikey')) {
            const data = sender.getAttribute('title') || sender.getAttribute('data-title');
        ...
        if (sender.classList.contains('show_fleet_apikey')) {
            ...
            if (data) {
                ...
                const second = data.indexOf('"', first + 7);
                sender.addEventListener('click', () => {
                    fadeBox('<br/>API Key copied in clipboard');
                    navigator.clipboard.writeText(
    ...
    overwriteFleetDispatcher(functionName, param, callback, callbackAfter) {
        const old = fleetDispatcher[functionName];
    ...
    utilities() {
        ...
        if (this.page == 'fleetdispatch') {
            ...
            const slider = this.createDOM('div', {
        ...
        if (this.page == 'movement') {
            ...
            document.querySelectorAll('.fleetDetails').forEach((fleet) => {
                const id = Number(fleet.getAttribute('id').replace('fleet', ''));
                if (id > lastFleetId && fleet.querySelector('.reversal a')) {
                    lastFleetId = id;
                    lastFleetBtn = fleet.querySelector('.reversal a');
                }
                const type = fleet.getAttribute('data-mission-type');
                const originCoords = fleet
              ...
              && fleet.querySelector('.nextTimer').classList.add('friendly');
                ...
                const fleetInfo = fleet.querySelector('.fleetinfo');
                let fleetCount = 0;
                const values = fleetInfo.querySelectorAll('td.value');
                const backed = [0, 0, 0];
          ...
          + (fleet.querySelector('.destinationData moon') ? 'M' : 'P');
                ...
                const details = fleet.appendChild(
                ...
                const back = fleet.querySelector('.reversal a').title
          || fleet.querySelector('.reversal a').getAttribute('data-title');
                ...
                const content = details.appendChild(
    ...
    getTranslatedText(id) {
        ...
        const line = this.gameLang == 'fr' ? 0 : 1;
    ...
    settings() {
        ...
        const dataDiv = container.appendChild(this.createDOM('div', {}));
        ...
        const timerCheck = optiondiv.appendChild(
        ...
        const disableautofetchempirebox = optiondiv.appendChild(
            this.createDOM('input', { type: 'checkbox' }),
        );
        ...
        const ptreInput = optiondiv.appendChild(
            this.createDOM('input', {
                type: 'password',
        ...
        const srvDatas = dataDiv.appendChild(
            this.createDOM(
                'span',
                {
                    style:
        ...
        const srvDatasBtn = this.createDOM('button', { class: 'btn_blue' }, 'Update');
        ...
        if (this.json.timezoneDiff != 0) {
            const spanZone = dataDiv.appendChild(
                this.createDOM(
                    'span',
                    {
                        style:
            ...
            const timeZoneCheck = spanZone.appendChild(
                this.createDOM('input', { type: 'checkbox' }),
            );
        ...
        const expeditionsBox = dataDiv.appendChild(
            this.createDOM(
                'div',
                { class: 'ogi-checkbox' },
        ...
        const combatsBox = dataDiv.appendChild(
            this.createDOM(
                'div',
                { class: 'ogi-checkbox' },
        ...
        const targetsBox = dataDiv.appendChild(
            this.createDOM(
                'div',
                { class: 'ogi-checkbox' },
        ...
        const scanBox = dataDiv.appendChild(
            this.createDOM(
                'div',
                { class: 'ogi-checkbox' },
        ...
        const OptionsBox = dataDiv.appendChild(
            this.createDOM(
                'div',
                { class: 'ogi-checkbox' },
        ...
        const cacheBox = dataDiv.appendChild(
            this.createDOM(
                'div',
                { class: 'ogi-checkbox' },
        ...
        const purgeBox = dataDiv.appendChild(
            this.createDOM(
                'div',
                {
                    class: 'ogi-checkbox',
        ...
        const dataSpan = dataDiv.appendChild(
            this.createDOM(
                'span',
                {},
        ...
        const pantryInput = dataSpan.appendChild(
            this.createDOM('input', {
                type: 'password',
        ...
        dataDiv.appendChild(this.createDOM('hr'));
        const dataBtns = dataDiv.appendChild(
            this.createDOM('div', { style: 'display: flex;align-items: flex-end;' }),
        );
        const exportBtn = dataBtns.appendChild(
            this.createDOM('button', { class: 'btn_blue' }, 'Export'),
        );
        const fileHandler = dataBtns.appendChild(
            this.createDOM('input', {
                id: 'file',
                name: 'file',
                class: 'inputfile',
        ...
        const own3 = missionDiv.appendChild(
            this.createDOM('div', {
        ...
        const own4 = missionDiv.appendChild(
            this.createDOM('div', {
        ...
        const other3 = missionDiv.appendChild(
            this.createDOM('div', {
                class: `ogl-mission-icon ogl-mission-3 ${
        ...
        const other1 = missionDiv.appendChild(
            this.createDOM('div', {
        ...
        const expe15 = missionDiv.appendChild(
            this.createDOM('div', {
        ...
        const expe6 = missionDiv.appendChild(
            this.createDOM('div', {
        ...
        const rvalInput = optiondiv.appendChild(
            this.createDOM('input', {
                type: 'text',
        ...
        const expeditionDefaultTime = optiondiv.appendChild(
            this.createDOM('input', {
                type: 'text',
                class: 'ogl-rvalInput ogl-formatInput tooltip',
        ...
        const fleetActivity = settingDiv.appendChild(
            this.createDOM(
                'div',
                { class: 'ogi-checkbox' },
        ...
        resetBtn.addEventListener('click', () => {
            ...
            if (reset) {
                ...
                if (!expeditionsBox.children[1].checked) {
                    ...
                    for (const id in this.json.harvests) {
                        if (this.json.harvests[id].coords.split(':')[2] == 16) {
                            json.harvests[id] = this.json.harvests[id];
                        }
                    }
                    for (const id in this.json.combats) {
                        if (this.json.combats[id].coordinates.position == 16) {
