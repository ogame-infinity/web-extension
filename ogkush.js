...
class OGLight {
  constructor() {
    ...
    this.current.planet = (document.querySelector('.smallplanet .active') ||
                           document.querySelector('.smallplanet .planetlink'))
                              .parentNode;
    this.current.coords = this.current.planet.querySelector('.planet-koords')
  ...
  minesLevel() {
    ...
    this.planetList.forEach((planet) => {
      let coords =
          planet.querySelector('.planet-koords').textContent.slice(1, -1);
  ...
  topBarUtilities() {
    ...
    document.querySelector('#bar').appendChild(this.createDOM(
        'span', {class: 'ogk-ping'},
  ...
  jumpGate() {
    ...
    for (const [coords, t] of Object.entries(this.json.jumpGate)) {
      let time = new Date(t);
      this.planetList.forEach((planet) => {
        if (planet.querySelector('.planet-koords').innerText.slice(1, -1) ==
            coords) {
          ...
          dataHelper.getExpeditionType(textContent).then((type) => {
            ...
            ressources.forEach((res, i) => {
              ...
              if (found) {
                ...
                type = normalized[i];
                sums.found[i] +=
                    Number(this.removeNumSeparator(split[split.length - 1]));
              }
            });
            // Fleet
  ...
  betterFleetDispatcher() {
    if (this.page == 'fleetdispatch' && document.querySelector('#civilships') &&
      ...
      let slider = briefing.appendChild(
          this.createDOM('div', {class: 'ogl-fleetSpeed'}, `
  ...
  autoHarvest() {
    this.planetList.forEach((p, index) => {
      ...
      if (this.mode == 3) {
        if (p.querySelector('.planet-koords').textContent.slice(1, -1) ==
            this.json.autoHarvest[0]) {
          let target = this.json.autoHarvest[1] == 3 ?
    ...
    let currentIndex =
        parseInt(this.current.planet.getAttribute('data-redirect'));
    let nextIndex =
        currentIndex + 1 >= this.planetList.length ? 0 : currentIndex + 1;

    this.next = {};
    this.next.planet =
        document.querySelector(`.smallplanet[data-redirect="${nextIndex}"]`);
    this.next.coords = this.next.planet.querySelector('.planet-koords')
                           .textContent.slice(1, -1);
    this.next.id = this.next.planet.getAttribute('id').replace('planet-', '');
    this.next.hasMoon =
        this.next.planet.querySelector('.moonlink') ? true : false;
    ...
    if (this.page == 'fleetdispatch') {
      if (this.mode == 3) {
        ...
        let link = `?page=ingame&component=fleetdispatch&galaxy=${
            coords[0]}&system=${coords[1]}&position=${coords[2]}&type=${
            type}&mission=${this.json.options.harvestMission}&cp=${
            id}&oglMode=${this.mode}`;
        link =
            'https://' + window.location.host + window.location.pathname + link;

        let skip = document.querySelector('#fleet1 #planet')
                       .appendChild(this.createDOM(
                           'button', {class: 'btn_blue ogl-skip'},
      ...
      if (res) {
        ...
        if (mines && mines.metal && mines.temperature) {
          ...
          metProd = this.production(
              1, Number(mines.metal.replace('(', '').replace(')', '')), true,
              coords);
          criProd = this.production(
              2, Number(mines.crystal.replace('(', '').replace(')', '')), true,
              coords);
          deutProd = this.production(
              3, Number(mines.deuterium.replace('(', '').replace(')', '')),
              true, coords);
...
`);
  ...
  sortTable(arr) {
    ...
    let cargo = header.appendChild(this.createDOM(
        'th', {},
        `<span class="ogl-option ogl-fleet-ship ogl-fleet-${
  ...
  loopUtilities() {
    ...
    if (this.page == 'galaxy') {
      if (document.querySelector('a[onclick*="sendShips"')) {
        let ships = document.querySelector('a[onclick*="sendShips"')
                        .getAttribute('onclick')
                        .match(/\d+/g)
                        .map(Number);
