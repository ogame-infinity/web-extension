...
let dotted = (value) => parseInt(value).toLocaleString("en-EN");
...
class OGInfinity {
  ...
  technoDetail() {
    if (this.page == "research" || this.page == "supplies" || this.page == "facilities" || this.page == "shipyard" || this.page == "defenses") {
      ...
      let currentRes = [document.querySelector("#resources_metal").getAttribute("data-raw"), document.querySelector("#resources_crystal").getAttribute("data-raw"), document.querySelector("#resources_deuterium").getAttribute("data-raw")];
      ...
      let updateResearchDetails = (technoId, baselvl, tolvl) => {
        ...
        if (that.page == "supplies") {
          ...
          if (consDiv) {
            ...
            consDiv.html(`<span>${(currentCons - baseCons).toLocaleString("en-EN")}<span class="${diff < 0 ? "overmark" : "undermark"}"> (${diff.toLocaleString("en-EN")})</span></span>`);
            prodDiv.html(`<strong>Production :</strong> <span class="value">${parseInt(baseProd).toLocaleString("en-EN")} <span class="bonus"> (+${parseInt(baseProd - currentProd).toLocaleString("en-EN")})</span></span>`);
          }
          if (energyDiv) {
            let currentProd = that.production(technoId, baselvl - 1, false);
            let baseProd = that.production(technoId, tolvl, false);
            energyDiv.html(`<span class="value">${parseInt(baseProd).toLocaleString("en-EN")} <span class="bonus"> (+${parseInt(baseProd - currentProd).toLocaleString("en-EN")})</span></span>`);
          }
        }
        ...
        if (techno.cost[0] != 0) {
          ...
          metal.setAttribute("data-title", parseInt(techno.cost[0]).toLocaleString("en-EN"));
          if (baselvl != tolvl) {
        ...
        if (techno.cost[1] != 0) {
          ...
          crystal.setAttribute("data-title", parseInt(techno.cost[1]).toLocaleString("en-EN"));
          if (baselvl != tolvl) {
        ...
        if (techno.cost[2] != 0) {
          ...
          deuterium.setAttribute("data-title", parseInt(techno.cost[2]).toLocaleString("en-EN"));
          if (baselvl != tolvl) {
        ...
        if (techno.cost[3] != 0) {
          ...
          if (energy) {
            ...
            energy.setAttribute("data-title", parseInt(techno.cost[3]).toLocaleString("en-EN"));
            if (baselvl != tolvl) {
          ...
          if ([202, 203, 208, 209, 210, 204, 205, 206, 219, 207, 215, 211, 212, 217, 213, 218, 214, 401, 402, 403, 404, 405, 406, 407, 408, 502, 503].includes(technologyId)) {
            ...
            let updateShipDetails = (value) => {
              ...
              if (technologyId == 212) {
                ...
                energyDiv.html((value * base).toLocaleString("en-EN") + `<span class="${diff < 0 ? "overmark" : "undermark"}"> (${diff.toLocaleString("en-EN")})</span>`);
    ...
    if (this.page == "fleetdispatch" && document.querySelector("#civilships") && fleetDispatcher.shipsOnPlanet.length != 0) {
      ...
      svg.addEventListener("mouseover", () => {
        document.querySelectorAll("#shipsChosen .technology").forEach((elem) => {
          ...
          elem.appendChild(this.createDOM("span", { class: "ogi-speed" }, fleetDispatcher.fleetHelper.shipsData[id].speed.toLocaleString("en-EN")));
        });
      });
      ...
      document.querySelectorAll(".msg_actions").forEach((elem) => {
        ...
        if (keyNode) {
          ...
          if (key.startsWith("sr")) {
            ...
            link.addEventListener("click", () => window.open(`https://ogotcha.universeview.be/${this.univerviewLang}?CR_KEY=${key}`, "_blank", `location=yes,scrollbars=yes,status=yes,width=${screen.availWidth},height=${screen.availHeight}`));
          }
        }
      });
      setTimeout(() => {
  ...
  generalStats(player) {
    ...
    let innerAstro = prod.appendChild(
      this.createDOM("span", {
        style: "display: flex; align-items: center; margin-left: auto; margin-top: 10px;",
      })
    );
  ...
  playerSearch(show, name) {
    let renderPlayerInfo = (player) => {
      ...
      let name = `<span>${player.name}</span> <span class="${this.getPlayerStatus(player.status)}"></span>\n                  <a target="_self"\n                    href="https://s${this.universe}-${this.gameLang}.ogame.gameforge.com/game/index.php?page=highscore&searchRelId=${
        player.id
    ...
    let updatePlayerList = (players, forced) => {
      players.forEach(async (player, index) => {
        ...
        let name = this.createDOM("span", { class: this.getPlayerStatus(player.status, noob) }, `${player.name} ${player.status == "" ? "" : "(" + player.status + ") "}`);
  ...
  minesStats() {
    ...
    this.planetList.forEach((planet) => {
      ...
      let link = `?page=ingame&component=supplies&cp=${planet.id}`;
      header.appendChild(
        this.createDOM("th", {}, `<p>${planet.name}</p> <a href="${link}" class="ogl-fleet-coords">${planet.coordinates}</a> <span class="ogl-planet-fields">${planet.fieldUsed} / ${planet.fieldMax}</span><br><span class="ogl-planet-temperature">${planet.temperature}ºC</span>`)
      );
      let td = metalRow.appendChild(this.createDOM("td"));
  ...
  betterFleetDispatcher() {
    if (this.page == "fleetdispatch" && document.querySelector("#civilships") && fleetDispatcher.shipsOnPlanet.length != 0) {
      ...
      let update = (submit) => {
        ...
        consDiv.innerText = fleetDispatcher.getConsumption().toLocaleString("en-EN");
      ...
      let refreshRes = () => {
        ...
        if (firstResRefresh && fleetDispatcher.currentPage == "fleet1") {
          ...
          mLeft.innerText = Math.max(0, metalAvailable - val).toLocaleString("en-EN");
          val = this.removeNumSeparator(document.querySelector("input#crystal").value);
          cLeft.innerText = Math.max(0, crystalAvailable - val).toLocaleString("en-EN");
          val = this.removeNumSeparator(document.querySelector("input#deuterium").value);
          dLeft.innerText = Math.max(0, deutAvailable - fleetDispatcher.getConsumption() - val).toLocaleString("en-EN");
        }
      };
      let kept = this.json.options.kept[this.current.coords + this.current.isMoon ? "M" : "P"] || this.json.options.defaultKept;
      ...
      let updateCargo = () => {
        ...
        bar.html(
          `<div class="fleft bar_container" data-current-amount="0" data-capacity="0">\n        <div class="filllevel_bar"></div>\n        </div>\n        <div>\n        <span class="${freeSpace >= 0 ? "undermark" : "overmark"}">${freeSpace.toLocaleString(
            "en-EN"
          )} </span>\n        / <span> ${fleetDispatcher.getCargoCapacity().toLocaleString("en-EN")}</span>\n        </div>`
        );
      ...
      let onResChange = (index) => {
        ...
        if (index == 2) {
          ...
          let old = deutLeft.innerText;
          deutLeft.innerText = (deutAvailable - fleetDispatcher.getConsumption() - fleetDispatcher.cargoDeuterium).toLocaleString("en-EN");
          ...
          if (fleetDispatcher.getFreeCargoSpace() == 0 && deutLeft.innerText != "0") {
            ...
            deutReal.innerText = Math.max(0, fleetDispatcher.cargoDeuterium).toLocaleString("en-EN");
          } else {
            deutLeft.classList.remove("overmark");
            let currentDeut = deutAvailable - fleetDispatcher.getConsumption();
            deutReal.innerText = currentDeut.toLocaleString("en-EN");
          }
          if (filled > Math.max(0, deutAvailable - fleetDispatcher.getConsumption())) {
            deutFiller.value = (deutAvailable - fleetDispatcher.getConsumption()).toLocaleString("en-EN");
          }
        } else if (index == 1) {
          ...
          crystalLeft.innerText = (crystalAvailable - fleetDispatcher.cargoCrystal).toLocaleString("en-EN");
          if (fleetDispatcher.getFreeCargoSpace() == 0 && crystalLeft.innerText != "0") {
            crystalLeft.classList.add("overmark");
            crystalReal.innerText = Math.max(0, fleetDispatcher.cargoCrystal).toLocaleString("en-EN");
          } else {
            crystalLeft.classList.remove("overmark");
            crystalReal.innerText = crystalAvailable.toLocaleString("en-EN");
          }
        } else if (index == 0) {
          ...
          metalLeft.innerText = (metalAvailable - fleetDispatcher.cargoMetal).toLocaleString("en-EN");
          if (fleetDispatcher.getFreeCargoSpace() == 0 && metalLeft.innerText != "0") {
            metalLeft.classList.add("overmark");
            metalReal.innerText = Math.max(0, fleetDispatcher.cargoMetal).toLocaleString("en-EN");
          } else {
            metalLeft.classList.remove("overmark");
            metalReal.innerText = metalAvailable.toLocaleString("en-EN");
          }
        }
        let ships = {};
    ...
    if (this.page == "fleetdispatch" && document.querySelector("#shipChosen")) {
      shipsOnPlanet.forEach((ship) => {
        if (ship.id == 202 || ship.id == 203) {
          ...
          let span = this.createDOM("span", { class: "ogl-needed" }, amount.toLocaleString("en-EN"));
            ...
            this.json.sideStalk
              ...
              .forEach((id) => {
                dataHelper.getPlayer(id).then((player) => {
                  ...
                  playerDiv.appendChild(this.createDOM("span", { class: this.getPlayerStatus(player.status) }, player.name));
    ...
    arr.forEach(async (report, index) => {
      ...
      ship.appendChild(
        this.createDOM(
          ...
          shipCount.toLocaleString("en-EN")
        )
      );
  ...
  checkInputs() {
    ...
    document.querySelectorAll("input.ogl-formatInput").forEach((input) => {
      ...
      if (input.value) input.value = parseInt(this.removeNumSeparator(input.value, true)).toLocaleString("en-EN");
    });
  }
  ...
  utilities() {
    ...
    if (this.page == "fleetdispatch") {
      ...
      for (let id in data) {
        let infos = `\n            <div class="ogl-fleetInfo">\n                ${data[id].name}\n                <hr>\n                <div><span>Fret</span>${data[id].cargoCapacity.toLocaleString("en-EN")}</div>\n                <div><span>${this.getTranslatedText(
          19
        )}</span>${data[id].speed.toLocaleString("en-EN")}</div>\n                <div><span>Conso</span>${data[id].fuelConsumption.toLocaleString("en-EN")}</div>\n            </div>`;
  ...
  settings() {
    ...
    dataDiv.appendChild(this.createDOM("div", { class: "ogi-checkbox" }, `<strong class="undermark">Contribute or bug report</strong>\n        <a target="_blank" href="https://discord.gg/9aMdQgk"> Discord </span>`));
    // dataDiv.appendChild(
    //   this.createDOM(
    //     "div",
    ...
    optiondiv = dataDiv.appendChild(this.createDOM("span", {}, '<a href="https://ptre.chez.gg/" target="_blank">PTRE</a> Teamkey'));
