...
class OGInfinity {
  ...
  async checkPantrySync(pantryKey) {
    ...
    let syncRequest = await fetch(
      `https://getpantry.cloud/apiv1/pantry/${pantryKey}/basket/${this.universe}-${this.gameLang}-sync`,
  ...
  async pantrySync(pantryKey, action = 1) {
    ...
    if (action === 0) {
      ...
      const targetsRequest = fetch(
        `https://getpantry.cloud/apiv1/pantry/${pantryKey}/basket/${this.universe}-${this.gameLang}-targets`,
        {
          priority: "low",
          method: "POST",
      ...
      const otherSettingsRequest = fetch(
        `https://getpantry.cloud/apiv1/pantry/${pantryKey}/basket/${this.universe}-${this.gameLang}-settings`,
        {
          priority: "low",
          method: "POST",
          headers: pantryHeaders,
  ...
  settings() {
    ...
    let di = dataDiv.appendChild(
      this.createDOM("div", { style: "margin: 10px 0px 0px 38px" })
    );
    di.html(
