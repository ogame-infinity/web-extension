...
class OGInfinity {
  ...
  topBarUtilities() {
    ...
    bar.appendChild(
      createDOM("li").appendChild(
        createDOM("a", { href: `https://obatsim.stevecohen.fr/${this.univerviewLang}`, target: "_blank" }, "Trash")
      ).parentElement
    );
    bar.appendChild(
      createDOM("li").appendChild(
        ...
        if (keyNode) {
          ...
          if (key.startsWith("sr")) {
            ...
            link.addEventListener("click", () => {
              ...
              window.open(
                `https://obatsim.stevecohen.fr/${this.univerviewLang}?SR_KEY=${key}#prefill=${base64}`,
                "_blank"
              );
            });
  ...
  sortTable(arr) {
    ...
    arr.forEach(async (report, index) => {
      ...
      simulateBtn.addEventListener("click", () => {
        ...
        let base64 = btoa(JSON.stringify(json));
        window.open(
          `https://obatsim.stevecohen.fr/${this.univerviewLang}?SR_KEY=${report.apiKey}#prefill=${base64}`,
          "_blank"
        );
      });
  ...
  trashsimTooltip(container, fleetinfo) {
    ...
    btn.addEventListener("click", () => {
      ...
      let base64 = btoa(JSON.stringify(json));
      window.open(`https://obatsim.stevecohen.fr/${this.univerviewLang}?#prefill=${base64}`, "_blank");
    });
  }
